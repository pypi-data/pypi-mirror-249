...
class nn:
    ...
    def add(self, layer=None, activation=None, optimizer=None, **kw):
        ...
        #     print("增加ReLU层。")
            ...
            self.model.add_module('conv2d' + str(self.layers_num), torch.nn.Conv2d(kw['size'][0], kw['size'][1], kw['kernel_size'],stride=kw['stride'],padding=kw['padding']))
            print("增加二维卷积层，输入维度:{},输出维度：{},kernel_size: {} ".format(kw['size'][0], kw['size'][1], kw['kernel_size']))
        elif layer == 'conv1d':
            self.model.add_module('conv1d' + str(self.layers_num), torch.nn.Conv1d(kw['size'][0], kw['size'][1], kw['kernel_size'],stride=kw['stride'],padding=kw['padding']))
            print("增加一维卷积层，输入维度:{},输出维度：{},kernel_size: {} ".format(kw['size'][0], kw['size'][1], kw['kernel_size']))
        elif layer == 'maxpool':
            self.model.add_module('maxpooling' + str(self.layers_num), torch.nn.MaxPool2d(kw['kernel_size'],stride=kw['stride'],padding=kw['padding']))
            print("增加最大池化层,kernel_size: {} ".format(kw['kernel_size']))
        elif layer == 'avgpool':
            self.model.add_module('avgpooling' + str(self.layers_num), torch.nn.AvgPool2d(kw['kernel_size'],stride=kw['stride'],padding=kw['padding']))
            ...
            if 'stride' not in kw.keys():
                kw['stride'] = 1
            self.model.add_module('res_stage'+str(self.layers_num),ResNet(BasicBlock, kw['num_blocks'],kw['size'],kw['stride']))
            print("增加BasicBlock(ResNet)层, 输入维度：{}, 输出维度：{}, 层数： {} ,步长：{}".format(kw['size'][0], kw['size'][1], kw['num_blocks'],kw['stride']))
        elif layer == 'res_bottleneck':
            if  'num_blocks' not in kw.keys():
                kw['num_blocks'] = 1
            if 'stride' not in kw.keys():
                kw['stride'] = 1
            self.model.add_module('res_stage'+str(self.layers_num),ResNet(Bottleneck, kw['num_blocks'],kw['size'],kw['stride']))
            print("增加Bottleneck(ResNet)层, 输入维度：{}, 输出维度：{}, 层数： {} ,步长：{}".format(kw['size'][0], kw['size'][1], kw['num_blocks'],kw['stride']))
