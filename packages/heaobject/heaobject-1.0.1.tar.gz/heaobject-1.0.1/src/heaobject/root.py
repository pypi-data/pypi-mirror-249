...
try:
    ...
    def _get_attributes(self, type_attrs):
        return tuple(m[0] for m in inspect.getmembers_static(self, lambda x: not inspect.isroutine(x)) \
                     if not m[0].startswith('_') and (inspect.isdatadescriptor(m[1]) or m[0] not in type_attrs))
except AttributeError:
    def _get_type_attributes(cls):
        return inspect.getmembers(cls, lambda x: not inspect.isroutine(x))
    def _get_attributes(self, type_attrs):
        return tuple(m[0] for m in inspect.getmembers(self, lambda x: not inspect.isroutine(x)) \
...
class AbstractHEAObject(HEAObject, abc.ABC):
    ...
    def __init__(self) -> None:
        super().__init__()
        self.__attributes: tuple | None = None
    ...
    def get_attributes(self) -> Iterator[str]:
        if self.__attributes is None:
            self.__attributes = _get_attributes(self, self.__get_type_attributes())
    ...
    def __setattr__(self, attr, value):
        if hasattr(self, '_AbstractHEAObject__attributes') and not hasattr(self, attr):
            super().__setattr__('_AbstractHEAObject__attributes', None)
