...
def display_longitude_latitude(json_code, full_list, call_back):
    ...
    display(selection_radio_buttons, hbox_longitude, submit_button, output)
...
def color_columns(df, color, column_indices):
    ...
    def apply_color(x):
        return ['background-color: ' + color if i in column_indices else '' for i in range(len(x))]
...
def display_and_ask_removal(df, missing_column_indices):
    ...
    df_sample = df[:5].copy()
    df_sample = color_columns(df_sample, 'lightgreen', missing_column_indices)
...
def color_columns_multiple(df, colors, column_indices_list):
    def apply_color(x):
        ...
        for color, indices in zip(colors, column_indices_list):
            for i in indices:
                color_style[i] = 'background-color: ' + color
        return color_style
...
def display_duplicated_columns_html(df, duplicate_column_indices):
    ...
    styled_df = color_columns_multiple(df_sample, colors, duplicate_column_indices)
...
NOT IMPLEMENTED YET!"""
    ...
    def display(self):
        if hasattr(self, 'sample_df'):
            ...
            self.sample_df.display()
            
            if not hasattr(self, 'output_sample_df'):
                self.output_sample_df = self.run_codes(self.sample_df)
            
            display(HTML(f"<hr><b>Example Output</b>:"))
            self.output_sample_df.display()
...
def CRS_select(callbackfunc=None):
    ...
    def on_submit_clicked(b):
        ...
        if checkbox1.value and not checkbox2.value:
            ...
            crs_string = f"+proj={proj_dropdown.value} +lat_0={lat_0.value} +lon_0={lon_0.value} +lat_1={lat_1.value} +lat_2={lat_2.value} +ellps={ellps_dropdown.value} +datum={datum_dropdown.value} +x_0={false_easting.value} +y_0={false_northing.value} +k={scale_factor.value}"
...
def plot_efficient_grid(df, x_col, y_col, value_col=None):
    ...
    pivot_table = df.pivot_table(index=y_col, columns=x_col, values=value_col, aggfunc=aggfunc, fill_value=0)

    if isinstance(pivot_table.columns, pd.MultiIndex):
        pivot_table.columns = pivot_table.columns.get_level_values(1)
    
    pivot_table_filled = pivot_table.astype(pd.SparseDtype("float", fill_value=np.nan))

    cmap = plt.cm.viridis
    cmap.set_under('white')

    plt.figure(figsize=(6, 3))
    ax = sns.heatmap(pivot_table_filled, cmap=cmap, linewidths=0, cbar=True, square=True, fmt=".1f", vmin=0)
...
def plot_bounding_boxes(bounding_boxes):
    ...
    for i, box in enumerate(bounding_boxes):
        ...
        rect = patches.Rectangle((left, bottom), right - left, top - bottom, linewidth=1, edgecolor=colors[i % len(colors)], facecolor='none', label=f'Box {i+1}')
...
class GeoDataCleaning(DataCleaning):
    ...
    def add_resample_step(self):
        ...
        print("🧐 Please specify the target resolution:")
