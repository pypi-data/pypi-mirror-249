    ...
    for item_description in items:
        ...
        if kind == "curve":
            ...
            if xaxis_channel_name is not None:
                ...
                xchannel = item = plot_model.XIndexChannelRef(plot, None)
...
def _get_stable_device_name(device: scan_model.Device) -> str:
    """
    Reach a name which is stable between 2 scans.

    FIXME: This have to be provided by the scan_info
    """
    if device.type() in [scan_model.DeviceType.MOSCA]:
...
def _initialize_image_plot_from_device(
    ...
    if is_default:
        stable_name = _get_stable_device_name(device)
    ...
    if is_default:
        if device.type() in [scan_model.DeviceType.LIMA, scan_model.DeviceType.LIMA2]:
        ...
        def parse_channels(d: scan_model.Device):
            ...
            for channel in d.channels():
                ...
                if plot is None:
                    ...
                    device_name = _get_stable_device_name(device)
    ...
    for device in scan.devices():
        ...
        for channel in device.channels():
            ...
            if plot is None:
                ...
                device_fullname = device.fullName()
            ...
            if xaxis_channel_name is not None:
                ...
                xchannel = plot_model.XIndexChannelRef(plot, None)
            item.setXChannel(xchannel)
