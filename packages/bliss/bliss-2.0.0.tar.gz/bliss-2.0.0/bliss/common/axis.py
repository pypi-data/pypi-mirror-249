...
class GroupMove:
    ...
    def _fill_motions_dict(self, motions_dict, axis_pos_dict, relative, polling_time):
        ...
        This is a recursive function, in order to ensure motions_dict will only refer to
        real physical motors (see issue #1388)
        """
        ...
        for controller, motions in motions_by_controller.items():
            ...
            if isinstance(controller, CalcController):
                # dig up to the real physical motors
    ...
    def move(
        ...
        # can lead to the same position of reals;
        # so use "internal_position" signal of reals to force pseudos position update
        # and exclude the "already in place reals" from the motion list.
        excluded_axes = set()
        ...
        for controller, motions in list(motions_dict.items()):
            to_delete = []
            for motion in motions:
                if math.isclose(motion.delta, 0):
                    to_delete.append(motion)
                elif isinstance(motion.axis, CalcAxis):
                    if not set(motion.axis.controller.reals) - excluded_axes:
                        # here all the reals of a pseudo are excluded (i.e already in place), so remove its motion.
                        to_delete.append(motion)

            for motion in to_delete:
                motions.remove(motion)

            if not motions:
                del motions_dict[controller]

        # return immediately if there are no reals to move at all (see possible removed motions above)
        if all(
            isinstance(motion.axis, CalcAxis)
            for motion in itertools.chain.from_iterable(motions_dict.values())
        ):
            return
    ...
    def start(
        ...
        # print([(m.axis.name, m.target_pos, m.delta) for m in self.motions_iter])
