...
class LimaAcquisitionMaster(AcquisitionMaster):
    ...
    def __init__(self, device, ctrl_params=None, **acq_params):
        ...
        if not prepare_once and not start_once:
            ...
            if acq_nb_frames == 0:
                # this case seems not possible because with acq_nb_frames = 0
                # with the simulator the state comes back to ready after each exposure
                # so that reading loop exit and because start_once is True,
                # the reading loop won t be respawned at next iter
                msg = f"{mdict} is incompatible with acq_nb_frames = 0"
                raise ValueError(msg)
                # if with real detectors camera status remains running
                # the following code could be used
                # ================================================================
                # if wait_frame_id is None:
                #     wait_frame_id = repeat(acq_nb_frames) # i.e repeat(0)
                # elif wait_frame_id is not iter(wait_frame_id):
                #     msg = f"With '{acq_trigger_mode}' and 'acq_nb_frames' == 0, "
                #     msg += "'wait_frame_id' must be provided as an infinite iterator"
                #     raise ValueError(msg)
                # self.__wait_frame_id_iterator = wait_frame_id
                # ================================================================
    ...
    def reading(self):
        ...
        while True:
            ...
            if self.fast_synchro and self.prepare_once and not self.start_once:
                if status["ready_for_next_image"]:
                    # self.__print_debug("ready event SET (ready_for_next_image)")
                    if not self.__infinite_sw_acq and last_acq_state != "ready":
                        self._ready_for_next_iter.set()
                ...
                if self.prepare_once:
                    if delta_acquired > 0:
                        ...
                        # )
                        if not self.__infinite_sw_acq and last_acq_state != "ready":
                            # lima can have acquired the last frame but still in RUNNING state
                            # and this could lead to a double _ready_for_next_iter.set() when nb_frame==0 and INTERNAL trigger
                            self._ready_for_next_iter.set()
            ...
            gevent.sleep(min(0.1, max(self.acq_params["acq_expo_time"] / 10.0, 10e-3)))
