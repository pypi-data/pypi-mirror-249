                            ...
                            for te in self.linetemp[symbol]:
                                if len(line)>0 and te["DateTime"]<=line[-1]['DateTime']:
                                    ...
                                    if "TC.O" in symbol:
                                        ...
                                        if linetype==9 or linetype==10 or linetype==19:
                                            if "TC.F" in symbol:
                                                itemkeys=["DateTime","Symbol","CTR","PTR","RCTR","RPTR","FCIV25","FPIV25","TV","ATV","HV_W4","HV_W8","HV_W13","HV_W26","HV_W52","PutD","CallD","D25CStraddle","D25PStraddle", "D25CTV","D25PTV","FIV","IV","Straddle","StraddleStrike","StraddleWeight","CIV25D","PIV25D","CIV10D","PIV10D","CallOI","PutOI","CallVol","PutVol","VIX"]
                                            else:
                                                itemkeys=["DateTime","Symbol","IV","CallOI","PutOI","CallVol","PutVol","VIX"]
            ...
            for i in range(datacout):
                if quote.GetItem(i,item):
                    if "TC.O" in quote.Symbol:
                        ...
                        if quote.GetItemData(-1, 2)=="2":
                            self.greeksputcallvol[quote.Symbol][datetime.strptime(str(item.Date)+" "+"{:0>6d}".format(item.Time),"%Y%m%d %H%M%S").replace(tzinfo=self.tz)+timedelta(hours=8)]={"CallOI":iff(item.GetOptValue("CallOI")),
                            ...
                            if quote.Symbol in self.greeksputcallvol:
                                if datetime.strptime(str(item.Date)+" "+"{:0>6d}".format(item.Time),"%Y%m%d %H%M%S").replace(tzinfo=self.tz)+timedelta(hours=8) in self.greeksputcallvol[quote.Symbol]:
                                    oivol=self.greeksputcallvol[quote.Symbol][datetime.strptime(str(item.Date)+" "+"{:0>6d}".format(item.Time),"%Y%m%d %H%M%S").replace(tzinfo=self.tz)+timedelta(hours=8)]
...
class QuoteClass:
    ...
    def subquote(self,datatype:int,symbol:str,startime:int,endtime:int):#"-1" :超過上限 "-10" 無報價權限
        ...
        try:
            ...
            if (datatype==9 or datatype==10 or datatype==19) and ("TC.F" in symbol or "TC.S" in symbol):
