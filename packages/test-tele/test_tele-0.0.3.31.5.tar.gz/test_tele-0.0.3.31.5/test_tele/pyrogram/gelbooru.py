...
from test_tele.pyrogram.utils import BOT_CONFIG
...
async def get_images(query: str, pid: int = 0) -> List[dict]:
...
async def image_keyboard(image: dict, query: str) -> InlineKeyboardMarkup:
    buttons = [[
                InlineKeyboardButton("👤🔗", switch_inline_query_current_chat=image['source']),
    ...
    if 'pixiv' in image['source']:
        ...
        buttons[0][0] = InlineKeyboardButton("👤🔄", switch_inline_query_current_chat=f'.px {art_px_id}')
...
async def inline_gelbooru(client, inline_query):
    ...
    # cek if not_tag given
    if 'no_tag' in query:
        no_tag = True
        query = query.replace("no_tag", "")
    ...
    results = []
    images = await get_images(query, pid=pid)
    ...
    for image in images:
        try:
            caption = f"Tags : {image['tags']}" if not no_tag else ''
