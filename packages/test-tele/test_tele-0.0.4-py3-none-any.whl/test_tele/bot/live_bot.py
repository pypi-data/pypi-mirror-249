    ...
    try:
        for i in range(0, len(lists), img_slice):
            ...
            for chunk in lists[i:i + img_slice]:
                if as_file:
                    img = await event.message.client(
                        UploadMediaRequest(chat_id, InputMediaDocumentExternal(chunk['img_urls']['img_original'], ttl_seconds=86000))
                    )
                    images = InputSingleMedia(InputMediaDocument(InputDocument(img.photo.id, img.photo.access_hash, b'')), '')
                    media_to_send.append(images)
                else:
                    img = await event.message.client(
                        UploadMediaRequest(chat_id, InputMediaPhotoExternal(chunk['img_urls']['img_sample'].replace('540x540_70', '600x1200_90'), ttl_seconds=86000))
                    )
                    images = InputSingleMedia(InputMediaPhoto(InputPhoto(img.photo.id, img.photo.access_hash, b'')), '')
                    media_to_send.append(images)
            try:
                await event.message.client(SendMultiMediaRequest(chat_id, media_to_send))
            except errors.FloodWaitError as e:
                logging.warning(f"Got FloodWait error: {e.seconds}, {e}")
                time.sleep(3)
                await event.message.client(SendMultiMediaRequest(chat_id, media_to_send))
