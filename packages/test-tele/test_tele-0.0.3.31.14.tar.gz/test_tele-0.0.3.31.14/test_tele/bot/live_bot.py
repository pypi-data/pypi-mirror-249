...
async def get_images_from_inline(event):
    if event.message.text.startswith('.px'):
        from test_tele.pyrogram.pixiv import set_pixiv_mode, get_pixiv_list
        px_mode, keywords = await set_pixiv_mode('no_inline ' + event.message.text)

        img_slice = 10
        chat_id = event.message.input_chat
        ...
        try:
            for chunk in (lists[i:i + img_slice] for i in range(0, (img_slice - 1) * img_slice, img_slice)):
                sample_img = chunk['img_urls']['img_sample'].replace('540x540_70', '600x1200_90')
                if as_file:
                    images = await event.message.client(
                        [UploadMediaRequest(chat_id, InputMediaDocumentExternal(chunk['img_urls']['img_original'], ttl_seconds=86000))
                        for result in chunk]
                    )
                    images = [InputSingleMedia(InputMediaDocument(InputDocument(img.photo.id, img.photo.access_hash, b'')), '') for img in images]
                else:
                    images = await event.message.client(
                        [UploadMediaRequest(chat_id, InputMediaPhotoExternal(sample_img, ttl_seconds=86000))
                        for result in chunk]
                    )
                    images = [InputSingleMedia(InputMediaPhoto(InputPhoto(img.photo.id, img.photo.access_hash, b'')), '') for img in images]
                await event.message.client(SendMultiMediaRequest(chat_id, images))
        except Exception as err:
