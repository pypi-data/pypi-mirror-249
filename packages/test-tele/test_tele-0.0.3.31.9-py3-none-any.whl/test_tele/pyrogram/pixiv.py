...
async def download_media(session, elemen, i):
    nama_file =  elemen['id'] + "_" + str(i) +'.jpg'
    folder = f"temps/pixiv/{elemen['id']}"

    logging.warning(f"ini folder : {folder}")
    if not os.path.exists(folder):
        os.makedirs(folder)
    
    path_file = os.path.join(folder, nama_file)
    sample_img = elemen['img_urls']['img_sample'].replace('540x540_70', '600x1200_90')
    logging.warning(f"url file: {sample_img}")
    async with session.get(sample_img) as response:
        if response.status == 200:
            with open(path_file, 'wb') as f:
                f.write(await response.read())
            logging.warning("sudah didownload")
        else:
...
async def download_px_images(lists):
    async with aiohttp.ClientSession() as session:
        tasks = [download_media(session, element, i) for i, element in enumerate(lists)]
        await asyncio.gather(*tasks)

    logging.warning(f"ini folder 2 : temps/pixiv/{lists[-1]['id']}")
