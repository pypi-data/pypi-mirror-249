...
from test_tele.extractors.utils import clean_up_tags
...
async def get_pixiv_list(query: str, offset: int, mode: dict = {}):
    if query != '':
        ...
        for val in input:
            if val.isdigit():
                return await set_dict_detail(api.illust_detail(int(val)), mode)
            if 'id:' in val:
...
async def set_dict_detail(pixiv_json, mode: dict) -> dict:
