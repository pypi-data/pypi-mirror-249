...
class _VortexFactoryVortexInfo:
    def __init__(self):
        self.__vortexServersByName: Dict[str, List[VortexServer]] = defaultdict(list)
        self.__vortexClientsByName: Dict[str, List[VortexABC]] = defaultdict(list)
    ...
    def getRemoteVortexInfos(self, name=None, uuid=None) -> list[tuple[VortexABC, str]]:
        assert name or uuid, "We expect a name or uuid"

        # If we're passed a UUID, then there will only be one result
        if uuid:
            vortex, foundName = self.__remoteVortexNameByUuid.get(uuid, (None, None))
            if not foundName:
                return []
            return [(vortex, uuid)]
        ...
        return [
            (vortex, foundUuid)
            for vortex, foundUuid in self.__remoteVortexUuidsByName.get(name, [])
        ]
    ...
    def getRemoteVortexIpByUuid(self, vortexUuid: str, vortexName=None) -> str:
        ...
        for server in self.vortexSevers:
            for conn in server.connections.values():
                if (
                    ...
                    and (conn.remoteVortexName == vortexName or vortexName is None)
                ):
        ...
        if not targetConns:
            raise NoVortexException(f"Can not find vortex with UUID {vortexUuid}")
    ...
    def getLocalVortexClients(self, localVortexName: str) -> List[VortexABC]:
        return [
            vortex
            for vortex, _ in self.__localVortexUuidsByName.get(localVortexName, [])
        ]
...
class VortexFactory:
    ...
    @classmethod
    def createHttpWebsocketResource(cls, name: str) -> VortexWebSocketUpgradeResource:
        vortexServer = VortexServer(name, requiresBase64Encoding=False)
        cls.__vortexInfoState.addVortexServer(name, vortexServer)
    ...
    def createWebsocketClient(
        ...
        vortexWebsocketClientFactory = VortexClientWebsocketFactory(name, url=url)
        cls.__vortexInfoState.addVortexClient(name, vortexWebsocketClientFactory)
        ...
        return vortexWebsocketClientFactory.connect(host, port, sslContextFactory)
    ...
    def addCustomServerVortex(cls, vortex: VortexABC):
        cls.__vortexInfoState.addVortexServer(vortex.localVortexInfo.name, vortex)
        ...
        if [v for v in vortexAndUuids if v[0].requiresBase64Encoding]:
            for index, vortexMsg in enumerate(base64VortexMsgs):
                if vortexMsg.startswith(b"{"):
                    base64VortexMsgs[index] = yield PayloadEnvelope.base64EncodeDefer(
                        vortexMsg
                    )

        deferreds = []
        for vortex, uuid in vortexAndUuids:
            msgToUse = base64VortexMsgs if vortex.requiresBase64Encoding else vortexMsgs
