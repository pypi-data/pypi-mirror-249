...
class bump_command(Command):
    ...
    def finalize_options(self):
        if self.version_file is None:
            raise DistutilsOptionError("Must supply version-file.")
        ver_opts = [self.major, self.minor, self.patch].count(True)
        pre_opts = [self.alpha, self.beta, self.rc].count(True)
        if ver_opts > 1:
            raise DistutilsOptionError("Must supply only one version field (major, minor, patch) to bump.")
        if pre_opts > 1:
            raise DistutilsOptionError("Must supply only one pre-release field (alpha, beta, rc) to bump.")
    ...
    def _bump_version(parts, bump=None, zero=(), clear=()):
        if bump is not None:
            if not isinstance(bump, string_base):
    ...
    def run(self):
        ...
        if version_file_match is None:
            raise DistutilsOptionError('Version identifier {0!r} followed by string not found!'.format(self.version_ident))
        ...
        if version_string_match is None:
            raise DistutilsOptionError('Invalid version format {0!r}'.format(version_string))
...
with open('README.rst', 'w') as readme:
    readme.write(easy_xml.EasyXML.__doc__)
    re_nl = re.compile(r'(\r\n|\n\r|\n|\r)')
    readme.write(re_nl.search(easy_xml.EasyXML.__doc__).group(0))
