...
* Only basic entity processing (``&lt;``, ``&gt;``, and ``&amp;``)
  is performed."""
    ...
    def __init__(self, name, attrs={}, content=[]):
        ...
        """
        if name is not None and not isinstance(name, string_base):
            raise ValueError("name must be a string.")
        if len(list(filter(
                    lambda kv:not isinstance(kv[0], string_base)
                    or not isinstance(kv[1], string_base)
                    , attrs.items()
                ))) > 0:
            raise ValueError("attribute names and values must be strings.")
        if len(list(filter(
                    lambda v:not isinstance(v, (EasyXML, string_base))
                    , content
                ))) > 0:
    ...
    def append(self, value):
        ...
        """
        if not isinstance(value, (EasyXML, string_base)):
            raise ValueError("content values must be strings and EasyXML objects.")
        ...
        if isinstance(value, EasyXML):
            while i < j:
                if isinstance(self.__content[i], EasyXML) and self.__content[i].__name == value.__name:
                    found = True
                    search_items = list(self.__content[i].items())
    ...
    def insert(self, i, value):
        ...
        object.
        """
        if not isinstance(value, (string_base, EasyXML)):
            raise ValueError("content values must be strings and EasyXML objects.")
    ...
    def __setitem__(self, key, value):
        if isinstance(key, string_base):
            if not isinstance(value, string_base):
    ...
    def __getattr__(self, key):
        ...
        elif isinstance(key, string_base):
            try:
                return self.__attrs[key]
            except KeyError:
    ...
    def __setattr__(self, key, value):
        ...
        if key.startswith('_EasyXML__'):
            ...
            if isinstance(value, string_base):
                self.__name = key
            else:
    ...
    def __str__(self):
        ...
        if self.__name is not None:
            s += "<%s%s" % (self.__name,"".join(
                map(
                    lambda kv:
                        " %s=%s" % (
                            kv[0],
                            xml.sax.saxutils.quoteattr(kv[1])
                        )
                    , self.__attrs.items()
                )
            ))
    ...
    def pretty(self, indent=4):
        s = ""
        if not isinstance(indent, string_base):
            indent = " "*int(indent)
        if self.__name is not None:
            s += "<%s%s" % (self.__name,"".join(
                map(
            ...
            if len(self.__content) > 0:
                s += ">"
        if len(self.__content) > 0:
            if len(self.__content) == 1 and isinstance(self.__content[0], string_base):
    ...
    class __xmlTreeSaxHandler(xml.sax.handler.ContentHandler):
        ...
        def startElement(self, name, attrs):
            if len(self.__stack[-1]) > 0 and isinstance(self.__stack[-1][-1], string_base) and self.__stack[-1][-1].isspace():
                del self.__stack[-1][-1]
            elem = EasyXML(name, dict(list(attrs.items())))
        ...
        def characters(self, text):
            if len(self.__stack[-1]) > 0 and isinstance(self.__stack[-1][-1], string_base):
    ...
    def parseXML(stream, postprocess=None):
        ...
        if isinstance(stream, string_base):
