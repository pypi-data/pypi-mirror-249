...
def install_requirements(req_file: str, venv_path: Path, **kwargs) -> None:
    ...
    if "akerbp.mlpet" in req_contents:
        ...
        for requirement in requirements:
            if "akerbp.mlpet" in requirement:
                mlpet_version = requirement.split("==")[1]
            else:
                new_requirements.append(requirement)
        requirements = new_requirements
    install_special_scikit_learn = False
    if "scikit-learn" in req_contents:
        new_requirements = []
        for requirement in requirements:
            if "scikit-learn" in requirement:
                sklearn_version = requirement.split("==")[1]
    ...
    if install_mlpet:
        subprocess_wrapper(
            c + ["--ignore-requires-python", "akerbp.mlpet==" + mlpet_version]
        )
