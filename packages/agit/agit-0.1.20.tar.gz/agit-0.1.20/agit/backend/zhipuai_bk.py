...
def call_llm_api_v4(prompt: str, model: str, api_key=None,
                    system=None, history=[], tools=[],
                    logger=None, stream=True, **kwargs) -> Any:
    ...
    show_kwargs = copy.copy(kwargs)
    the_logger.debug(
        f"{model=}, {stream=}, history_len={len(history)}, {show_kwargs=}")
    ...
    resp_gen = resp2generator_v4(response)
    if not stream:
        return "".join(resp_gen)
    
    return resp_gen
    

def call_llm_api(*args, **kwargs):
    if "version" in vars(zhipuai):
