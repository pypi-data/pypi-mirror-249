...
class DialogCalendar(GenericCalendar):
    ...
    async def _get_days_kb(self, year: int, month: int, available_dates: list = None):
        ...
        def format_day_string():
            ...
            if (
                available_dates
                and date_to_check.strftime("%Y-%m-%d") not in available_dates
            ):
                # if not today
                if not (now_month == month and now_year == year and now_day == day):
                    return superscript(str(" "))
            elif self.min_date and date_to_check < self.min_date:
