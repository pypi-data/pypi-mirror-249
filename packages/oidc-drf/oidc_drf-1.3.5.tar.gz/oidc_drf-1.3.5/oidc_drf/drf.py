...
class OIDCAuthentication(authentication.BaseAuthentication):
    ...
    def get_signing_key(self, access_token):
        jwks_uri = import_from_settings("OIDC_OP_JWKS_ENDPOINT", None)        
        jwks_client = jwt.PyJWKClient(jwks_uri, cache_jwk_set=True, lifespan=360)
    ...
    def authenticate(self, request):
        ...
        try:
            ...
            try:
                ...
                algo = import_from_settings("OIDC_RP_SIGN_ALGO", 'RS256')
                
                decoded_token = jwt.decode(
                    access_token,
                    self.get_signing_key(access_token),
                    algorithms=import_from_settings("OIDC_SUPPORTED_ALGO",[algo]),
                    audience= import_from_settings("OIDC_TRUSTED_AUDIENCE",["account"]),
                    issuer=import_from_settings("OIDC_ISSUER", None),
                    options={
                ...
                print("Tken has expired")
                raise exceptions.AuthenticationFailed('Token has expired')
            except jwt.InvalidTokenError:
                print("Invalid token")
                raise exceptions.AuthenticationFailed('Invalid token')
            
            user = self.backend.get_user(decoded_token)
