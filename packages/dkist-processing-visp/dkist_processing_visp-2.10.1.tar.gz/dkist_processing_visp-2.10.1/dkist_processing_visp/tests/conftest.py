...
from dkist_processing_visp.tests.header_models import VispHeadersValidOutputFrames
...
@pytest.fixture()
def calibrated_visp_header(visp_dataset) -> fits.Header:
    """
...
def tag_on_map_raster_stokes(frame: VispHeadersValidOutputFrames) -> list[str]:
...
def write_full_sci_outputs_to_task(
    task, *, num_maps: int, num_steps: int, pol_type: str, data_shape: tuple[int, int]
):
    array_shape = (1, *data_shape)
    dataset = VispHeadersValidOutputFrames(
