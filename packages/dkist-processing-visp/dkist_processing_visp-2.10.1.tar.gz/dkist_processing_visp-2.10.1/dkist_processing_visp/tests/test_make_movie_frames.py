...
def test_make_movie_frames(movie_frames_task, pol_type, mocker):
    """
    ...
    write_full_sci_outputs_to_task(
        task=task, num_maps=num_maps, num_steps=num_steps, data_shape=data_shape, pol_type=pol_type
    )

    expected_movie_fram_shape = (spectral_size, num_steps)
    if pol_type == "Full Stokes":
