...
def build_extend_infos():
    ...
    extend_infos = dict()
    # 服务商微信公众号APPID对应密钥
    extend_infos["wx_woa_secret"] = "1234567890"
    # 公司类型
    extend_infos["ent_type"] = "2"
    # 登记证书
    extend_infos["certificate_info"] = getCertificateInfo()
    # 最终受益人信息
    extend_infos["ubo_info"] = getUboInfo()
    # 银行账户信息
    extend_infos["bank_account_info"] = getBankAccountInfo()
    # 补充说明
    extend_infos["business_addition_msg"] = "补充说明"
    # 文件列表
    extend_infos["file_list"] = getFileList()
    return extend_infos


class TestV2MerchantDirectWechatSignRequestDemo(unittest.TestCase):
    ...
    def test_request(self):
        ...
        # 接口请求对象
        request = dg_sdk.V2MerchantDirectWechatSignRequest()
        request.req_seq_id = ""
        request.req_date = ""
        request.huifu_id = "6666000003134509"
        request.upper_huifu_id = "6666000003134508"
        request.app_id = "20200506780602902"
        request.mch_id = "748851385"
        request.owner = "Y"
        request.contact_info = getContactInfo()
        request.sales_scenes_type = "SALES_SCENES_STORE,SALES_SCENES_MP,SALES_SCENES_MINI_PROGRAM,SALES_SCENES_WEB,SALES_SCENES_APP,SALES_SCENES_WEWORK"
        request.sales_info = getSalesInfo()
        request.settlement_info = getSettlementInfo()
