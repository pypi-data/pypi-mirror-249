...
from TeleGenic import TeleGenicError, Message, User, Chat, Update, Bot, MessageEntity
...
from TeleGenic.ext.dispatcher import block, Dispatcher, DispatcherHandlerStop
from TeleGenic.utils.deprecate import TeleGenicDeprecationWarning
from TeleGenic.utils.helpers import DEFAULT_FALSE
...
class TestDispatcher:
    ...
    def callback_raise_error(self, bot, update):
        if isinstance(bot, Bot):
            raise TeleGenicError(update.message.text)
        raise TeleGenicError(bot.message.text)
    ...
    def callback_context(self, update, context):
        if (
            ...
            and isinstance(context.error, TeleGenicError)
        ):
            self.received = context.error.message
    ...
    def test_error_handler(self, dp):
        ...
        error = TeleGenicError('Unauthorized.')
    ...
    def test_multiple_run_async_deprecation(self, dp):
        ...
        with pytest.warns(TeleGenicDeprecationWarning, match='@run_async decorator'):
            dp.process_update(self.message_update)
    ...
    def test_non_context_deprecation(self, dp):
        with pytest.warns(TeleGenicDeprecationWarning):
            Dispatcher(
