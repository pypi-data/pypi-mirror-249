...
def add_to_task_mapper(request_id, task_data, task_queue_key, pipe):
    """
    加入任务映射队列，用于重试任务源数据
    """
    _data = {
...
def _init_redis(redis_key, rows: List[StreamTaskItem]):
    ...
    for row in rows:
        ...
        # 加入重试源数据集合
        add_to_task_mapper(request_id, task, redis_key, pipe)
...
def _init_redis_batch(redis_key, rows: List[StreamTaskItem], batch_size):
    ...
    for row in rows:
        ...
        if len(batch_list) >= batch_size:
            ...
            # 加入重试源数据集合
            add_to_task_mapper(request_id, batch_row_data, redis_key, pipe)
            batch_list = []
    if batch_list:
        request_id = str(uuid.uuid4())
        batch_row_data = {
