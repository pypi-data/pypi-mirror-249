...
def plot_hist_cat(
    df: DataFrame, col: str, title: str = None, hue: str = None, max_bins: int = 0, figsize: Tuple[int, int] = (12, 8)
) -> None:
    group = [col]
    if hue is not None:
        group.append(hue)
    ...
    ax1.set_xticklabels([t.get_text() for t in ax1.get_xticklabels()], fontsize=8)
...
def plot_hist_num(df: DataFrame, col: str, bins: int = 12, title: str = None, figsize: Tuple[int, int] = (12, 8)):
    ...
    ax1.set_xticklabels(["{:.2f}".format(float(t.get_text())) for t in ax1.get_xticklabels()], fontsize=8)
    ax1.xaxis.set_tick_params(rotation=45)
    if title is not None:
        plt.title(title)
    plt.show()
