...
class UtilsReflection:
    ...
    @staticmethod
    def load_class_from_module(module: str, clazz: str, super_clazz: type) -> Any:
        logger.info(f"[load_class_from_module|in] (module={module}, clazz={clazz}, super_clazz={super_clazz})")
        ...
        logger.info(f"[load_class_from_module|out] => {result}")
        return result
    ...
    @staticmethod
    def find_class_implementations(comma_separated_packages_list: str, clazz: Any) -> Dict[str, Any]:
        """
        throws UtilsReflectionException
        """
        logger.info(f"[find_class_implementations|in] ({comma_separated_packages_list}, {clazz})")
        result = {}
        _packages = []
        try:
            _packages = [a.strip() for a in comma_separated_packages_list.split(",")]
        except LookupError as x:
            raise UtilsReflectionException("no config defining where to find implementations", x)
