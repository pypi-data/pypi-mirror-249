...
def converter_factory(
    obj_converter_factory,
) -> typing.Callable[[typing.Type, ctor.JsonSerializationContext], ctor.IConverter]:
...
def test_load_empty_object(converter_factory, context):
    converter = converter_factory(EmptyClass, context)
    assert isinstance(
        converter.load({}, key=ctor.NOT_PROVIDED, context=context), EmptyClass
    )


def test_dump_empty_object(converter_factory, context):
    converter = converter_factory(EmptyClass, context)
    assert converter.dump(EmptyClass(), context=context) == {}
...
def test_discriminated_union_class_load(
    data, expected, discriminated_converter_factory, context
):
    context.converter_factories.insert(0, discriminated_converter_factory)
    obj = ctor.load(ClassWithDiscriminatedUnionAttr, data=data, context=context)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            {"attr": {"attr": 1}},
            NestedClassForwardRef(ClassWithIntAttr(1)),
        ),
        (RecursiveClass, {"attr": None}, RecursiveClass(None)),
        (
            RecursiveClass,
            {"attr": {"attr": None}},
            RecursiveClass(RecursiveClass(None)),
        ),
        (
            RecursiveClass,
            {"attr": {"attr": {"attr": None}}},
            RecursiveClass(RecursiveClass(RecursiveClass(None))),
        ),
    ],
)
def test_load_class(cls, data, expected, converter_factory, context):
    converter = converter_factory(cls, context)
    obj = converter.load(data, key=ctor.NOT_PROVIDED, context=context)
    assert obj == expected
...
def test_load_invalid_class_raises(cls, data, converter_factory, context):
    converter = converter_factory(cls, context)
    with pytest.raises(ctor.LoadError):
        converter.load(data, key=ctor.NOT_PROVIDED, context=context)
...
def test_dump_class(cls, data, expected, converter_factory, context):
    converter = converter_factory(cls, context)
    obj = converter.dump(data, context=context)
    assert obj == expected
...
def test_load_obj_with_optional_attr_factory(context):
    obj = ctor.load(
        AttrsWithOptionalAttrFactory, {}, key=ctor.NOT_PROVIDED, context=context
    )
...
def test_load_obj_with_attr_factory(context):
    obj = ctor.load(AttrsWithAttrFactory, {}, key=ctor.NOT_PROVIDED, context=context)
...
def test_load_obj_with_optional_any_default(context):
    obj = ctor.load(ClassWithOptionalAny, {}, key=ctor.NOT_PROVIDED, context=context)
    assert obj == ClassWithOptionalAny()


def test_load_obj_with_optional_any(context):
    obj = ctor.load(
        ClassWithOptionalAny,
        {"attr": "not default"},
        key=ctor.NOT_PROVIDED,
        context=context,
    )
...
class DummyClass:
    ...
    dict_of_child: typing.Dict[str, Child] = {"a": Child(1), "b": Child(2)}
...
def test_any_dump(context):
    obj = AnyDummy(foo=[1, "hello", {}])
    data = ctor.dump(obj, context=context)
    assert data == {"foo": [1, "hello", {}]}


def test_any_load(context):
    data = {"foo": [1, "hello", {}]}
