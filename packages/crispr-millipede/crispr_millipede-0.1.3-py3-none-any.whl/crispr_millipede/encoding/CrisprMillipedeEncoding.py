...
def get_substitution_encoding(aligned_sequence, original_seq, skip_index=0):
    ...
    mismatch_mappings_per_position_REF_list = np.asarray(list(original_seq)).repeat(mismatch_mappings_per_position_df.shape[0]).astype(np.object)
...
class EncodingDataFrames:
    ...
    @classmethod
    def encode_crispresso_allele_table(self, cores=1):
    ...
    def postprocess_encoding(self):
        ...
        # Merge the encodings between the different populations. The WT is kept separate since it isn't used for modelling.
