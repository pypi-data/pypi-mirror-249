...
class GenericCRUD:
    ...
    def insert(self, table_name: str = None, data_json: dict = None,
        ...
        if table_name in table_definition:
            if table_definition[table_name]["is_number_column"]:
                # TODO change get_random_number( schema_name, table_name)
                number = NumberGenerator.get_random_number(schema=self.schema_name, table=table_name)
                data_json["number"] = number
        else:
            logger.warn("Table not found in table_definition_table")
    ...
    # Old name: update
    # TODO Let's use DEFAULT_SQL_SELECT_LIMIT instead of 100 everywhere
    ...
    # TODO: delete this when not in use anymore
