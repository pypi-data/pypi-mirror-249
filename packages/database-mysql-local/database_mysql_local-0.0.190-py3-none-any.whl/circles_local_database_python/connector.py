...
class Connector:
    ...
    def _connect_to_db(self):
        ...
        try:
            ...
            self._cursor = self.connection.cursor()
            try:
                # We must have `` around {self.schema} to support schema name using reserved words such as databae, group ...
                self._cursor.execute(f"USE `{self.schema}`;")
                logger.info(f"set schema to database: {self.schema}")
            except mysql.connector.Error as err:
                logger.exception(object=err)
