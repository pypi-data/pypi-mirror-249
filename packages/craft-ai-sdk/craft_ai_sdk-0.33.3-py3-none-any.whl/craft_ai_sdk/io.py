...
from craft_ai_sdk.experimental_warnings import (
...
class OutputDestination:
    ...
    Warning:
        Using dynamic fields when mapping an output to the datastore, is experimental
        and may be unstable.
    ...
    def __init__(
        ...
        if datastore_path is not None and (
            "{execution_id}" in datastore_path
            or "{date}" in datastore_path
            or "{date_time}" in datastore_path
        ):
            warnings.warn(
                DYNAMIC_DATASTORE_WARNING_MESSAGE,
                FutureWarning,
