...
from unctl.config.app_config import Mask
...
class ResolvingScreen(Screen):
    ...
    def __init__(self, item: CheckReport, masks: list[Mask], checker: ResourceChecker):
        super().__init__()
        self._item = item
        self._masks = masks
    ...
    async def execute_commands(self, commands: list[str]):
        ...
        await self.app.push_screen(
            PreviewScreen(outputs, self._masks), self.add_outputs_to_chat_body
        )
