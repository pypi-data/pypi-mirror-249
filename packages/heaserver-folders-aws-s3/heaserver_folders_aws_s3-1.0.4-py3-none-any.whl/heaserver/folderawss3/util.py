        ...
        with ZipFile(fileobj, mode='w', compression=ZIP_DEFLATED) as zf:
            async for obj in awsservicelib.list_objects(s3_client, bucket_name, folder_key, include_restore_status=True):
                try:
                    ...
                    if not folder:
                        continue
                    if obj['StorageClass'] in (S3StorageClass.DEEP_ARCHIVE.name, S3StorageClass.GLACIER.name) and ((restore:= obj.get('RestoreStatus')) is None or restore.get('RestoreExpiryDate') is None):
                        if logger.getEffectiveLevel() == logging.WARN:
                            logger.warn('Not including %s in zip file because it is in incompatible storage class %s', awsservicelib._activity_object_display_name(bucket_name, obj['Key']), obj['StorageClass'])
                    else:
                    ...
                    logger.warn('Error downloading %s in bucket %s: %s', obj['Key'], bucket_name, e)
    return response_
