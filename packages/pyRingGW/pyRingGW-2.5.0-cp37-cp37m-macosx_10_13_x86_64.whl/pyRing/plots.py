...
def Mf_af_plot(samples, Mf_LAL_samples, af_LAL_samples, **input_par):
    ...
    pairplot = sns.pairplot(combined_data, hue='Distribution', markers='.', kind='scatter', diag_kws={'alpha': 0.0}, palette=palette_dict)
    ...
    for i, ax in enumerate(pairplot.axes[-1, :]): ax.set_xlabel(r'$\mathrm{M_f \, (M_{\odot})}$', fontsize=15)
    for i, ax in enumerate(pairplot.axes[ :, 0]): ax.set_ylabel(r'$\mathrm{a_f}$'               , fontsize=15)
