...
class ExLlama_TTT:
    ...
    def _generate_simple(self, prompt, max_new_tokens = 128):
        ...
        ids, mask = self.client.tokenizer.encode(prompt, return_mask = True, max_seq_len = self.gai_config["max_seq_len"])
        self.client.gen_begin(ids, mask = mask)

        max_new_tokens = min(max_new_tokens, self.gai_config["max_seq_len"] - ids.shape[1])
