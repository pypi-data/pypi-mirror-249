...
class IVersionedService(ABC):
    # The version of the protocol this service is built for
    protocol_version: str

    # Whether the service supports the given protocol version
    def supports_protocol(self, protocol_version: float) -> bool:
        return self.protocol_version.startswith(str(protocol_version))


T = TypeVar('T', bound=IVersionedService)
...
"""
class VersionsService(Generic[T]):
    def __init__(self, versions: list[T]):
        """
    ...
    def get_service_versions(self, protocol_version: float) -> list[T]:
        """
    ...
    def get(self, version: str) -> Optional[T]:
        """
