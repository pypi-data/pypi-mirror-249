...
def flux(salt3dir,obsphase,obswave,z,x0,x1,c,mwebv,fill_value="extrapolate",include_colorlaw=False):
    ...
    if include_colorlaw:
        m0interp = interp1d(np.unique(m0phase)*(1+z),m0flux*1e-12/(1+z)*10**(-0.4*c*salt3colorlaw(np.unique(m0wave))),axis=0,
        ...
        m1interp = interp1d(np.unique(m0phase)*(1+z),m1flux*1e-12/(1+z)*10**(-0.4*c*salt3colorlaw(np.unique(m0wave))),axis=0,
                            kind='nearest',bounds_error=False,fill_value=fill_value)
...
#           continue
        ...
        for k in specdata.keys():
            ...
            uncalledModel = flux(salt3dir,specdata[k].tobs,specdata[k].wavelength,
                                  snPars['z'],snPars['x0'],snPars['x1'],snPars['c0'],
                                 mwebv=datadict[sn].MWEBV,include_colorlaw=not specdata[k].n_specrecal)
            
            modelflux = uncalledModel*recalexp if specdata[k].n_specrecal is None or specdata[k].n_specrecal > 0 else uncalledModel
