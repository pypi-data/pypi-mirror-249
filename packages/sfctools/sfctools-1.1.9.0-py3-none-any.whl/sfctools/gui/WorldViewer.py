...
class WorldViewer:
    ...
    def __init__(self, cluster_params = None, market_params = None):
        """
        ...
        :param cluster_params: TODO insert documentation 
        :param market_params: TODO insert documentation
        """

        self.window = WorldViewerWindow(cluster_params, market_params)
...
class WorldViewerWindow(QtWidgets.QDialog):
    ...
    def __init__(self, cluster_params, market_params):
        ...
        self.setGeometry(cx-400, cy-300, 800, 600)
    ...
    def float_positions(self):
        ...
        for k,v in self.flucts.items():
            dx = int(np.clip(v[0] + np.random.uniform(-1,2), -20, 20))
            dy = int(np.clip(v[1] + np.random.uniform(-1,2), -20, 20))
    ...
    def paint(self):
        ...
        for edge in G.edges.data():
            ...
            a = int(scale*(nodePos[a1][0] ) + offsetx + self.sizes[a1]*0.5 +  self.flucts[a1][0])
            b = int(scale*(nodePos[a1][1] ) + offsety + self.sizes[a1]*0.5 +  self.flucts[a1][1])

            u = int(scale*(nodePos[a2][0]) + offsetx + self.sizes[a2]*0.5 + self.flucts[a2][0])
            v = int(scale*(nodePos[a2][1]) + offsety + self.sizes[a2]*0.5 + self.flucts[a2][1])
