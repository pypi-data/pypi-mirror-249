...
class RReceiver(object):
    ...
    def _handler_file(
        ...
        # Save.
        rfolder_receive = RFolder(self.rwechat.dir_file_receive)
        generate_path = None
        ...
        if message.type == 3:
            ...
            ### Exist.
            pattern = f"^{file_md5}"
            search_path = rfolder_receive.search(pattern)
        ...
        if generate_path is not None:
            require_time = file_size / 125_000 / self.bandwidth_downstream
            timeout = 10 + require_time * (self.max_receiver + 1)
            wrap_wait(
                os_exists,
                generate_path,
                _interval = 0.05,
