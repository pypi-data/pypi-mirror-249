...
class Trigger:
    ...
    def fire_when_encoder_dist(
        ...
        Args:
            ...
            dist: The measured distance between trigger fires.
            unit: Units of dist.
        """
        if dist <= 0:
            raise ValueError('Invalid value; dist must be a positive value.')

        request = main_pb2.TriggerFireWhenDistRequest()
        request.interface_id = self.device.connection.interface_id
        request.device = self.device.device_address
        request.trigger_number = self.trigger_number
        request.axis = axis
        ...
        call("trigger/fire_when_encoder_dist", request)

    async def fire_when_encoder_dist_async(
            self,
            axis: int,
            dist: float,
            unit: LengthUnits = Units.NATIVE
        ...
        request.dist = dist
        request.unit = units_from_literals(unit).value
        await call_async("trigger/fire_when_encoder_dist", request)
    ...
    def fire_when_dist(
        ...
        request.dist = dist
        request.unit = units_from_literals(unit).value
        call("trigger/fire_when_dist", request)

    async def fire_when_dist_async(
            self,
            axis: int,
            dist: float,
            unit: LengthUnits = Units.NATIVE
        ...
        request.dist = dist
        request.unit = units_from_literals(unit).value
        await call_async("trigger/fire_when_dist", request)
