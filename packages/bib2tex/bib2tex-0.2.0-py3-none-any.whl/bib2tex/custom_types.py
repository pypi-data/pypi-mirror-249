...
from bib2tex.config import COL_ENTRYTYPE, CT_KEY_BIBTEXTYPE, CT_KEY_FORMATSCHEMES, CT_KEY_SEARCHSTRING
from bib2tex.bibtex_filter import filter_by_key
from bib2tex.format_schemes import FormatSchemeManager

CUSTOM_TYPE_KEYS_REQUIRED = [CT_KEY_BIBTEXTYPE, CT_KEY_FORMATSCHEMES, CT_KEY_SEARCHSTRING]
...
def register_custom_types(file_path: str, fsm: FormatSchemeManager) -> None:
    ...
    for entry_type, tdefs in definitions.items():
        ...
        if missing_keys:
            ...
            raise KeyError(f"Required definitions {missing_keys_str} not provided for custom entry type {entry_type!r}.")

        # register format scheme
        for fsname, format_scheme in definitions[entry_type][CT_KEY_FORMATSCHEMES].items():
            fsm.add_format_scheme(fsname, entry_type, format_scheme)
...
def get_entries_for_custom_type(entries: list[dict[str, Any]], entry_type: str) -> list[dict[str, Any]]:
    ...
    ct_entries = []
    for s_field, s_value in ctype_defs[CT_KEY_SEARCHSTRING].items():
