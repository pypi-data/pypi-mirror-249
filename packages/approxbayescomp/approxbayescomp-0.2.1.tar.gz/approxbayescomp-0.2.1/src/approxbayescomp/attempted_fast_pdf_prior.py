...
class IndependentPrior(object):
    def __init__(self, marginals, names=None, types=None):
        ...
        for i, marginal in enumerate(self.marginals):
            if self.types[i] == "continuous":
                dist_args, loc, scale = marginal.dist._parse_args(*marginal.args, **marginal.kwds)
                self.marginals[i]._dist_args = dist_args
                self.marginals[i]._loc = loc
                self.marginals[i]._scale = scale
    ...
    def pdf(self, theta):
        ...
        for i, theta_i in enumerate(theta.reshape(-1)):
            if self.types[i] == "discrete":
                ...
                x = (theta_i - self.marginals[i]._loc) / self.marginals[i]._scale
                density *= self.marginals[i].dist._pdf(x, *self.marginals[i]._dist_args) / self.marginals[i]._scale
        return density
