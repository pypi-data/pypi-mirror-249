...
class MFLES:
        ...
        if seasonal_period is not None:
            ...
            for period in seasonal_period:
                ...
                fourier_series.append(get_fourier_series(n,
                                                    period,
                                                    fourier))
            k += int(np.shape(fourier_series[0])[1] / 2)
        else:
            self.seasonality = None
    ...
    def conformal(self, y, forecast_horizon, n_windows, coverage, **kwargs):
        ...
        if isinstance(coverage, list):
            ...
            for i in range(len(coverage)):
                upper.append(np.append(fitted + intervals[0][i], predicted + intervals[1][i]))
                lower.append(np.append(fitted - intervals[0][i], predicted - intervals[1][i]))
        else:
            upper = np.append(fitted + intervals[0], predicted + intervals[1])
            lower = np.append(fitted - intervals[0], predicted - intervals[1])
        self.upper = upper
        self.lower = lower
    ...
    def seasonal_decompose(self, y, seasonal_period, **kwargs):
        ...
        if self.multiplicative:
            ...
            if seasonal_period is not None:
                seasonality = self.mean + (seasonality * self.std)
