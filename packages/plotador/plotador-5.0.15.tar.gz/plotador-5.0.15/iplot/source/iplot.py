...
from iplot.source.go import DadoEixo 
...
class iPlot():
    ...
    def interpreta(self, formula):
        df = pd.DataFrame()
        for caso in self.gerenciador.mapaCasos:
            dadoEixo = interpretador().interpreta(self.gerenciador.getClasse(caso), formula)
            data = dadoEixo.dado
    ...
    def figure(self, figure, caminhoDeSaida = ""):
        figura = Figura(figure, caminhoDeSaida)
