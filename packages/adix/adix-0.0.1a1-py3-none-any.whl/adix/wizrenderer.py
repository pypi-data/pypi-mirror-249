                ...
                <section id="{{ context.type[idx] }}-{{ context.unique_id }}" class="tab-panel" {% if not loop.first %}style="display: none;"{% endif %}>
                    ...
                    {% elif context.image[idx] is not none %}
                         {% for image_data in context.image[idx] %}
                            <img src="data:image/png;base64, {{ image_data }}" alt="{{ context.title[idx] }}" style="width: 500px; margin-right: 0px;">
                        {% endfor %}
                    {% endif %}
                </section>
            {% endfor %}
...
class DataManager:
    ...
    # Create a Context object and pass the 'plots' and 'unique_id' variables
