...
def _uniffi_check_api_checksums(lib):
    ...
    if lib.uniffi_nostr_sdk_ffi_checksum_method_client_shutdown() != 18928:
        raise InternalError("UniFFI API checksum mismatch: try cleaning and rebuilding your project")
    if lib.uniffi_nostr_sdk_ffi_checksum_method_client_signer() != 8016:
        raise InternalError("UniFFI API checksum mismatch: try cleaning and rebuilding your project")
    ...
    if lib.uniffi_nostr_sdk_ffi_checksum_method_clientbuilder_opts() != 13520:
        raise InternalError("UniFFI API checksum mismatch: try cleaning and rebuilding your project")
    if lib.uniffi_nostr_sdk_ffi_checksum_method_clientbuilder_signer() != 47855:
        raise InternalError("UniFFI API checksum mismatch: try cleaning and rebuilding your project")
    ...
    if lib.uniffi_nostr_sdk_ffi_checksum_method_relayconnectionstats_uptime() != 18216:
        raise InternalError("UniFFI API checksum mismatch: try cleaning and rebuilding your project")
    if lib.uniffi_nostr_sdk_ffi_checksum_constructor_client_new() != 37668:
        raise InternalError("UniFFI API checksum mismatch: try cleaning and rebuilding your project")
    if lib.uniffi_nostr_sdk_ffi_checksum_constructor_client_with_opts() != 53855:
        raise InternalError("UniFFI API checksum mismatch: try cleaning and rebuilding your project")
...
_UniffiLib.uniffi_nostr_sdk_ffi_fn_method_client_signer.restype = _UniffiRustBuffer
...
class ClientSigner:
    def __init__(self):
        raise RuntimeError("ClientSigner cannot be instantiated directly")
    ...
    class KEYS:
        ...
        def __init__(self,signer: "Keys"):
            
            self.signer = signer
            

        def __str__(self):
            return "ClientSigner.KEYS(signer={})".format(self.signer)
    ...
    class NIP46:
        ...
        @typing.no_type_check
        def __init__(self,signer: "Nip46Signer"):
            
            self.signer = signer
            

        def __str__(self):
            return "ClientSigner.NIP46(signer={})".format(self.signer)

        def __eq__(self, other):
            if not other.is_nip46():
                return False
            if self.signer != other.signer:
                return False
            return True
    ...
    def is_keys(self) -> bool:
        return isinstance(self, ClientSigner.KEYS)
    def is_nip46(self) -> bool:
        return isinstance(self, ClientSigner.NIP46)
    

# Now, a little trick - we make each nested variant class be a subclass of the main
# enum class, so that method calls and instance checks etc will work intuitively.
# We might be able to do this a little more neatly with a metaclass, but this'll do.
ClientSigner.KEYS = type("ClientSigner.KEYS", (ClientSigner.KEYS, ClientSigner,), {})  # type: ignore
ClientSigner.NIP46 = type("ClientSigner.NIP46", (ClientSigner.NIP46, ClientSigner,), {})  # type: ignore




class _UniffiConverterTypeClientSigner(_UniffiConverterRustBuffer):
    @staticmethod
    def read(buf):
        variant = buf.read_i32()
        if variant == 1:
            return ClientSigner.KEYS(
                _UniffiConverterTypeKeys.read(buf),
            )
        if variant == 2:
            return ClientSigner.NIP46(
                _UniffiConverterTypeNip46Signer.read(buf),
            )
    ...
    def check_lower(value):
        if value.is_keys():
            _UniffiConverterTypeKeys.check_lower(value.signer)
            return
        if value.is_nip46():
            _UniffiConverterTypeNip46Signer.check_lower(value.signer)
            return
    ...
    def write(value, buf):
        if value.is_keys():
            buf.write_i32(1)
            _UniffiConverterTypeKeys.write(value.signer, buf)
        if value.is_nip46():
            buf.write_i32(2)
            _UniffiConverterTypeNip46Signer.write(value.signer, buf)
