            ...
            try:
                if ssh接口.有效:
                    ...
                    ssh管道.connect(hostname=ssh接口.主机地址,
                                    ...
                                    password=ssh接口.密码,
                                    port=ssh接口.端口号)
                    stdin, stdout, stderr = ssh管道.exec_command(指令, timeout=1000)
                ...
                try:
                    if self.ssh接口.无效 and self.在linux系统中:
                        ...
                        ssh管道.connect(hostname=self.ssh接口.主机地址,
                                        ...
                                        password=self.ssh接口.密码,
                                        port=self.ssh接口.端口号)
                        stdin, stdout, stderr = ssh管道.exec_command(locate指令, timeout=1000)
                        locate结果 = stdout.read().decode(encoding='utf-8').strip()
                        locate错误 = stderr.read().decode(encoding='utf-8').strip()
                except Exception as exp:
                    搜索结果.状态码 = 1
                    搜索结果.错误消息 = str(exp)
                    self.服务在线 = False
