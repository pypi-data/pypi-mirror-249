...
from itry import number, nparray
...
@Alias(int, list, tuple, itertype, nparray, attrs=dict(dtype=int))
...
@Alias(float, list, tuple, itertype, nparray, attrs=dict(dtype=float))
class floats: '''iterable of ints'''
...
@Alias(real, list, tuple, itertype, nparray, attrs=dict(dtype=real))
...
@Alias(num, list, tuple, itertype, nparray, attrs=dict(dtype=num))
class nums: '''list of `num`'''

# %% ../nbs/03_nums.ipynb 24
@Alias(itertype, attrs=dict(dtype=num))
class iternum: '''iterable of `num``'''

# %% ../nbs/03_nums.ipynb 26
@Alias(num, str)
