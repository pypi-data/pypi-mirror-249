...
def delta_hedged_strangle_lite(
    ...
    while current_time().time() < time(*end_time):
        ...
        if option_to_move:
            ...
            overlap = check_for_overlap(new_option, target_strangle)
            if overlap:
                notifier(
        ...
        logger.debug(
            f"No change in strangle {target_strangle}. "
            f"Current call avg: {current_call_avg}, current put avg: {current_put_avg}."
        )
        sleep(1)
