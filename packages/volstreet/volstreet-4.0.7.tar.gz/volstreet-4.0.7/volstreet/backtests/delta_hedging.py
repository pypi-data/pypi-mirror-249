...
class DeltaBackTest(IntradayBackTest):
    ...
    def calculate_strikes_and_ratios(
        ...
        if best_call is not None:
            trade_dict["calls"] = [*zip([best_call["strike"], 0.5] * 2)]

        if best_put is not None:
            trade_dict["puts"] = [*zip([best_put["strike"], 0.5] * 2)]
    ...
    def process_segment(
        ...
        for i, row in prepared_segment.iterrows():
            ...
            if abs(total_delta) > delta_threshold:
                ...
                neutralized_delta = self.calculate_hedge_delta(i, row, hedges)
                ...
                if total_hedge_qty_call < -max_hedge_qty or (
                    ...
                    hedge_position_history.append(
                        {k: v for k, v in hedges.items() if v}
                    )
            ...
            hedge_delta_history.append(hedge_delta)
            net_delta_history.append(total_delta)
            neutralized_delta_history.append(np.nan)
            hedge_position_history.append({k: v for k, v in hedges.items() if v})
            mtm_history.append(np.nan)
...
def update_hedges(
    ...
    qty_to_sell = int((abs(net_delta) - 0) / abs(atm_info[f"{adjustment_leg}_delta"]))
...
if __name__ == "__main__":
    nif = UnderlyingInfo("NIFTY")
    dbt = DeltaBackTest(nif)
    day = dbt.fetch_and_prepare_index_prices("2023-12-27", "2023-12-27 15:30:00", False)
    # %%
    result = dbt.run_day(day)
