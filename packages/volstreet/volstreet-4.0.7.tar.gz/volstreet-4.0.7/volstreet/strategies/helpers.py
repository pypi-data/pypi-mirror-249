...
def check_for_move(
    ...
    call_move = abs(call_avg - call_ltp) / call_avg
    put_move = abs(put_avg - put_ltp) / put_avg
    if call_move > sl:
...
def check_for_overlap(new_option: Option, strangle: Strangle) -> bool:
    """Equal delta helper function"""
    counter_part = (
        strangle.put_option
    ...
    return (
        new_option.strike < counter_part.strike
        if new_option.option_type == OptionType.CALL
