...
class ConsumerProducer:
    ...
    def __init__(self, *,
        ...
        if nbytes > (3 * 2**30):
            raise ValueError(f"ConsumerProducer '{name}': {nbuffers} buffers need over 3 GiB ({nbytes/(2**23)} GiB)")
    ...
    def __repr__(self):
        ...
        for attr in self.__dir__():
            ...
            val = f"ndarray(shpae={val.shape}, dtype={val.dtype})" if isinstance(val, np.ndarray) else val
            R.append(f"{attr}={val}")
        return r + ", ".join(R) + ")"
...
def compile_buffer_logic():
    ...
    @njit(nogil=True, locals=dict(
        i=int64, state=uint64, waited=int64, delay=int64))
