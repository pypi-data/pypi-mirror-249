"""
Module xengsort.subtable_hashfunctions
...
def get_random_multipliers(n, *, maxfactor=(2**23 - 1), mod_value=None):
    while True:
        M = [randrange(3, maxfactor, 2) for _ in range(n)]
...
def populate_hashfunc_tuple(hashfunc_tuple, *, functype="linear",
        maxfactor=2**32 - 1, mod_value=None):
    """
    ...
    M = get_random_multipliers(todo, maxfactor=maxfactor, mod_value=mod_value)
    L = []
    m = 0
