...
DEFAULT_METHODS: Final[Dict[XAI_MethodType, Dict]] = {
    ...
    XAI_MethodType.EDGEMAP: {
        "preprocessor": lambda x: np.clip(x, -1, 1),
        "filter_fn": lambda _: True,
...
def compute_relevances(
    ...
    scores = normalize_scores(scores, unique_segments) if normalize else scores

    scores = {int(k): float(v) for k, v in scores.items()}  # fix types to match spec
