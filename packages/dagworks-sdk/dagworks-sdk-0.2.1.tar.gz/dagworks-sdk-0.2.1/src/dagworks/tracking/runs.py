...
def monkey_patch_adapter(
    ...
    def execute_without_adapter(node, kwargs):
        return node(**kwargs)

    execute_fn = execute_without_adapter

    if adapter.does_method("do_node_execute", is_async=False):

        def execute_with_adapter(node, kwargs):
            return adapter.call_lifecycle_method_sync(
                "do_node_execute", node, kwargs, tracking_state.run_id
            )

        execute_fn = execute_with_adapter

    try:
        run_tracker = RunTracker(tracking_state=tracking_state)
        # monkey patch the adapter
        adapter.execute_node = functools.partial(run_tracker.execute_node, execute_fn)
        yield
    finally:
        adapter.execute_node = execute_fn
