            ...
            async for obj in list_objects(s3_client, source_bucket_name, prefix=source_key):
                if obj['StorageClass'] in (S3StorageClass.DEEP_ARCHIVE.name, S3StorageClass.GLACIER.name):
                    if is_folder(source_key):
                        raise ValueError(f'{_activity_object_display_name(source_bucket_name, source_key)} contains archived objects')
                    else:
                        raise ValueError(f'{_activity_object_display_name(source_bucket_name, source_key)} is archived')
