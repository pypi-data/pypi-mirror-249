...
class CardHandlers:
    ...
    def parse(self):  # type: () -> None
        ...
        if (
            httpCard.type == CALLBACK_TYPE_CHALLENGE
    ...
    def complement(self, exception):  # type: (Union[None, Exception]) -> None
        ...
        if exception is not None:
            ...
            conf.logger.error(
                "request_id={} err_msg={}".format(self.ctx.get_request_id(), err_msg)
            )
            return
        ...
        if self.respBody is not None:
            ...
            self.write_response(
                HTTP_STATUS_OK, DEFAULT_CONTENT_TYPE, RESPONSE_FORMAT % "success"
            )
