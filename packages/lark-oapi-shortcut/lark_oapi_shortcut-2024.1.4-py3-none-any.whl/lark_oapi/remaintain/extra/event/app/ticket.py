...
def app_ticket_event_callback(ctx, conf, event):
    ...
    try:
        ...
        conf.logger.warn(
            "set %s=%s, expire=%d into storage occurred an error:%s"
            % (key, val, expire, e.__str__())
        )
...
def set_app_ticket_event_handler(conf):
    ...
    if conf.app_settings.app_type == APP_TYPE_INTERNAL:
        return
    Manager.set_event_callback(
        conf, "app_ticket", app_ticket_event_callback, clazz=AppTicketEvent
    )
