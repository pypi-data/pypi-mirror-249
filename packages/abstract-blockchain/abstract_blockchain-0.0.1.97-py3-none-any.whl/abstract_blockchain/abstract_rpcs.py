...
class RPCBridge: 
    ...
    def __init__(self, rpc_js: dict = None, testnet=False, rpc_list=None, rpc_gui=False):
        ...
        self.rpc_js=self.update_rpc_js(new_rpc_js=rpc_js or get_default_rpc())
        
    def update_rpc_js(self, new_rpc_js: dict):
        """
        Directly updates the RPC JavaScript object with new settings.
        """
 
        if isinstance(new_rpc_js, dict):
            self.rpc_js = new_rpc_js
...
class RPCGUIManager:
    ...
    def get_static_variables(self,rpc_js):
        ...
        if rpc_js:
            ...
            rpcs = self.rpc_mgr.get_rpc_url(rpc_js)
