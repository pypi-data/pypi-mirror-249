...
class APIBridge:
    ...
    def update_address(self,address=None,contract_address=None,derive=True):
        if address == False:
            self.address=None
        else:
        ...
        if contract_address == False:
            ...
            if self.contract_address:
                ...
                if derive:
                    self.derive_network(contract_address=self.contract_address,api_data_type='sourcecode',multiple=False,initial_network=self.rpc_mgr.rpc_js)
    def update_api_call_partameters(self,rpc_mgr=None,rpc_js=None,address=None,contract_address=None,start_block=None,end_block=None,api_data_type=None,api_gui=None,derive=False):
        if rpc_mgr or rpc_js:
            self.update_rpc(rpc_mgr=rpc_mgr,rpc_js=rpc_js)
            self.get_api_key()
        self.update_address(address=address,contract_address=contract_address,derive=derive)
        ...
        for list_itteration,list_obj in enumerate([make_list(initial_network),common_chains,rpc_list]):
            for network in list_obj:
                ...
                if network:
                    ...
                    self.update_api_call_partameters(rpc_js=rpc_js,address=False,contract_address=self.contract_address,api_data_type=self.api_data_type,derive=False)
