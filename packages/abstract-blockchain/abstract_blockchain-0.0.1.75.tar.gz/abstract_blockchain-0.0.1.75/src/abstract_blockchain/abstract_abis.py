...
from abstract_blockchain.abstract_rpcs import RPCBridge, RPCGUIManager
...
class ABIBridge:
    ...
    def fetch_abi(self,contract_address=None,initial_network=None):
        """
        Fetches the ABI for the contract.

        :return: ABI of the contract.
        """
        initial_network = initial_network or self.api_mgr.rpc_mgr.scanner
        return get_abi(contract_address=contract_address or self.contract_address,initial_network=initial_network)
    ...
    def update_api_mgr(self,api_mgr=None,contract_address=None):
        ...
        if self.api_mgr == None:
            self.api
            _mgr= APIBridge(contract_address=self.contract_address)
        self.contract_address=contract_address or self.contract_address
        if self.api_mgr.contract_address != self.contract_address:
            source_code = derive_network(contract_address=self.contract_address,initial_network=self.api_mgr.rpc_mgr.name)
            input(source_code)
            self.abi=source_code['contract_data'][0]['ABI']
            self.rpc_js = source_code['network']
        self.contract_address = self.api_mgr.contract_address
        self.update_rpc_mgr(rpc_mgr=self.api_mgr.rpc_mgr,rpc_js=self.rpc_js)
    def create_abi_bridge(self,contract_address:str=None,abi:list=None,rpc_mgr=None,rpc_js=None,api_gui=False):
        ...
        """
        abi= abi or self.abi
        contract_address = checksum(address=contract_address or self.contract_address)
...
from abstract_blockchain import APIBridge,RPCBridge
from abstract_blockchain.abstract_blockchain_functions import *

rpc_mgr = RPCBridge(rpc_js='avalanche')
api_mgr = APIBridge(rpc_mgr=rpc_mgr)
abi_mgr = ABIBridge(api_mgr=api_mgr,contract_address='0xc4D8C30B988Cb46913efEC01142033Ee9A1CdE3A')
input(abi_mgr.api_mgr.rpc_mgr.rpc_js)
contract_address='0xc4D8C30B988Cb46913efEC01142033Ee9A1CdE3A'
source_code = derive_network(contract_address=contract_address,initial_network=api_mgr.rpc_mgr.name)
input(source_code)
abi=source_code['contract_data'][0]['ABI']
