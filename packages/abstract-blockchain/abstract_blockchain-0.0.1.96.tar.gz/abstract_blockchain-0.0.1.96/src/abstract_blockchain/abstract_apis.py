...
class APIURLManager:
    ...
    def add_url(self,url,output):
        ...
        if self.scanner not in self.url_manager:
            self.url_manager[self.scanner]=[]
        url_parts = url.split('?')
        scanner=strip_web(url_parts[0])
        self.url_manager[self.scanner].append({"action":"","url":[],"output":[],"inputs":{}})
        self.url_manager[self.scanner][-1]["url"].append(url)
        self.url_manager[self.scanner][-1]["output"].append(output)
        for item in url_parts[-1].split('&'):
            item_parts = item.split('=')
            if item_parts[0] == 'action':
                self.url_manager[self.scanner][-1]['action']=item_parts[-1]
            else:
                self.url_manager[self.scanner][-1]["inputs"][item_parts[0]]=item_parts[-1]
...
class APIBridge:
    ...
    def _make_request(self, url):
            ...
            try:
                ...
                return None
api_mgr=APIBridge()        
api_mgr_gui = ApiGUI(api_mgr=api_mgr,api_gui=True)
