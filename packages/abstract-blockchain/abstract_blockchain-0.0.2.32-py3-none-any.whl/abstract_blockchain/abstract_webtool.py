...
class UserAgentManager:
    ...
    @staticmethod
    def user_agent_db():
        from abstract_webtools.big_user_agent_list import big_user_agent_dict
...
class SafeRequest:
    ...
    def __init__(self,
        ...
        self.dynamic_rate_mgr = DynamicRateLimiterManager()
    ...
    def make_request(self):
        ...
        return None
    def try_request(self,url=None) -> Union[requests.Response, None]:
        """
        ...
        """
        url = url or self.url_manager.url
        try:
            return self.session.get(url=url, timeout=self.timeout,stream=self.stream)
        except requests.exceptions.RequestException as e:
            print(e)
            return None
    def get_limited_request(self,  request_url, service_name):
        manager = self.dynamic_rate_mgr
        while True:
            if manager.request(service_name):
                response = self.try_request(url=request_url)
                if response:
                    if response.status_code == 429:
                        self.handle_rate_limit_error(manager, service_name, response)
                        continue
                    ...
                    self.handle_network_or_other_errors(manager, service_name)
            else:
                self.wait_for_next_epoch(manager, service_name)

    def handle_rate_limit_error(self, manager, service_name, response):
        print(f"Rate limited. Response: {response.json()}")
        manager.services[service_name].request_tracker(False)
        self.wait_based_on_current_tokens(manager, service_name)

    def handle_network_or_other_errors(self, manager, service_name):
        manager.services[service_name].request_tracker(False)
        print("Network error or other issue. Retrying...")
        self.wait_for_next_epoch(manager, service_name)

    def wait_for_next_epoch(self, manager, service_name):
        sleep_duration = manager.services[service_name].get_sleep()["current_sleep"]
        print(f"Rate limit reached for {service_name}. Waiting for the next epoch...")
        demo_one_line_progress_meter(f"Service Name: {service_name}",
                                                 f"Limit epoch: {manager.services[service_name].limit_epoch}",
                                                 f"epoch Left: {manager.services[service_name].calculate_tokens()['epoch_left']}",
                                                 f"succesful attempts: {len(manager.services[service_name].calculate_tokens()['succesful'])}",
                                                 f"unsuccesful attempts: {len(manager.services[service_name].calculate_tokens()['unsuccesful'])}",
                                                 f"Tokens Avalable : {manager.services[service_name].get_current_tokens()}",
                                                 f"Count since Fail: {manager.services[service_name].calculate_tokens()['count_since_fail']}",
                                                 f"status code: ...",
                                                 f"Rate limited by {service_name}. Adjusted limit. Retrying...",
                                                 max(manager.services[service_name].limit_epoch,1)/ max(len(manager.services[service_name].calculate_tokens()["succesful"]),1))  # Fill in with appropriate arguments
     

    def wait_based_on_current_tokens(self, manager, service_name):
        tokens = len(manager.services[service_name].calculate_tokens()["succesful"])
        if tokens < 2:
            self.wait_for_next_epoch(manager, service_name)
        else:
            sleep_duration = manager.services[service_name].limit_epoch / tokens
            demo_one_line_progress_meter(f"Service Name: {service_name}",
                                                 f"Limit epoch: {manager.services[service_name].limit_epoch}",
                                                 f"epoch Left: {manager.services[service_name].calculate_tokens()['epoch_left']}",
                                                 f"succesful attempts: {len(manager.services[service_name].calculate_tokens()['succesful'])}",
                                                 ...
                                                 f"status code: ...",
                                                 f"Rate limited by {service_name}. Adjusted limit. Retrying...",
                                                 manager.services[service_name].limit_epoch) # Fill in with appropriate arguments
...
def demo_one_line_progress_meter(service_name,limit_epoch,epoch_left,successful_requests, unsuccessful_requests,token_available,status_code,count_since_fail,response_message,sleep_period):
    # Display a progress meter. Allow user to break out of loop using cancel button

    
 for i in range(int(sleep_period)):
        if not sg.one_line_progress_meter(service_name,
                                          i+1, int(sleep_period),
                                          f"{limit_epoch}\n{epoch_left}",
...
class DynamicRateLimiterManager:
    def __init__(self,service_name='ethereum'):
        ...
        self.add_service(service_name=self.service_name)
    ...
    def get_limited_request(self,limiter,request_url):
        ...
        while True:
            ...
            if not limiter.request():
                ...
                demo_one_line_progress_meter(f"Service Name: {self.service_name}",
                                                 f"Limit epoch: {self.services[self.service_name].limit_epoch}",
                                                 f"epoch Left: {self.services[self.service_name].calculate_tokens()['epoch_left']}",
                                                 f"succesful attempts: {len(self.services[self.service_name].calculate_tokens()['succesful'])}",
                                                 f"unsuccesful attempts: {len(self.services[self.service_name].calculate_tokens()['unsuccesful'])}",
                                                 f"Tokens Avalable : {self.services[self.service_name].get_current_tokens()}",
                                                 f"Count since Fail: {self.services[self.service_name].calculate_tokens()['count_since_fail']}",
                                                 f"status code: None",
                                                 "Rate limit reached for coin_gecko. Waiting for the next epoch...",
                                                 self.services[self.service_name].get_sleep()["current_sleep"])# Wait for the limit_epoch duration
            ...
            if response and response.status_code ==429:
                self.services[self.service_name].request_tracker(False)
                print(f"Rate limited by {self.service_name}. Adjusted limit. Retrying...")
                if len(manager.services[self.service_name].calculate_tokens()["succesful"])<2:
                    ...
                    demo_one_line_progress_meter(f"Service Name: {self.service_name}",
                                                 ...
                                                 "Rate limited by coin_gecko. Adjusted limit. Retrying...",
                                                 self.services[self.service_name].get_sleep()["current_sleep"])  # Wait for the limit_epoch duration
                else:
                    self.services[self.service_name].current_limit-=1
            ...
            if response and response.status_code == 200:
                ...
                demo_one_line_progress_meter(f"Service Name: {self.service_name}",
                                                 ...
                                                 f"Count since Fail: {self.services[self.service_name].calculate_tokens()['count_since_fail']}",
                                                 f"status code: {response.status_code}",
                                                 f"Unexpected response: {response.status_code}. Message: {response.text}",
                                                 self.services[self.service_name].get_sleep()["current_sleep"])
                print(f"Unexpected response: {response.status_code}. Message: {response.text}")
                return None
