...
class RPCBridge: 
    ...
    def get_default_rpc_list():
        rpc_list =safe_read_from_json(os.path.join(self.directory_path,'data','rpc_list.json'))
...
class RPCGUIManager:
    def __init__(self, rpc_mgr=None,rpc_list:list=None,gui_window=True):
        ...
        if gui_window:
            self.window_name = self.window_mgr.add_window(title='RPC Selector',layout=self.get_rpc_layout(),close_events=[text_to_key("-OK_RPC-",section=section),text_to_key("-EXIT_RPC-",section=section)],event_handlers=[self.rpc_win_while],suppress_raise_key_errors=False, suppress_error_popups=False, suppress_key_guessing=False,finalize=True)
    ...
    def get_rpc_layout(self,section=None):
        ...
        layout = [
            [sg.Text('SEARCH'), sg.Input('', key=text_to_key('SEARCH',section=section),size=(20,1), enable_events=True)],
            [sg.Text('Network Name:'), sg.Combo(self.network_names,default_value=name, key=text_to_key('NETWORK_NAME',section=section), enable_events=True)],
            [sg.Text('Network:'), sg.Combo(networks,default_value=network,key=text_to_key('NETWORK',section=section),size=(20,1), enable_events=True)],
            [sg.Text('RPC:'), sg.Combo(rpcs,default_value=rpc, key=text_to_key('RPC',section=section),size=(20,1), enable_events=True)],
            [sg.Text('ChainID:'), sg.InputText(chain_ids,key=text_to_key('CHAINID',section=section),size=(20,1), disabled=True)],  # Make this an InputText to display ChainID
            [sg.Text('Block Explorer:'), sg.Combo(explorers,default_value=explorer, key=text_to_key('BLOCK_EXPLORER',section=section),size=(20,1), enable_events=True)],
            [sg.Text('Symbol:'), sg.InputText(symbol,key=text_to_key('SYMBOL',section=section),size=(20,1), disabled=True)]  # Make this an InputText to display Symbol
        ]
        layout.append(create_row_of_buttons({"button_text":"OK","enable_event":True,"key":text_to_key("OK_RPC",section=section)},
                                            {"button_text":"Show","enable_event":True,"key":text_to_key("SHOW_RPC",section=section)},
                                            {"button_text":"Reset","enable_event":True,"key":text_to_key("RESET_RPC",section=section)},
                                            {"button_text":"Exit","enable_event":True,"key":text_to_key("EXIT_RPC",section=section)}))
        return layout
    ...
    def get_rpc_values(self,values) -> dict or None:
        ...
        testnet=False
        need = []
        for section_key in [self.script_event_js['-NETWORK_NAME-'],self.script_event_js['-SYMBOL-'],self.script_event_js['-CHAINID-'],self.script_event_js['-NETWORK-'],self.script_event_js['-BLOCK_EXPLORER-'],self.script_event_js['-RPC-']]:
            value = values.get(section_key)
            original_key = section_key[:-len(f"_{self.script_event_js['section']}-")]+'-'
            converted_key = self.get_key_from_value(value=original_key)
            if section_key == self.script_event_js['-NETWORK-']:
                if 'testnet'.lower() in str(value).lower():
                    testnet=True
            else:
                new_rpc[converted_key] = value
        new_rpc['rpc']=[{"url":new_rpc.get('rpc')}]
        new_rpc['explorers']=[{"url":new_rpc.get('explorer')}]
