...
class w3Manager:
    def __init__(self):
        self.w3_management = {}
        self.default_rpc_js = RPCBridge.get_default_rpc(Network_Name="Ethereum")  # Assuming get_default_rpc is defined elsewhere
...
def checksum(address: str=None,rpc=None):
    """
...
def derive_network(address=None,multiple=False,initial_network='ethereum'):
    ...
    rpc_list,common_chains = rpc_mgr_reference.get_default_rpc_list(),rpc_mgr_reference.get_common_chains()
    ...
    if source_code:
        return {"network":rpc_mgr_reference.rpc_js,"contract_data":read_json(source_code)}
    ...
    for list_itteration,rpc_js in enumerate(network_list):
        ...
        if rpc_js:
            ...
            if found:
                ...
                if multiple:
                    save_source_code(rpc_mgr.rpc_js,response,address)
                    networks_found.append({"network":rpc_mgr.rpc_js,"contract_data":response})
                else:
                    save_source_code(rpc_mgr_reference.rpc_js,  response, address)
                    return {"network":rpc_mgr_reference.rpc_js,"contract_data":response}
