...
class ABIBridge:
    ...
    def fetch_abi(self,contract_address=None,initial_network=None):
        ...
        """
        initial_network = initial_network or self.api_mgr.rpc_mgr.rpc_js
    ...
    def create_abi_bridge(self,contract_address,abi,rpc_js):
        """
        Create a contract bridge using the ABI and contract address.

        :return: Contract bridge instance.
        """
        contract_address = checksum(address=contract_address,rpc_js)
        if abi==None:
            self.abi = self.fetch_abi(contract_address)
        return RPCBridge(rpc_js=rpc_js).w3.eth.contract(address=contract_address, abi=abi)
...
input(rpc_mgr.rpc)
api_mgr = APIBridge(rpc_mgr=rpc_mgr,contract_address='0x7D656F7Bd75De5CBeEf598EbF6abBeCAB854088F')
input(api_mgr.api_url)
input(api_mgr.rpc_mgr.rpc)
abi_mgr = ABIBridge(api_mgr=api_mgr)
input(abi_mgr.api_mgr.rpc_mgr.explorer)
input(abi_mgr.api_mgr.contract_address)
input(abi_mgr.contract_functions)
def call_function(self,function_name:str,contract_bridge=None,*args,**kwargs):
    ...
    # If no arguments, just call the function.
        ...
        return contract_function().call()
input(call_function("decimals",abi_mgr.contract_bridge))
#call_function(function_name='decimal'))
input(abi_mgr.api_mgr.rpc_mgr.w3)
contract_address='0x9c07e632706A4B5730D68614DAE84d589B647a2d'
