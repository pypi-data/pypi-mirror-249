...
from ..internal import FxConfig, t
...
async def _check_blacklist(interaction: discord.Interaction) -> bool:
    ...
    if interaction.user.id in bans and FxConfig.blacklist:
        if FxConfig.blacklist.raise_error:
            raise Blacklisted()
        else:
...
def check_command(decorator):
    ...
    def decorator_wrapper(func):
        ...
        if name not in FxConfig.blacklist.disabled_commands:
...
async def check_overwrite(name, *args) -> bool:
    ...
    if not FxConfig.blacklist:
        return False

    if name in FxConfig.blacklist.overwrites:
        await FxConfig.blacklist.overwrites[name](*args)
        return True
...
class Blacklist(Cog, hidden=True):
    ...
    async def cog_check(self, ctx):
        if FxConfig.blacklist.owner_only:
    ...
    if PYCORD:
        admin = discord.SlashCommandGroup(
            t("admin_group"),
            description="fxstcloud admin commands",
            guild_ids=FxConfig.admin_guilds,
