...
class MarkerApp:
    def __init__(
        ...
        self.mode = Mode.READ
        self.mode_last: Optional[Mode] = self.mode
        self.task = Task.RECT
        ...
        self.trackbar_mode = self.build_trackbar(
            ...
            name=f"You can {' | '.join([x.name for x in Mode])}",
            count=len(Mode),
            # fn=lambda x: self.set_mode(x),
        ).unwrap()

        # task trackbar
        self.trackbar_task = self.build_trackbar(
            window=self.window_name,
            name=f"You can draw {' | '.join([x.name for x in Task])}",
            count=len(Task),
            # fn=lambda x: self.set_task(x),
        ).unwrap()

        # class trackbar
        self.trackbar_class = self.build_trackbar(
            window=self.window_name,
            name=f"Which class to display? ({', '.join(self.m_cls.names + ['ALL INCLUDED'])})",
            initial=self.m_cls.count + 1,
            count=self.m_cls.count + 1,
            fn=lambda x: self.m_cls.set_single_class(x),
        ).unwrap()
        ...
        if model is None:
            ...
            if self.model.task is NotImplemented:
                raise NotImplementedError(
                    "Model type not supported! Try using yolov8 serial models."
                )
                ...
                # build nc trackbar
                self.trackbar_nc = self.build_trackbar(
                    window=self.window_name,
                ...
                raise NotImplementedError(
                    "Segments model is not supported for now. Please wait."
                )
            else:
                raise NotImplementedError(
                    "Model is not supported. Try yolov8 detect model."
                )

    def set_mode(self, idx: int):
        # TODO
        self.mode_last = self.mode
        self.mode = list(Mode)[idx]

    def set_task(self, idx: int):
        # TODO
        self.task = list(Task)[idx]
    ...
    def mainloop(self):
        while True:
            ...
            #     self.try_overlay_print("> Bboxes highly overlappped")
    ...
    def check_mark_task(self, line: list) -> Task:
        ...
        # =>>>> 5 + 3*x = 1 + 2*y
        # =>>>> collision happens a lot! e.g. 65
        n = len(line)
        if 0 < n < 5:
            ...
            with open(self.m_image.path_label, "r") as f:
                for idx, line in enumerate(f):
                    ...
                    if task_ is NotImplemented:
                        self.try_overlay_print(
                            f"Error: Label format is wrong, length < 5."
                        )
                        print(
                            f"> Unsupported label format path: {self.m_image.path_label}"
                        )
                        continue
                    ...
                    if task_ in (Task.RECT, Task.POINT):
                        ...
                        # show single class
                        if self.m_cls.single_clsidx4showing is not None:
                            if cls_id != self.m_cls.single_clsidx4showing:
                                continue
                        ...
                        color = self.palette(cls_id)
                        cv2.polylines(
                            img, np.int32([xys_]), True, color, line_thickness
                        )
    ...
    def highlight_selected_or_not(self, img):
        if self.m_image.has_bbox_selected():
            ...
            if _bbox.segments is not None:
                cv2.fillPoly(
                    mask_highlight, np.int32([_bbox.segments]), (255, 255, 255, 0)
                )
