...
class KeyBoard:
    ...
    def listen(self, delay=1):
        ...
        # DOODLE <-> READ
            ...
            if self.marker.mode in (Mode.READ, Mode.MARK):
                self.marker.mode_last = self.marker.mode
                self.marker.mode = Mode.DOODLE
                self.marker.try_overlay_print("You're doodling!")
            elif self.marker.mode is Mode.DOODLE:
                self.marker.mode = self.marker.mode_last
                self.marker.convas = None
                self.marker.try_overlay_print(
                    f"You're back to {self.marker.mode.map_str()} mode!"
                )
        ...
        # MARK <-> READ
            ...
            if self.marker.mode is Mode.READ:
                ...
                self.marker.try_overlay_print(
                    f"You're {self.marker.mode.map_str()} mode!"
                )
        ...
        # switch classes
            ...
            # TODO: update class trackbar
            if self.marker.task is Task.POINT:
                cv2.setTrackbarPos(
        ...
        # show single class [0-9]
        elif _key in range(48, 58):
            value = int(chr(_key))
            if value <= self.marker.m_cls.count - 1:
                self.marker.m_cls.single_clsidx4showing = value
                self.marker.try_overlay_print(
                    f"> Only show class_id: {self.marker.m_cls.single_clsidx4showing} => {self.marker.m_cls.names[self.marker.m_cls.single_clsidx4showing]}"
                )
                cv2.setTrackbarPos(
                    self.marker.trackbar_class,
                    self.marker.window_name,
                    self.marker.m_cls.single_clsidx4showing,
                )
            else:
                self.marker.m_cls.single_clsidx4showing = None
                cv2.setTrackbarPos(
                    self.marker.trackbar_class,
                    self.marker.window_name,
                    self.marker.m_cls.count + 1,
                )
                self.marker.try_overlay_print(
