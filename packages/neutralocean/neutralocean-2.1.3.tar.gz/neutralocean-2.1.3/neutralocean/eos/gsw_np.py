...
def _process(SA, CT, p):
    # # Attempts to apply SA = max(SA, 0) below, but easier to just remove this
    # # and only trust (do tests on) values with SA >= 0.
    # if (
    #     isinstance(SA, (float, np.floating))
    #     or isinstance(SA, np.ndarray)
    #     and SA.dtype == np.floating
    # ):
    #     SA = np.maximum(SA, 0.0)
    # else:
    #     SA = np.maximum(SA, mp.mpf(0.0))
    x2 = sfac * SA + offset
    x = np.sqrt(x2)
    y = CT * tfac
    z = p * pfac
