...
class DataFrameGroupBy:
    def __init__(self, df, by, sort=True, dropna=True, nanstats=True, device=None):
        ...
        if dropna:
            ...
            isna = DataFrame({k: v.isna() for k, v in df[by]._items()})._to_numeric().any(dim=1)
            df = df[:, ~isna.data]
...
class ColumnGroupBy:
    ...
    def _agg(self, func, results):
        ...
        if func == 'count':
            ...
            # In pytorch calling __setitem__ with an index tensor that has duplicate elements is undefined and non-deterministic by
            # default. See the documentation on index_put_. In my own experiments, setting use_deterministic_algorithms resolves this
            # issue and makes pytorch behave the same way as numpy, but I don't think this behavior is gauranteed.
            ...
            if self.nan_policy == 'omit':
                if v.is_tensor and v.device.type == 'mps':
                    # bug with MPS argsort where -torch.nan is treated as the smallest, so we need to restrict the negative sign
                    # to non-NaN elements. See Github issue #116567
                    isnan = v.data.isnan()
                    v_ = ar.Array(v.data.clone())
                    v_[~isnan] = -v_[~isnan]
                    i = v_.argsort(descending=True) # same as descending=False but NaNs are at the front
                else:
