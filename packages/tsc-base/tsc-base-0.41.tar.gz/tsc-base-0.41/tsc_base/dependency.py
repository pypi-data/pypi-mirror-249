...
class Excel:
    ...
    def get_excel_table(
        ...
        **kw):
        ...
        Args:
            ...
            ordered (bool, optional): 是否保留 doc_L 中字段的顺序，使用后速度会慢一些
    ...
    def excel_add_sheet(workbook, name, th_L, td_L, package, index=0, save_path=None, tqdm_f=None,
        ...
        Args:
            ...
            auto_adjust_width (bool, optional): 是否自动调整单元格列宽以适应文本长度, 只有 openpyxl 支持
            ...
            if auto_adjust_width:
                ...
                for row in worksheet.iter_rows():  # 遍历所有行
                    for cell in row:  # 遍历行中的所有单元格
                        if isinstance(cell, package.cell.cell.MergedCell):  # 合并单元格跳过
                            continue
                        if cell.value:
...
if __name__ == '__main__':
    ...
    print('=' * 10, 'Excel')
    doc_L = [{
        'a1': {'b11': 1, 'b2': 2, 'b3': {'c1': 1, 'c22222': False}},
        'a2': '123123123',
        'a3': {},
        'a4': None,
        'a5': [],
    ...
    th_D, th_L, td_L = Excel.get_excel_table(doc_L, ignore_th_f=ignore_th_f)
    print('th_D:')
    pprint(th_D)
    import xlwt
    import openpyxl
    from tqdm import tqdm
    Excel.excel_add_sheet(openpyxl.Workbook(), 'test', th_L, td_L, openpyxl, 0, 'test/openpyxl', tqdm, auto_adjust_width=True)
    Excel.excel_add_sheet(xlwt.Workbook(encoding='utf8'), 'test', th_L, td_L, xlwt, 0, 'test/xlwt', tqdm, auto_adjust_width=True)
