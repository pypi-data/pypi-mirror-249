...
class LinearTikhonov(Tikhonov):
    ...
    # TODO: rename derivaties to be more general
    def invert(self, signals, gmat, derivatives, param_kw):
        """
        Inverses normalised signals using `mfi_num` Fisher Information cycles each with `iter_max` steps of regularisation parameter optimisation.
        ...
        gmat : scipy.sparse.csr_matrix
            geometry matrix normalised by estimated errors
        derivatives : list
            list of sparse derivatives matrices
        w_factor : numpy.ndarray, optional
            weight matrix multipliers with shape (#nodes, )
        w_max : float, optional
            value used in weigh matrix for zero or negative nodes
        ...
        npix = gmat.shape[1]
        g = np.ones(npix)
        objective = self.regulariastion_matrix(derivatives)
        parameter = self.determine_parameter(objective)
        m = self.gdg + 10 ** parameter * objective
        g = self.solve(m, self.gdsig)
        ela = time.time() - ela
        print('last chi^2 = {:.4f}, time: {:.2f} s'.format(self.last_chi, ela))
        return g

class NonlinearTikhonov(Tikhonov):
    def invert(self, signals, gmat, derivatives, w_factor=None, optimization_loops=3, bounds=(-15, 0), iter_max=10, w_max=1, aniso=0):
        """
        Inverses normalised signals using `mfi_num` Fisher Information cycles each with `iter_max` steps of regularisation parameter optimisation.
        ...
        g = np.ones(npix)
        loop_count = 0
        while loop_count < optimization_loops:
            w = 1 / g
            w[w < 0] = w_max
            w = sparse.diags(w)
            if w_factor is not None:
                w = w * sparse.diags(w_factor)
            regularisation, stats = self.determine_parameter()
            m = self.gdg + regularisation
            g = self.solve(m, self.gdsig)
            loop_count += 1
        ela = time.time() - ela
        print('last chi^2 = {:.4f}, time: {:.2f} s'.format(self.last_chi, ela))
