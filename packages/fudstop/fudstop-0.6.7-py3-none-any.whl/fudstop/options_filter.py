...
sdk = PolygonOptions(user='postgres', database='fudstop')
wb_opts = WebullOptions(access_token=os.environ.get('ACCESS_TOKEN'), osv=os.environ.get('OSV'), did=os.environ.get('DID'), user='postgres', database='fudstop')
...
class QueryMenu(disnake.ui.Select):
    def __init__(self):
        ...
        super().__init__(custom_id='results', row=2, min_values=1, max_values=5, options= [ 
            ...
            disnake.SelectOption(label='expire_date', description='Filter options by expiration eact or min/max.', emoji=expiry_em),
...
class FilterModal(disnake.ui.Modal):
    ...
    async def callback(self, inter: disnake.ModalInteraction):
        conn = await wb_opts.db_manager.get_connection()
