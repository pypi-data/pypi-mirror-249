...
from .models.option_models.universal_snap import UniversalOptionSnapshot
...
class PolygonOptions:
    def __init__(self,user:str='chuck', database:str='poly'):
        ...
        self.db_config = {
            ...
            "user": os.environ.get(f'{user}'), # Default to 'postgres' if 'DB_USER' not found
    ...
    async def update_options(self, ticker):
        ...
        df = all_options.df
        print(df)
        asyncio.create_task(self.batch_insert_dataframe(df, table_name='opts', unique_columns='option_symbol'))
    ...
    async def filter_options(self,select_columns:str=None, **kwargs):
        ...
        query = f"SELECT * FROM opts WHERE oi > 0" if select_columns is None else f"SELECT ticker, strike, cp, expiry, {select_columns} FROM poly_options WHERE oi > 0"
    ...
    async def fetch_filtered_records(self, **kwargs):
        ...
        select_sql = "SELECT ticker, strike, expiry, call_put FROM poly_options"
...
# Example usage
    ...
    async def get_price(self, ticker:str):
        ...
        if datas is not None and hasattr(datas, 'web_stock_close'):
    ...
    async def get_option_chain_all(self, underlying_asset, strike_price=None, strike_price_lte=None, strike_price_gte=None, expiration_date=None, expiration_date_gte=None, expiration_date_lte=None, contract_type=None, order=None, limit=250, sort=None, pool=None):
        """
        ...
        :param expiration_date_lte: Query for expiration dates less than or equal to a value.
    ...
    async def find_symbols(self, underlying_asset, strike_price=None, strike_price_lte=None, strike_price_gte=None, expiration_date=None, expiration_date_gte=None, expiration_date_lte=None, contract_type=None, order=None, limit=250, sort=None):
        """
        Get all options contracts for an underlying ticker across all pages.
    ...
    async def full_skew(self, ticker:str):
        ...
        columns_to_save = ['ticker', 'underlying_price', 'strike', 'expiry', 'dte', 'call_put','gamma','vega','iv','vol','oi','ask', 'bid']
        # Define the list of columns you want to keep
        columns_to_keep = ['strike', 'expiry', 'call_put', 'iv']  # Replace with your desired columns
        ...
        async with asyncpg.create_pool(**db_config) as pool:
            ...
            for ticker, status in extreme_tickers_with_status:
                if status == 'overbought':
                    ...
                    all_options = await self.get_option_chain_all(ticker, expiration_date_gte='2024-03-01', expiration_date_lte='2024-06-30', contract_type='put')
                ...
                if status == 'oversold':
                    ...
                    all_options = await self.get_option_chain_all(ticker, expiration_date_gte='2024-03-01', expiration_date_lte='2024-11-30', contract_type='call')
    ...
    async def vol_oi_top_strike(self, ticker: str):
        ...
        # Columns to return
        columns_to_return = ['ticker', 'strike', 'expiry', 'call_put', 'oi', 'vol']

        # Create an empty DataFrame to store the final results
        final_df = pd.DataFrame()

        # Group by 'cp' (call/put) and 'expiry'
        grouped = all_options_df.groupby(['call_put', 'expiry'])
    ...
    async def gpt_filter(self, ticker:str=None, strike:float=None, strike_min:float=None, strike_max:float=None, call_put:str=None, expiry:str=None, expiry_min:str=None, expiry_max:str=None, dte_max:int=None, dte_min:int=None, 
        ...
        table = tabulate(df, headers=['sym', 'strike', 'call_put', 'expiry', 'theta'], tablefmt='fancy', showindex=False)
