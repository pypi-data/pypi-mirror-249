...
def unet_manual_cast(weight_dtype, inference_device):
    ...
    fp16_supported = vispunk_motion.comfy.model_management.should_use_fp16(inference_device, prioritize_performance=False)
...
def resolve_lowvram_weight(weight, model, key):
    if weight.device == torch.device("meta"): #lowvram NOTE: this depends on the inner working of the accelerate library so it might break.
        ...
        op = vispunk_motion.comfy.utils.get_attr(model, '.'.join(key_split[:-1]))
