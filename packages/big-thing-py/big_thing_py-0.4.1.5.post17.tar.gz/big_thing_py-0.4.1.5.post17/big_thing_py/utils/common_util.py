...
def check_python_version():
    if sys.version_info[0] <= 3 and sys.version_info[1] < 6:
        raise Exception('Avahi feature is not supported on python3.6<=. try python3.7 or higher. or disable avahi feature. (avahi_enable=False)')
...
def sdk_version():
    init_path = os.path.join(get_project_root(), 'big_thing_py', '__init__.py')
    with open(init_path) as fp:
        return re.compile(r'''.*__version__ = ["'](.*?)['"]''', re.S).match(fp.read()).group(1)
