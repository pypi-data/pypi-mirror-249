...
class ExtraNamelistItem:
    ...
    def __init__(self, value, index=None, active: Optional[bool] = None):
        ...
        if self._validate_scalar_value(value) and index is None:
            ...
            self._type = type(value[0][0])
        else:
            raise ExtraNamelistsError(f'Invalid extra namelist item index {index} for value {value}')
    ...
    def _validate_vector_value(cls, value) -> bool:
        ...
        For a vector item, a value is valid is it is a valid scalar item value, or an iterable thereof. All elements of
        an iterable must be the same type.
        ...
        return all(cls._validate_scalar_value(v) for v in value) and \
               all(isinstance(v, type_) for v in value if v is not None)

    @staticmethod
    def _validate_array_value(value) -> bool:
        ...
        if ncolumns == 0:
            return False
        if not all(len(row) == ncolumns for row in value):
            return False
        ...
        for row in value:
            if any(type(col) != type_ for col in row if col is not None):
                return False
