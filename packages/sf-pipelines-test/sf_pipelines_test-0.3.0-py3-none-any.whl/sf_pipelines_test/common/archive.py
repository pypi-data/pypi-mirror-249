...
from sf_pipelines_test.common.pyodide import in_pyodide
from sf_pipelines_test.common.data import DataContainer, PipelineConfig, TableConfig
...
class DataframeArchive:
    ...
    def _add_table(self, snap_dir: FS, table_spec: TableConfig, df: pd.DataFrame):
        ...
        ext = ".csv" if in_pyodide else ".parquet"
        with snap_dir.open(table_spec.id + ext, "wb") as f:
    ...
    def load_snapshot(self, snap_id) -> DataContainer:
        ...
        ext = ".csv" if in_pyodide else ".parquet"
        for table_spec in self.config.table_list:
            if snap_dir.exists(table_spec.id + ext):
                with snap_dir.open(table_spec.id + ext, "rb") as f:
                    if in_pyodide:
                        df = pd.read_csv(f)
                    else:
