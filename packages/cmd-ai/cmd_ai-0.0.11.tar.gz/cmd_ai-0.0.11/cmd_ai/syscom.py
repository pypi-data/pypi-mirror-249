...
def process_syscom(cmd):
    ...
    # *************************************** google functions
        ...
        if len(config.TOOLLIST)==0:
            ...
            config.TOOLLIST = [ texts.tool_getCzechWeather, texts.tool_searchGoogle, texts.tool_getWebContent ]
            for i in config.TOOLLIST:
                print(" ... ",i['function']['name'])
        else:
            print(f"i...  {bg.green}{fg.white} google search functions OFF NOW {bg.default}{fg.default}")
            config.TOOLLIST = []
