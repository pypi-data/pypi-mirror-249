...
def check_run_states_of_vms(vm_name):
    vm_group = f'/AosUnits/{vm_name}'

    print_message(f'Counting Units in Group: {vm_group} for the units...')
    ...
    detailed_list_vms_out = execute_command(detailed_list_vms_command, catch_error=True)
    detailed_list_vms_states = dict(list())
    processed_group_vm = ''
    for output_line in detailed_list_vms_out.splitlines():
        if output_line.startswith('Groups:') and len(output_line.split()) == 2:
            # line format is: 'Groups:                      /AosUnits/my-test-vm1'
            processed_group_vm = output_line.split()[1]
        elif output_line.startswith('State:') and len(output_line.split()) >= 2:
            # line format is: 'State:                        powered off (since 2023-12-21T12:57:04.699000000)'
            vm_state = output_line.split()[1]
            list_of_states = []
            if detailed_list_vms_states.get(processed_group_vm):
                list_of_states = detailed_list_vms_states.get(processed_group_vm)
            if vm_state not in list_of_states:
                list_of_states.append(vm_state)
                detailed_list_vms_states[processed_group_vm] = list_of_states

    if not detailed_list_vms_states.get(vm_group):
        raise OnUnitError(f'VM Group: {vm_group} is not presented on system.')

    count_of_running_vms = 0
    for vm_state in detailed_list_vms_states[vm_group]:
        if 'running' in vm_state:
            count_of_running_vms += 1

    if count_of_running_vms != len(detailed_list_vms_states[vm_group]):
        raise OnUnitError(f'Not all VMs in the Group: {vm_group} are running')

    print_message(f'All VMs from VM Group: {vm_group} are running')
