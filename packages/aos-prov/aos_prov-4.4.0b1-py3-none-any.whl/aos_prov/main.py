...
from aos_prov.actions import create_new_unit, download_image, provision_unit, start_vm_unit, start_check_vm_unit, remove_vm_unit
...
_COMMAND_START_CHECK_VM = 'vm-start-check'
...
def _parse_args():
    ...
    start_check_vm_command = sub_parser.add_parser(
        _COMMAND_START_CHECK_VM,
        help='Check that all VMs from groups are running'
    )
    start_check_vm_command.add_argument(
        '-N',
        '--name',
        required=True,
        help='Name of the VirtualBox group where VMs are located.'
    )
    start_check_vm_command.set_defaults(which=_COMMAND_START_CHECK_VM)
...
def main():
    ...
    try:
        ...
        if args.which == _COMMAND_START_CHECK_VM:
            start_check_vm_unit(args.name)
