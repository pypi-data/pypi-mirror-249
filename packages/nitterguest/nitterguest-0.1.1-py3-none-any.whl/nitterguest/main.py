...
class NitterGuest:
    ...
    def file_output(self, oauth_token: dict[str, str], filename: str) -> None:
        data = json.dumps(oauth_token)
        with open(filename, "w") as outfile:
            outfile.write(data)
        self.logger.info("File write succeed: token ", oauth_token, " filename ", filename)
    ...
    def _get_guest_token(self) -> str:
        ...
        if req.status_code == 200:
            ...
            self.logger.info("guest token received: ", t)
            return t
    ...
    def _get_flow_token(self, guest_token: str) -> str:
        ...
        if req.status_code == 200:
            ...
            self.logger.info("flow token received: ", t)
            return t
        raise ConnectionError()
    ...
    def _get_oauth_token(self, guest_token: str, flow_token: str) -> dict[str, str]:
        ...
        if req.status_code == 200:
            if req.json()["subtasks"][0]:
                ...
                self.logger.info("oauth token received: ", t)
                return t
        raise ConnectionError()
