...
def main():
    ...
    """
    parser = argparse.ArgumentParser(prog=PACKAGE_NAME, usage='nitterguest -t YOUR_ACCESS_TOKEN -o guest_accounts.jsonl -r', description=PACKAGE_NAME + ': create nitter guest json')
...
def _run(token: str, output: Optional[str]) -> None:
    ...
    print(oauth_token)
    if output:
        client.file_output(oauth_token, output)
