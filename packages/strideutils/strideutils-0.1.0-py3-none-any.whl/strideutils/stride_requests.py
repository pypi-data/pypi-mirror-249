...
import pytz
import requests
import stride_upstash
from requests.adapters import HTTPAdapter, Retry
from stride_config import config
...
@lru_cache
def get_supply(token: str, block_height: Optional[int] = None, LCD: str = config.stride.library_api):
    """
...
def get_block_height_in_past(
    ...
    if prev_time is not None:
        ...
        ts = (datetime.datetime.now() - prev_time).total_seconds()
...
def get_sttoken_apr(token: str) -> Optional[float]:
    ...
    """
    return float(stride_upstash.get('STTOKEN_APR_' + token.upper(), db_name='public'))
