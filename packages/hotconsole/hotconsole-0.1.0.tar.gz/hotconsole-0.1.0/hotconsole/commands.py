...
Ниже работающий пример класса main, при его запуске:
- Приложение перезапустится из-под админа
- Произойдет инициализация: у пользователя появится конфиг с версией = 1
- Сгенерируется батник, который ваш пользователь может запускать вместо получения ваших библиотек через pip
- Произойдет запуск: откроется окошко со списком горячих клавиш
- По горячей клавише alt+t:
    поскольку в качестве аргумента с номером опции указан None,
    появится вопрос в консоли с вариантами "Включить, Выключить, Перезапустить SERVICE",
    выбранный номер передастся в функцию
- По горячей клавише alt+1 
    поскольку в качестве аргумента указан 1,
    сразу же включится SERVICE
- Можно перейти в консольный режим и запустить команду
    фразой turn - тогда нужно будет выбрать вариант из списка опций
    фразой turn 1 / turn 2 / turn 3 - тогда сразу запустится команда
...
class Config(BaseModel):
    ...
    @staticmethod
    def actualize():
        raise NotImplementedError("Надо заменить на свой метод")
...
class Runner:
    def __init__(
        ...
        config_actualizer: Callable = lambda: None,
        title: str | None = None,
