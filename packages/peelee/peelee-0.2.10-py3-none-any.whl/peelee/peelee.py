...
def generate_random_colors(
    ...
    for _base_color in base_colors:
        hls_base_color = hex2hls(_base_color)
        # if very dark color but not pure colors(e.g. pure black or red), then use saturater to get lighter colors
        if hls_base_color[1] < 0.1 and hls_base_color[0] > 0:
            lighter_colors = saturater(_base_color, color_gradations)
        else:
            lighter_colors = lighter(_base_color, color_gradations)
...
def generate_random_hex_color_code(
    ...
    if hue is not None:
        random_hex_color = set_hue(random_hex_color, hue)
