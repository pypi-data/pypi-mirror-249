...
def get_color_slices(
    ...
    if int_lightness == 0 and color_slice_type == ColorSliceType.DARKER:
        print(
            f"{base_color}'s lightness is {lightness} and cannot create slices with {lightness} lightness for darker color. use 0 lightness for slices."
        )
    ...
    if int_saturation == 0:
        if color_slice_type == ColorSliceType.DARKER:
            print(
                f"{base_color}'s saturation is {saturation} and cannot create slices with 0 saturation for darker color. use 0 saturation for slices."
            )
        else:
            print(
                f"{base_color}'s saturation is {saturation} and not proper to create slices because it might change the color."
            )
...
def darker(base_color, n_color):
    """Given base color, return 'n' color hex codes from base color to darkest
    color."""
    return get_color_slices(base_color, n_color, ColorSliceType.DARKER)


def lighter(base_color, n_color):
    """Given base color, return 'n' color hex codes from base color to lightest
    color."""
    return get_color_slices(base_color, n_color, ColorSliceType.LIGHTER)
...
def set_hue(hex_color, hue):
    ...
    print(f"hue:{hls_color} -> {new_hls_color}")
...
def set_hls_values(hex_color, hue, saturation, lightness):
    ...
    print(f"hls update ({hex_color}):{hls_color} -> {new_hls_color}")
    return hls2hex(new_hls_color)
