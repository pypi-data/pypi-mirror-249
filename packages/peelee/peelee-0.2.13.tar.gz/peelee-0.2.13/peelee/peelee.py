...
def get_slice_colors(
    ...
    is_black = max(rgb_base_color) == min(rgb_base_color)
    ...
    if int_saturation == 0 and (color_slice_type == ColorSliceType.DARKER or is_black):
        saturation_list = [0 for _ in range(n_slices)]
    else:
        print(base_color, rgb_base_color, hls_base_color)
...
def set_hls_values(hex_color, hue, saturation, lightness):
    ...
    print(hex_color, hls_color, new_hls_color)
    return hls2hex(new_hls_color)
...
def generate_base_colors(hex_color_code, total):
    ...
    print("base color and triadic colors: ", hex_color_code, base_colors)
    return base_colors
