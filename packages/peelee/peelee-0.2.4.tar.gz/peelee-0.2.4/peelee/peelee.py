...
def _padding(num, target_length=2):
    """
...
def lighter(base_color, n_color):
    ...
    lighter_colors = [
        f"#{''.join(tuple(_padding(hex(color_ligher[index])) for color_ligher in color_rgb_ligher))}"
...
def generate_random_colors(
    ...
    random_color = generate_random_hex_color_code(min_color, max_color)
...
def generate_random_hex_color_code(min_color, max_color, color_name: ColorName = None):
    """
    ...
    if max_color <= 30:
        for index in range(0, 3):
            ...
            _random_color = _padding(hex(random_int))
    ...
    return random_hex_color_code + "".join(rgb)
