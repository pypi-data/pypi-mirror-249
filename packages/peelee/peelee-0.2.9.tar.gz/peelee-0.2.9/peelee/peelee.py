...
def lighter(base_color, n_color):
    ...
    try:
        ...
        print(
            f"Invalid argument: n_color={n_color}, base_color={base_color}, max={_max}"
        )
...
def divide_lightness_or_saturation(
    lightness_or_saturation, max_lightness_or_saturation, total
):
    """Create a list of values of lightness or saturation.

    Args:
        lightness_or_saturation (float): The value of lightness or saturation.
        max_lightness_or_saturation (float): The maximum value of lightness or saturation.
        total (int): The total number of divided values.
    """
    if lightness_or_saturation >= max_lightness_or_saturation:
        print(
            f"Invalid argument:max_hsl_value={max_lightness_or_saturation}, hsl_value={lightness_or_saturation}, total={total}"
        )
        return [lightness_or_saturation for _ in range(total)]
    ...
    # in most cases, the decimal length of lightness or saturation value is 16
    # the value of lightness or saturation is between 0 and 1, so (max_lightness_or_saturation - lightness_or_saturation) / total would be 0. therefore, multiply 10 to the 16th power and then calculate the division to get the divided lightness and saturation
    _decimal_length = 16
    _max = int(10**_decimal_length * max_lightness_or_saturation)
    _min = int(lightness_or_saturation * _max)

    if (_max - _min) // total == 0:
        print(f"Abnormal: (_max={_max}, _min={_min}, total={total}).")
        return [lightness_or_saturation for _ in range(total)]

    divided_lightness_or_saturation = list(range(_min, _max, (_max - _min) // total))[
        0:total
    ]

    # divide by _max to get the real value
    return [l / _max for l in divided_lightness_or_saturation]
...
def saturater(base_color, n_color, max_saturation=0.8, max_lightness=0.8):
    ...
    lightness_list = divide_lightness_or_saturation(lightness, max_lightness, n_color)
    saturation_list = []
    # don't apply saturation for black or dark gray
    if lightness < 0.01:
        saturation_list = [0 for _ in range(n_color)]
    else:
        saturation_list = divide_lightness_or_saturation(
            saturation, max_saturation, n_color
        )
...
def set_saturation(hex_color, saturation):
    ...
    print(f"saturation:{hls_color} -> {new_hls_color}")
...
def set_lightness(hex_color, lightness):
    ...
    print(f"lightness:{hls_color} -> {new_hls_color}")
    return hls2hex(new_hls_color)
...
def generate_random_colors(
    ...
    Secondly, it will generate 'colors_gradations' different hex color codes from base color to the lightest color.
        ...
        color_name - color name. default: None. If None, then use random color name. it's used to generate the 'seed' random color which will be used to get more triadic colors.
...
def generate_random_hex_color_code(
    min_color, max_color, color_name: ColorName = None, saturation=None, lightness=None
):
    """
    ...
    if lightness is not None:
        random_hex_color = set_lightness(random_hex_color, lightness)
...
class Palette:
    ...
    def __init__(
        ...
        dark_colors_colors_gradations=60,
        dark_colors_min=15,
        dark_colors_max=20,
        dark_colors_total=7,
        ...
        **kwargs,
        ...
        self.dark_colors_colors_gradations = dark_colors_colors_gradations
        ...
        self.dark_base_color_name = kwargs.get(
            "dark_base_color_name", ColorName.BLUE.name
        )
        self.dark_base_color_name = ColorName[self.dark_base_color_name]
        self.dark_base_colors = kwargs.get("dark_base_colors", None)
    ...
    def generate_palette_colors(self):
        ...
        if (
            ...
            dark_colors = [
                lighter(_base_color, self.dark_colors_colors_gradations)
...
def main():
    ...
    palette = Palette(
        ...
        dark_colors_colors_gradations=dark_color_colors_gradations,
        colors_min=0,
