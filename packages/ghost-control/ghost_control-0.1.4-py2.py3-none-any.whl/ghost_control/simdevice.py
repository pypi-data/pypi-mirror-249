...
if os.path.exists(r"lib/gbild64.dll") or os.path.exists(r"lib/gbild32.dll"):
    pass
else:
    if os.path.exists(r"lib"):
        ...
        os.mkdir(r"lib")
    ctypes.windll.user32.MessageBoxW(0, "请下载DLL文件并放入lib文件夹中", "提示", 0)
    url = r'https://pan.baidu.com/s/1wOYG90Iy33Si1IhWHoZeVg?pwd=7itb'
    os.system('explorer.exe %s' % url)
    sys.exit()

if sys.maxsize > 2 ** 32:
    try:
        dll = windll.LoadLibrary(r"lib/gbild64.dll")
        print('正在加载64位DLL')
    except:
        print('加载64位DLL失败, 请检查是否正确放置DLL文件')
        sys.exit()
else:
    try:
        dll = windll.LoadLibrary(r"lib/gbild32.dll")
        print('正在加载32位DLL')
    except:
        print('加载32位DLL失败, 请检查是否正确放置DLL文件')
        sys.exit()
