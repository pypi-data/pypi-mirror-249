...
def get_state_zip_from_instance(services=_A):
	C=services;G=f"{get_pods_endpoint()}/state";H=','.join(C)if C else'';I={INTERNAL_REQUEST_PARAMS_HEADER:'{}'};D=in_ci();A=requests.get(G,params={_D:H},headers=I,stream=not D);J=int(A.headers.get('Content-Length',0));B=A.content if D else b''
	with Progress()as E:
...
class CloudPodsService(ABC):
	...
	def _get_localstack_pod_version(J,pod_name,version=_A):
		...
		D=json.loads(C.content);E=D[_E];F=int(D['max_version'])
		...
		G=list(filter(lambda v:v[_F]==A,E));H=G[0]if G else E[-1];I=H['localstack_version'];return I
...
class CloudPodsClient(CloudPodsService):
	...
	def get_versions(D,pod_name,remote=_A):
		...
		return json.loads(A.content).get(_E,[])
...
class StateService:
	...
	def import_pod(I,source,show_progress=_C):
		...
		C=load_file(A,mode='rb');F=zipfile.ZipFile(io.BytesIO(C),'r');D=read_metadata_from_pod(F)or{};G=D.get(_D,[]);H=get_environment_metadata().get(E)
...
def load_state_with_progress_bar(content,number_services=0):
	...
	with Progress()as B:
		...
		for F in D.iter_lines():A=json.loads(F);LOG.debug('Loaded service: %s',A);G,H=A['service'],'✅'if A['status']else'❌';B.log(f"{G}: {H}");C+=1;B.update(E,completed=C)
...
def get_ls_version_from_health():
	try:A=f"{config.external_service_url()}/_localstack/health";B=requests.get(A).json();return B[_F]
