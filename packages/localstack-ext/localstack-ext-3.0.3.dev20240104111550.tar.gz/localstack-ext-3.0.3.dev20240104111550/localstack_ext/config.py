...
# truncate and hash S3 object keys to not go over the filesystem limit
# this is behind a feature flag to not break persistence between release, but will be switched on by default on the next
# release
S3_TRUNCATE_KEYS = localstack_config.is_env_true("S3_TRUNCATE_KEYS")
