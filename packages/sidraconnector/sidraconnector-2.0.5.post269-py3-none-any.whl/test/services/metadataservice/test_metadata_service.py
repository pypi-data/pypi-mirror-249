...
class TestMetadataService(unittest.TestCase):
  ...
  def test_should_get_entity_with_attributes_when_get_entity(self):
    ...
    self.assertEqual(entity_model.attributes[0].name, 'LoadDate')
    ...
    self.assertEqual(entity_model.attributes[0].databricks_type, 'STRING')
    self.assertEqual(entity_model.attributes[0].load_properties.special_format, "FROM_UNIXTIME(UNIX_TIMESTAMP())")
  ...
  def test_should_register_an_asset_and_update_its_info(self):
    ...
    # Act
    service.update_asset_loaded(actual.asset_id, entities_count, errors_count)
    
    # Assert
    actual_asset = service.get_asset(actual.asset_id)
    self.assertEqual(actual_asset.entities, entities_count)
    self.assertEqual(actual_asset.validation_errors, errors_count)  
