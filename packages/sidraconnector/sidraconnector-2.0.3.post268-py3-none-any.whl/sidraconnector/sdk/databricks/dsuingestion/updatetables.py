...
class UpdateTables:
  ...
  def execute(self, id_DIP):
    ...
    try:
        ...
        # SchemaEvolution
        validation_errors_sufix = "validationerrors"
        for entity in entities:
            ...
            if newAttributes:
                for newAttribute in newAttributes:
                    ...
                    self.spark.sql(f"ALTER TABLE {provider.database_name}.{entity.table_name}{validation_errors_sufix} ADD COLUMN {newAttribute.name} {newAttribute.hive_type} AFTER {previous_attribute[0].name}")  
                    print(f"ADDED new column {newAttribute.name} to {provider.database_name}.{entity.table_name}{validation_errors_sufix}")  
                    self.logger.info(f"ADDED new column {newAttribute.name} to {provider.database_name}.{entity.table_name}{validation_errors_sufix}")
