...
class EntityService():
  ...
  def get_edl_for_entity(self, id_entity):
      ...
      edl_items = edl.to_dict()['items']
      edl_for_entity = {"id": None, "last_delta_value": None, "id_delta_attribute": None, "delta_is_date": None}
      for d in edl_items:
          if d.get("id_entity") == id_entity:
              id_edl = d.get("id")
              last_delta_value_edl = d.get("last_delta_value")
              id_delta_attribute_edl = d.get("id_delta_attribute")
              delta_is_date = d.get("delta_is_date")
              edl_for_entity = {"id": id_edl, "last_delta_value": last_delta_value_edl, "id_delta_attribute": id_delta_attribute_edl, "delta_is_date": delta_is_date}
  ...
  def get_delta_attribute_name(self, entity_delta_load, attributes):
    attribute_name = None
  ...
  def update_entity_delta_load_in_incremental_loads(self, entity, data_loaded_df):
    edl = self.get_edl_for_entity(entity.id)
    if (self.is_incremental(edl)):
      delta_attribute = self.get_delta_attribute_name(edl, entity.attributes)
      if (delta_attribute is not None):
         edl["last_delta_value"] = data_loaded_df.agg({delta_attribute: "max"}).collect()[0][0]
