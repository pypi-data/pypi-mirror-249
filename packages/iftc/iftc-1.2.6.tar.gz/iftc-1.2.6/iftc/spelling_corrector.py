...
class SpellingCorrector:
    def __init__(self, additonal_acronyms=None):
        self.it_corrector = Intentional(additonal_acronyms)
    ...
    def detect_spelling_error(self, text: str):
        text = text_normalize(text)
        acronym_corrected = self.it_corrector.spelling_correction(text)
        word_candidates = self.uit_corrector.gen_candidate.word_candidate(acronym_corrected)
        return text, word_candidates