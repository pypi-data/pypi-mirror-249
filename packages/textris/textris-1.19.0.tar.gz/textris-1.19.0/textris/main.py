...
def title_screen():
    ...
    text=art.text2art("Textris 1.19.0  by  YknotTYD   ","big")
    l=len(text.split("\n")[1])
...
def main_menu_loop():
    ...
    if direction:=(Textris.click("up")-Textris.click("down")):
        if direction:
            selection=max(min(selection-direction,3),0)
    ...
    for b,string in zip((level_selection,creditsTYD,title,ewit),("Level  Selection","Credits","Title  Screen","Exit")):
        text+=art.text2art(string,("small","big")[b==selection])+"\n"*2
    ...
    if Textris.click("enter"):
        (level_selection_loop,lambda: None,title_screen,door)[selection]()
...
def level_selection_loop():
    ...
    if Textris.click("enter"):
        ...
        Textris.gravity=1/(grid[selection[1]][selection[0]]*2+1)
        main_loop()
