...
class packetHandler:
    ...
    def capture_packets_and_save_to_csv(self):
        ...
        if current_os == 'Windows' or current_os == 'Linux':
            ...
            # Apply the split_values function
            # df[['xnap.NG_RANnodeUEXnAPID_src', 'xnap.NG_RANnodeUEXnAPID_dst']] = df['xnap.NG_RANnodeUEXnAPID'].apply(
            #      lambda x: pd.Series(split_values(x)))
            # Convert the column to string and strip whitespace
            # df['xnap.NG_RANnodeUEXnAPID'] = df['xnap.NG_RANnodeUEXnAPID'].astype(str).str.strip()

            # Split the column into two
            df[['xnap.NG_RANnodeUEXnAPID_src', 'xnap.NG_RANnodeUEXnAPID_dst']] = df[
                'xnap.NG_RANnodeUEXnAPID'].str.split(',', expand=True)
            print(df[['xnap.NG_RANnodeUEXnAPID_src', 'xnap.NG_RANnodeUEXnAPID_dst']])

            # Replace 'nan' strings with actual NaN values
            df[['xnap.NG_RANnodeUEXnAPID_src', 'xnap.NG_RANnodeUEXnAPID_dst']] = df[
                ['xnap.NG_RANnodeUEXnAPID_src', 'xnap.NG_RANnodeUEXnAPID_dst']].replace('nan', pd.NA)
