...
class TreePlot(Svg):
    ...
    def __init__(
        ...
        self.end_sha_seg_map = {seg.end_sha: seg for seg in self.tree.flatten_segments()}
        self.end_ref_name_seg_map = {x.ref_name: x for x in self.tree.flatten_segments()}
    ...
    def _get_end_seg_counts(self) -> Dict[str, Tuple[int, int]]:
        ...
        for segment in self.end_ref_name_seg_map.values():
            ...
            seconds_from_root_end = int(
                (root_end_segment.end_entry.commit_date - curr_segment.end_entry.commit_date).total_seconds()
            )
            ...
            while curr_segment.end_sha != root_end_segment.end_sha:
                if curr_segment.ref_name != root_ref_name:
                    ...
                    if curr_segment.ref_name != root_ref_name:
                        ...
                        seconds_from_root_end = int(
                            (
                                root_end_segment.end_entry.commit_date - curr_segment.end_entry.commit_date
                            ).total_seconds()
                        )
                else:
                    curr_segment = list(
    ...
    def _calculate_positions(self):
        lane_progess_map = {}
        lane_initial_datetime_map = {x.ref_name: x.items[0].date_time for x in self.lanes}
        ...
        for item in self._sorted_items():
            ...
            days_from_offset = (curr_offset_date - item.date_time).total_seconds() / SECONDS_IN_DAY
            ...
            item.ypos = max(
                curr_offset,
                lane_progess_map[lane.ref_name] + 10 if lane.ref_name in lane_progess_map else curr_offset,
            )
    ...
    def _linear_position_correction(self):
        ...
        for index, item in enumerate(items):
            ...
            if prev_next_ts >= 0.001:
                item.ypos = prev_y + (next_y - prev_y) * (item.date_time - prev_d).total_seconds() / prev_next_ts
            else:
