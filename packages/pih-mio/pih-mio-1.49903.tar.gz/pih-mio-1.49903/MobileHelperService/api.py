...
# rid of SSHService.const
# from SSHService.const import Hosts
...
class MobileHelper(OutputStub):
    ...
    def get_file_arg_information(self, value: str | None) -> tuple[int, int]:
        if e(value):
            return (0, 0)
        file_index: int = -1
        ...
        marketer_unit_node: CommandNode = CommandNode(
            ...
            text=lambda: j(
                (
                    ...
                    b(
                        one(
                            A.R_U.by_job_position(A.CT_AD.JobPositions.MARKETER)
                        ).name
                    ),
                    "."
                )
    ...
    def choose_file(
        ...
        if nn(command_type):
            search_value = j(
                (FROM_FILE_REDIRECT_SYMBOL, A.D.get(command_type)[1], SPLITTER)
            )
        value_source: str | None = search_value
        if nn(search_value):
        ...
        file_index: int = -1
        file_pattern_index: int = -1
        file_index, file_pattern_index = self.get_file_arg_information(search_value)
        if file_index == 0:
            with self.output.make_loading(1, "Идет загрузка файлов. Ожидайте"):
                ...
                file_list: list[Note] = sorted(
                    A.R.filter(
                        ...
                        A.R_N.files(force_fetch=self.is_forced),
                    ).data,
            ...
            if len(search_value) != len(FILE_PATTERN_LIST[file_pattern_index]):
                splitter_index: int = search_value.find(SPLITTER)
                file_list_temp: list[Note] | None = (
                    file_list
                    if splitter_index == -1
                    else A.D.filter(
                        lambda note: note.title.lower().startswith(
                            search_value[file_name_index:splitter_index]
                        ),
                        file_list,
                    )
                )
                if splitter_index != -1:
                    search_value = search_value[splitter_index + len(SPLITTER) :]
                file_list_temp = A.D.filter(
                    lambda note: note.title.lower().find(
                        search_value[file_name_index:].lower()
                    )
                    != -1,
                    file_list_temp,
                )
                if e(file_list_temp):
                    self.output.error(
                        js(
                            (
                                "Файл с именем",
                                esc(search_value[file_name_index:]),
                                "не найден",
                            )
                        )
                    )
            ...
            for item in A.CT_CMDT:
                if title_list[0].lower() in A.D.map(
                    ...
                    command_type = item
                    break
            if nn(value_source) and value_source in self.arg_list:
                self.arg_list.remove(value_source)
            self.output.head1(
                js(
                    (
                        None if n(command_type) else "Файл",
    ...
    def robocopy_job_run_handler(self) -> None:
        ...
        if ne(source_job_name) and is_active(source_job_name) and not forced:
            ...
            def job_label_function(name: str) -> str:
                return j((b(name), ":", job_status_list_label_function(name)))
    ...
    def run_command(
        ...
        if e(text):
            if self.yes_no("Выбрать файл"):
                text = self.choose_file(command_type=command_type)[1]

        if command_type == A.CT_CMDT.SSH:
            host: str = (
                self.arg()
                or self.input.item_by_index(
                    "Выберите хост", A.D.to_list(Hosts), lambda item, _: item.name
                ).name
            )
            if ne(text):
            ...
            with self.output.make_loading(
                ...
                self.write_line(nl(j(("Выполнение запроса на ", b(host))), 2))
                self.write_line(
                    jnl(
                        A.R_SSH.execute(
                            text
        ...
        if command_type == A.CT_CMDT.CMD:
            ...
            if self.session.is_mobile:
                while True:
                    ...
                    try:
                        # with self.output.make_loading(1, "Выполнение команды. Ожидайте"):
    ...
    def show_journal_handler(self) -> None:
        # with self.output.make_separated_lines():
        self.execute_file(
            "show_journal_records",
            redirect_to_mobile_output=True,
            force_fetch=self.is_forced,
        )
    ...
    def marketer_statistics_handler(self) -> None:
        with self.output.make_loading(text="Создание статистики. Ожидайте"):
            self.execute_file("marketer_statistics_create", force_fetch=True)
        self.output.write_image(
                            "Статистика отзывов",
                            A.D_CO.file_to_base64(
                                A.PTH_STAT.get_file_path(A.CT_STAT.Types.POLIBASE_PERSON_REVIEW_NOTIFICATION)
                            ),
                        )
