...
class AsyncBingScraper:
    ...
    def __init__(self, headers=headers, max_retries=3, num_results=10, error_sleep = 1):
        """
        ...
        Args:
            ...
            max_retries: The maximum number of retries for failed requests (default: 3).
            num_results: The maximum number of results to get (default: 10)
            error_sleep: Time to sleep if the result not get or status not 200
        """
        self.headers = headers
        self.max_retries = max_retries
        self.num_results = num_results
    ...
    async def _fetch_results(self, session, q, pbar, retries=0):
        q_encode = urllib.parse.quote_plus(q)
        ...
        try:
            async with session.get(url, headers=self.headers, read_until_eof=False) as response:
                ...
                if response.status == 200:
                    ...
                    return await self._fetch_results(session, q, pbar, retries=retries + 1)
                else:
                    pbar.update(1)
                    return url, None, response.status
        except:
            return await self._fetch_results(session, q, pbar, retries=retries)

    async def _collect_results(self, queries):
        pbar = tqdm(total=len(queries), desc="Queries")
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=30)) as session:
            tasks = [self._fetch_results(session=session, q=query, pbar=pbar) for query in queries]
