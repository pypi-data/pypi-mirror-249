...
class RhoApiGraphqlAdapter:
    ...
    def process_file(self, file_id: str, name: str) -> dict:
        query = """
        mutation ProcessFile($fileId: String!, $name: String!) {
          processFile(fileId: $fileId, tableName: $name) {
            table {
              id
              name
              fileId
        ...
        """
        variables = {"fileId": file_id, "name": name}
        data, errors = self._make_request(query, variables)
        if errors:
            raise RhoApiError(errors)
