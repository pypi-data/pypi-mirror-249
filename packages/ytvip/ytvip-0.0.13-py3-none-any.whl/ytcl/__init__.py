...
class ToggleAutoDownloadPreview(HTTPEndpoint):
    async def post(self, request: Request):
        ...
        channel.save()
        ic(channel.auto_download_previews)
        # download previous videos
        if channel.auto_download_previews:
            schedule_download_previews_chunk(channel)
        else:
            QueuedTask.delete().where(
                QueuedTask.channel == channel,
                QueuedTask.operation == QueuedTask.DOWNLOAD_PREVIEW,
            ).execute()
