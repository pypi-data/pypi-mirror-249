...
class Video(Model):
    ...
    def schedule_download_preview(self):
        QueuedTask.create(
            ...
            operation=QueuedTask.DOWNLOAD_PREVIEW,
            priority=3,
...
class QueuedTask(Model):
    ...
    # important so that when you delete a channel, you cancel all tasks
    # but a task is not necessarily associated with a channel, so set null=True
    channel: Channel = ForeignKeyField(Channel, on_delete='CASCADE', null=True)
    operation = CharField()
...
def migrate():
    ...
    _DB_VERSION = 5
    with db.atomic():
        # if it's never been set before, it will be 0.
        if user_version == 0:
            pass

        elif user_version < _DB_VERSION:
            migrate(
                migrator.add_column(
                    QueuedTask._meta.table_name,
                    'channel_id',
                    QueuedTask.channel
                )
            )

        if user_version < _DB_VERSION:
            cur.execute(f"PRAGMA user_version = {_DB_VERSION}")
