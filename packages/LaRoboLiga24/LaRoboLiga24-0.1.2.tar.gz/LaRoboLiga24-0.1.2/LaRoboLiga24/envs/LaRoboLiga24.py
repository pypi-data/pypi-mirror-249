...
class LaRoboLiga_Arena(gym.Env):
    ...
    def load_env(self):
        ...
        if self.arena == "arena1":
            SDF = p.loadSDF('urdf/arena/robo_lega/model.sdf')
            texture_id = p.loadTexture('urdf/arena/robo_lega/model.sdf')
            p.changeVisualShape(SDF, 0, textureUniqueId=texture_id)
        ...
        if self.balls_location is not None:
            ...
            self.ball_3 = p.loadURDF('urdf/ball/ball_yellow.urdf', self.balls_location['yellow'],
                                     p.getQuaternionFromEuler([0, 0, 0]))
            self.ball_4 = p.loadURDF('urdf/ball/ball_green.urdf', self.balls_location['green'],
                                     p.getQuaternionFromEuler([0, 0, 0]))
        if self.humanoids_location is not None:
            ...
            self.humnaoid_4 = p.loadURDF('urdf/humanoid/humanoid_green.urdf', self.humanoids_location['green'],
                                         p.getQuaternionFromEuler([0, 0, 1.15]))
