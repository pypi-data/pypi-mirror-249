        ...
        async with S3ClientContext(request=request, volume_id=volume_id) as s3_client:
            ...
            try:
                ...
                if request.if_modified_since and last_modified and request.if_modified_since >= last_modified:
                    activity.status = Status.FAILED
                    return web.HTTPNotModified()
                # Documentation says that StorageClass is always an attribute, but in reality it is not included if the
                # storage class is STANDARD.
                if resp.get('StorageClass', None) in (S3StorageClass.DEEP_ARCHIVE.name, S3StorageClass.GLACIER.name):
                    return response.status_internal_error(f'Cannot access {awsservicelib._activity_object_display_name(bucket_name, key)} because it is archived in {S3StorageClass[resp["StorageClass"]].display_name}. Unarchive it and try again.')
