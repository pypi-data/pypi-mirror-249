...
class ActorNetwork(nn.Module):
    def __init__(
        ...
        modules = nn.ModuleList(
            [layer_init(skip_init(nn.Linear, state_dim, fc_dims[0])), nn.ReLU()]
        )  # Input Layer
        for i in range(0, len(fc_dims) - 1):
            modules.extend(
                [
                    layer_init(skip_init(nn.Linear, fc_dims[i], fc_dims[i + 1])),
                    nn.ReLU(),
                ]
            )  # Hidden Layers
        modules.extend(  # "The policy output layer weights are initialised with the scale of 0.01"
            [
                layer_init(skip_init(nn.Linear, fc_dims[-1], action_dim), std=0.01),
                nn.Tanh(),
            ]
        )  # Output Layer
