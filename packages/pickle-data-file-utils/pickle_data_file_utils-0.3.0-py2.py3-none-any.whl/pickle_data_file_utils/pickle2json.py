...
from pickle_data_file_utils.helper import pickle_to_json
...
def main(infile: str, logfile: str, outdir: str, outfile: str, verbose: bool):
    """Convert pickle file to JSON format file."""
    error_ctr = 0
    ...
    if outfile is None:
        ...
        if basename.endswith(".json.pkl"):
            basename = basename.replace(".json.pkl", ".json")
        elif basename.endswith(".pkl"):
            basename = basename.replace(".pkl", ".json")
        else:
