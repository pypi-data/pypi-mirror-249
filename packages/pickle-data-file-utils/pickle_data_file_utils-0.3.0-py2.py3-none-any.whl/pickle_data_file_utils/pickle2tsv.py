...
from pickle_data_file_utils.helper import pickle_to_tsv
...
def main(infile: str, logfile: str, outdir: str, outfile: str, verbose: bool):
    """Convert pickle file to tab-delimited format file."""
    error_ctr = 0
    ...
    if outfile is None:
        ...
        if basename.endswith(".tsv.pkl"):
            basename = basename.replace(".tsv.pkl", ".tsv")
        elif basename.endswith(".pkl"):
            basename = basename.replace(".pkl", ".tsv")
        else:
