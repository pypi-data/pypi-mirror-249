    ...
    with TemporaryDirectory() as tmpdir:
        ...
        if 'ocp_release_image' in overrides:
            ...
            spec['volumes'] = [{'name': 'certs', 'hostPath': {'path': "/etc/pki/ca-trust/extracted/pem",
                                                              "type": "Directory"}}]
            if os.path.exists('/root/containers'):
                new_entry = {'mountPath': '/etc/containers', 'name': 'containers'}
                spec['containers'][-1]['volumeMounts'].append(new_entry)
                new_entry = {'name': 'containers', 'hostPath': {'path': "/root/containers", "type": "Directory"}}
                spec['volumes'].append(new_entry)
            pod['spec'] = spec
            with open('/root/pod.yml', 'w') as f:
