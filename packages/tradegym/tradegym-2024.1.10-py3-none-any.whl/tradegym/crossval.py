...
class BackTest():
    ...
    def _get_vb_backtest(self, symbol, lot=0.1):
    ...
    def get_stat_by_symbol_df(self, lot=0.1) -> pd.DataFrame:
        select_metrics = [
            'End Value', 
            'Total Return [%]', 
            'Benchmark Return [%]', 
        ...
        for symbol in self.env.symbols:
            pf = self._get_vb_backtest(symbol=symbol, lot=lot)
            stats = pf.stats()
    ...
    def get_plot_trades_by_symbol(self, symbol, lot=0.1,  template=None) -> plotly.graph_objs.Figure:
        pf = self._get_vb_backtest(symbol=symbol, lot=lot)
