...
def group_cycles_by_year(cycles: list[dict]) -> dict:
    """
    Groups Hestia `Cycle`s by year using their end date.

    Multi-year cycles are counted for each year between their
    end and start dates.
    ...
    -------
    dict
        The annualised dictionary of `Cycle`s by year.
    """

    start_dates = [cycle.get('startDate', None) for cycle in cycles]
    end_dates = [cycle.get('endDate') for cycle in cycles]

    def group_cycles(groups: dict, index: int):
        try:
            cycle = cycles[index]
            ...
            repeat = int(max(1, abs(diff_in_years(start_date, end_date)))) if start_date else 1
            range_end = int(end_date[0:4]) + 1
            range_start = range_end - repeat

            for year in range(range_start, range_end):
                groups[year] = groups.get(year, []) + [cycle]

        except IndexError:
            pass
        return groups

    grouped = reduce(group_cycles, range(0, len(end_dates)), dict())
    return grouped
