...
class Lea(object):
    ...
    def given_prob(self, *evidences, p):
        """ returns a new Alea instance from current distribution, such that
            ...
            is false; each evidence is either a boolean or a Lea instance with
            boolean values;
    ...
    def given(self, *evidences):
        """ returns a new Ilea instance representing the current distribution
            constrained by the given evidences, which are each either a boolean or
            ...
            evidences;
            note: the resulting probability distribution (Alea instance) is
            calculated at the time the Ilea instance is evaluated ('calc' method);
            at that time, an exception shall be raised if the given evidences are
            unfeasible
        """
    ...
    def such_that(self, *constraints):
        """ returns an instance of Tlea representing a conditional probability table (CPT)
            defined as self constrained by the given constraints, which are each either a
            boolean or a Lea instance with boolean values; the values present in the returned
            distribution are those and only those compatible with the given AND of constraints;
            unlike the 'given' method, the instance returned by such_that is independent of self
        """
        constraints = tuple(coerce(constraint) for constraint in constraints)
        lea_children = frozenset(lea_child for constraint in constraints
                                           for lea_child in constraint.get_leaves_set(restricted=True))
        lea_children -= self.get_leaves_set(restricted=True)
        joint_alea = joint(*lea_children, self).given(*constraints).calc(sorting=False)
        values_dict = defaultdict(list)
        for (v, p) in joint_alea._gen_vp():
            values_dict[v[:-1]].append((v[-1], p))
        lea_dict = {k: pmf(vps) for (k, vps) in values_dict.items()}
        return Tlea.build(joint(*lea_children), lea_dict)
...
def if_(cond_lea, then_lea, else_lea=Lea._DUMMY_VAL, prior_lea=Lea._DUMMY_VAL):
    """ returns an instance of Tlea representing the
