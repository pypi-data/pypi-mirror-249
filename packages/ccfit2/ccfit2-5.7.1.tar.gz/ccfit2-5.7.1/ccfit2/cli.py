...
def dc_mode_func(uargs):
    ...
    if all(not model.fit_status for model in models):
        ut.cprint('\n    ***Error***:', 'red')
        ut.cprint('All fits failed.', 'red')
        sys.exit(1)
    # Exit if only one temperature for all points
...
def relaxation_mode_func(uargs):
    ...
    if uargs.filetype == 'rate':
        dataset = _dataset_class.from_rate_files(uargs.input_files)
    elif uargs.filetype == 'legacy':
        # Create dataset to store relaxation rate and temperature data
        dataset = _dataset_class._from_ccfit2_files(
            uargs.input_files
        )
    else:
        # Create dataset to store relaxation rate and temperature data
        dataset = _dataset_class.from_ccfit2_csv(
            uargs.input_files
        )
