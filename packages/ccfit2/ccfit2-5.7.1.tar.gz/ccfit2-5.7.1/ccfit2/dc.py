...
class DoubleExponentialModel(Model):
    ...
    def calc_lntau_fit_ul(tau_star1: ArrayLike, tau_star2: ArrayLike,
                          ...
                          beta1_std: ArrayLike,
                          beta2_std: ArrayLike) -> list[list[float | NDArray]]:
        '''
        Calculates upper and lower bounds of ln(tau) from fit uncertainty\n
        in fitted parameters
        ...
        bounds1 = [upper1, lower1]
        bounds2 = [upper2, lower2]

        args = [
            tau_star1, tau_star1_std, tau_star2,
            tau_star2_std, beta1, beta2, beta1_std, beta2_std
        ]
        if any(isinstance(arg, float) for arg in args):
