...
def start(filePath, correlation='PearsonR(+)', minsize=3, threshold=0.95, mininc=0.01, split_tol = 0.8, epochs = 5,random_seed=42, Generate_Features = True, tolerance_list = None , weight=True,
         gplearn = True, gpl_dummyfea = None, population_size = 500, generations = 100, verbose = 1, 
    ...
    :param gplearn : Whether to call the embedded gplearn package of TCLR to regress formula (default=True).
        ...
        if correlation == 'MIC' or correlation == 'R2':
            ...
            if gpl_dummyfea == None: 
                ...
                gpmodel = genetic.SymbolicRegressor(
                    ...
                    verbose = verbose,feature_names = sr_featurname[:-5],function_set = function_set,
                    metric = metric,stopping_criteria=0.01, p_crossover=0.7, p_subtree_mutation=0.1,
                           p_hoist_mutation=0.05, p_point_mutation=0.1,max_samples=0.9,parsimony_coefficient=0.01,
                    )
                formula = gpmodel.fit(sr_data[:,:-5], sr_data[:,-3])
                score = gpmodel.score(sr_data[:,:-5], sr_data[:,-3])
                ...
                # remove fea_loc
                index_array = [i for i in range(len(sr_featurname)-5)]
                for i in range(len(gpl_dummyfea)):
