...
logtc_context = contextvars.ContextVar('logtc_context', default="logtc")
...
class Logtc:
    ...
    def setup_file_handler(self, file_path: str, when: str, interval: int):
        if self.file_handler:
            raise Exception("logtc: File handler existed")
    ...
    def setup_stream_handler(self):
        if self.stream_handler:
            raise Exception("logtc: Stream handler existed")
        self.stream_handler = logging.StreamHandler()
        self.stream_handler.setLevel(self.level)
        self.stream_handler.setFormatter(self.formatter)
        self.logger.addHandler(self.stream_handler)
        print("logtc: Add stream handler")
    ...
    def __setup_socket_connection(self):
        ...
        print("logtc: Add socket handler")
    ...
    def remove_socket_logging(self):
        if self.socket_handler:
            print("logtc: Remove socket handler")
