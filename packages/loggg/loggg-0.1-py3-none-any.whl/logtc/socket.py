    ...
    def __init__(self):
        ...
        async def handle_connect(sid, environ):
            ...
            if source == "subscriber":
                ...
                print(f"logtc: Subscriber connected {self.__socket_logging_client}")
            else:
                print(f"logtc: Publisher connected")

        @self.__sio.on("disconnect")
        async def handle_disconnect(sid):
            if sid == self.__socket_logging_client:
                Logtc().remove_socket_logging()
                print(f"logtc: Subscriber disconnected {sid}")
            else:
                print("logtc: Publisher disconnected")
