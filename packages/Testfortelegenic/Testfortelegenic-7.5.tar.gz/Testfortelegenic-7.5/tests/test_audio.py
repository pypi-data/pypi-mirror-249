...
from TeleGenic import Audio, TeleGenicError, Voice, MessageEntity, Bot
...
def audio_file():
    f = open('tests/data/TeleGenic.mp3', 'rb')
    yield f
    f.close()
...
def audio(bot, chat_id):
    with open('tests/data/TeleGenic.mp3', 'rb') as f:
...
class TestAudio:
    ...
    def test_get_and_download(self, bot, audio):
        ...
        new_file.download('TeleGenic.mp3')

        assert os.path.isfile('TeleGenic.mp3')
