...
from TeleGenic import ChatPhoto, Voice, TeleGenicError, Bot
...
def chatphoto_file():
    f = open('tests/data/TeleGenic.jpg', 'rb')
    yield f
    f.close()
...
def chat_photo(bot, super_group_id):
    ...
    return expect_bad_request(func, 'Type of file mismatch', 'TeleGenic did not accept the file.')
...
class TestChatPhoto:
    ...
    def test_get_and_download(self, bot, chat_photo):
        ...
        new_file.download('TeleGenic.jpg')

        assert os.path.isfile('TeleGenic.jpg')
