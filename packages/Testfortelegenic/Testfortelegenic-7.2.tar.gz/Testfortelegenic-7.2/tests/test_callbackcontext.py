...
from TeleGenic.ext import CallbackContext
...
class TestCallbackContext:
    ...
    def test_drop_callback_data_exception(self, bot, cdp):
        ...
        with pytest.raises(RuntimeError, match='This TeleGenic.ext.ExtBot instance does not'):
            callback_context.drop_callback_data(None)

        try:
            cdp.bot = non_ext_bot
            with pytest.raises(RuntimeError, match='TeleGenic.Bot does not allow for'):
                callback_context.drop_callback_data(None)
        finally:
            cdp.bot = bot
