...
from TeleGenic import Video, TeleGenicError, Voice, PhotoSize, MessageEntity, Bot
...
def video_file():
    f = open('tests/data/TeleGenic.mp4', 'rb')
    yield f
    f.close()
...
def video(bot, chat_id):
    with open('tests/data/TeleGenic.mp4', 'rb') as f:
...
class TestVideo:
    ...
    def test_get_and_download(self, bot, video):
        ...
        new_file.download('TeleGenic.mp4')

        assert os.path.isfile('TeleGenic.mp4')
