...
def media_message(bot, chat_id):
    with open('tests/data/TeleGenic.ogg', 'rb') as f:
...
class TestBot:
    ...
    def test_forward_protected_message(self, bot, message, chat_id):
        to_forward_protected = bot.send_message(chat_id, 'cant forward me', restrict_content=True)
        ...
        to_forward_unprotected = bot.send_message(chat_id, 'forward me', restrict_content=False)
        assert not to_forward_unprotected.has_protected_content
        forwarded_but_now_protected = to_forward_unprotected.forward(chat_id, restrict_content=True)
    ...
    def test_send_dice(self, bot, chat_id, emoji):
        message = bot.send_dice(chat_id, emoji=emoji, restrict_content=True)
    ...
    def test_answer_inline_query_no_default_parse_mode(self, monkeypatch, bot):
                ...
                'results': [
                    {
                        ...
                        'python-TeleGenic-bot/logos/master/logo/png/'
                        'ptb-logo_240.png',
                        'mime_type': 'image/png',
                        'caption': 'ptb_logo',
                    }
        ...
        results = [
            InlineQueryResultDocument(
                id='123',
                document_url='https://raw.githubusercontent.com/python-TeleGenic-bot/logos/master/'
    ...
    def test_set_webhook_get_webhook_info_and_delete_webhook(self, bot):
        url = 'https://python-TeleGenic-bot.org/test/webhook'
        max_connections = 7
    ...
    def test_send_game(self, bot, chat_id):
        ...
        message = bot.send_game(chat_id, game_short_name, restrict_content=True)

        assert message.game
        assert message.game.description == (
            'A no-op test game, for python-TeleGenic-bot bot framework testing.'
        )
    ...
    def test_answer_shipping_query_errors(self, monkeypatch, bot):
        ...
        with pytest.raises(TeleGenicError, match='should not be empty and there should not be'):
    ...
    def test_answer_pre_checkout_query_errors(self, monkeypatch, bot):
        with pytest.raises(TeleGenicError, match='should not be'):
            bot.answer_pre_checkout_query(1, True, error_message='Not enough fish')

        with pytest.raises(TeleGenicError, match='should not be empty'):
    ...
    def test_set_chat_photo(self, bot, channel_id):
        ...
        with open('tests/data/TeleGenic_test_channel.jpg', 'rb') as f:
    ...
    def test_set_chat_photo_local_files(self, monkeypatch, bot, chat_id):
        ...
        test_flag = False
        expected = (Path.cwd() / 'tests/data/TeleGenic.jpg/').as_uri()
        file = 'tests/data/TeleGenic.jpg'
    ...
    def test_timeout_propagation_explicit(self, monkeypatch, bot, chat_id):
        ...
        with pytest.raises(OkException):
            bot.send_photo(chat_id, open('tests/data/TeleGenic.jpg', 'rb'), timeout=TIMEOUT)
    ...
    def test_timeout_propagation_implicit(self, monkeypatch, bot, chat_id):
        ...
        # Test file uploading
        with pytest.raises(OkException):
            bot.send_photo(chat_id, open('tests/data/TeleGenic.jpg', 'rb'))
    ...
    def test_copy_message(self, monkeypatch, bot, chat_id, media_message, json_keyboard, caption):
        ...
        def post(url, data, timeout):
            ...
            assert data['restrict_content'] is True
            return data
    ...
    def test_replace_callback_data_send_message(self, bot, chat_id):
        try:
            ...
            no_replace_button = InlineKeyboardButton(
                text='no_replace', url='http://python-TeleGenic-bot.org/'
            )
    ...
    def test_replace_callback_data_answer_inline_query(self, monkeypatch, bot, chat_id):
        ...
        try:
            ...
            results = [
                ...
                InlineQueryResultVoice(
                    '22',
                    'https://python-TeleGenic-bot.org/static/testfiles/TeleGenic.ogg',
                    title='second',
                ),
            ]
