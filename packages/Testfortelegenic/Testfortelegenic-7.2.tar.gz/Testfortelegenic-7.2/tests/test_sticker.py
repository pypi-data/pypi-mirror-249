...
from TeleGenic import Sticker, PhotoSize, TeleGenicError, StickerSet, Audio, MaskPosition, Bot
...
def sticker_file():
    f = open('tests/data/TeleGenic.webp', 'rb')
    yield f
    f.close()
...
def sticker(bot, chat_id):
    with open('tests/data/TeleGenic.webp', 'rb') as f:
        return bot.send_sticker(chat_id, sticker=f, timeout=50).sticker


@pytest.fixture(scope='function')
def animated_sticker_file():
    f = open('tests/data/TeleGenic_animated_sticker.tgs', 'rb')
    yield f
    f.close()
...
def animated_sticker(bot, chat_id):
    with open('tests/data/TeleGenic_animated_sticker.tgs', 'rb') as f:
        return bot.send_sticker(chat_id, sticker=f, timeout=50).sticker


class TestSticker:
    # sticker_file_url = 'https://python-TeleGenic-bot.org/static/testfiles/TeleGenic.webp'
    ...
    def test_send_all_args(self, bot, chat_id, sticker_file, sticker):
        message = bot.send_sticker(
            chat_id, sticker=sticker_file, disable_notification=False, restrict_content=True
        )
    ...
    def test_get_and_download(self, bot, sticker):
        ...
        new_file.download('TeleGenic.webp')

        assert os.path.isfile('TeleGenic.webp')
    ...
    def test_error_send_empty_file(self, bot, chat_id):
        with pytest.raises(TeleGenicError):
...
class TestStickerSet:
    ...
    def test_bot_methods_1_png(self, bot, chat_id, sticker_file):
        with open('tests/data/TeleGenic_sticker.png', 'rb') as f:
    ...
    def test_bot_methods_1_tgs(self, bot, chat_id):
        assert bot.add_sticker_to_set(
            ...
            tgs_sticker=open('tests/data/TeleGenic_animated_sticker.tgs', 'rb'),
            emojis='😄',
        )
