...
from TeleGenic import Audio, Voice, TeleGenicError, MessageEntity, Bot
...
def voice_file():
    f = open('tests/data/TeleGenic.ogg', 'rb')
    yield f
    f.close()
...
class TestVoice:
    ...
    def test_get_and_download(self, bot, voice):
        ...
        new_file.download('TeleGenic.ogg')

        assert os.path.isfile('TeleGenic.ogg')
