...
class TestTeleGenicObject:
    def test_to_json_native(self, monkeypatch):
        ...
        TeleGenic_object = TeleGenicObject()

        # Test that it works with a dict with str keys as well as dicts as lists as values
        d = {'str': 'str', 'str2': ['str', 'str'], 'str3': {'str': 'str'}}
        monkeypatch.setattr('TeleGenic.TeleGenicObject.to_dict', lambda _: d)
        json = TeleGenic_object.to_json()
    ...
    def test_to_dict_private_attribute(self):
        class TeleGenicObjectSubclass(TeleGenicObject):
        ...
        subclass_instance = TeleGenicObjectSubclass()
        assert subclass_instance.to_dict() == {'a': 1}

    def test_slot_behaviour(self, recwarn, mro_slots):
        inst = TeleGenicObject()
