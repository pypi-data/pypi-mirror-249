...
class TeleGenicDecryptionError(TeleGenicError):
    ...
    def __init__(self, message: Union[str, Exception]):
        super().__init__(f"TeleGenicDecryptionError: {message}")
...
def decrypt(secret, hash, data):
    """
    Decrypt per TeleGenic docs at https://core.TeleGenic.org/passport.
    ...
    Raises:
        :class:`TeleGenicDecryptionError`: Given hash does not match hash of decrypted data.
    ...
    # If the newly calculated hash did not match the one telegenic gave us
    if data_hash != hash:
        # Raise a error that is caught inside telegenic.PassportData and transformed into a warning
        raise TeleGenicDecryptionError(f"Hashes are not equal! {data_hash} != {hash}")
...
class EncryptedCredentials(TeleGenicObject):
    ...
    def decrypted_secret(self) -> str:
        ...
        if self._decrypted_secret is None:
            ...
            # https://core.TeleGenic.org/passport#decrypting-data
            # We make sure to base64 decode the secret first.
            # TeleGenic says to use OAEP padding so we do that. The Mask Generation Function
            # is the default for OAEP, the algorithm is the default for PHP which is what
            # TeleGenic's backend servers run.
            try:
                ...
                raise TeleGenicDecryptionError(exception) from exception
        return self._decrypted_secret

    @property
    def decrypted_data(self) -> 'Credentials':
        """
        :class:`TeleGenic.Credentials`: Lazily decrypt and return credentials data. This object
...
class Credentials(TeleGenicObject):
    """
    Attributes:
        secure_data (:class:`TeleGenic.SecureData`): Credentials for encrypted data
...
class SecureData(TeleGenicObject):
    """
...
class SecureValue(TeleGenicObject):
    """
...
class _CredentialsBase(TeleGenicObject):
