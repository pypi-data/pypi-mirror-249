...
def preprocess_1(image, input_size, mean, std):
    # 传入图像默认为RGB
    img = np.array(image).astype(np.float32)
    if img.shape[:2] == input_size:
        padded_img = img
    else:
        padded_img = cv2.resize(img, (input_size[1], input_size[0]), interpolation=cv2.INTER_LINEAR)
...
def preprocess(image, input_size):
    ...
    if img.shape[:2] == input_size:
        padded_img = img.astype(np.uint8)
    else:
        # 传入图像默认为RGB
