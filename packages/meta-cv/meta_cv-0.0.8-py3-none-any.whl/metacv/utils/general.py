...
def preprocess(img, input_size, pad=None, normal=None, mean=None, std=None, swap=None):
    ...
    if img.shape[:2] == input_size:
        ...
        if pad is not None:
            pad_img = np.ones((input_size[0], input_size[1], 3), dtype=np.float32) * 114
            resized_img = cv2.resize(img,
                                     (int(round(img.shape[1] * r)), int(round(img.shape[0] * r))),
                                     interpolation=cv2.INTER_LINEAR).astype(np.float32)
            pad_img[: int(round(img.shape[0] * r)), : int(round(img.shape[1] * r))] = resized_img
        else:
...
def postprocess_2(predictions, input_size, score_thr, nms_thr, feat_stride_fpn=[8, 16, 32]):
    ...
    for idx, stride in enumerate(feat_stride_fpn):
        ...
        bbox_preds = predictions[idx * 3 + 1] * stride
