...
class Palettizer(ABC):
    ...
    """
    @abstractmethod
    def init_model_and_test_data(self, **cfg: Dict[Any]) -> None:
    ...
    def profile_mixed_bit_response(self,
        ...
        if top_k_in_default_dtype is not None:
            per_layer_divs = list(self.per_layer_results[str(max(allowed_nbits))].values())
            layer_names = list(self.per_layer_results[str(max(allowed_nbits))].keys())
        ...
        for i in trange(NUM_MIXED_BIT_RECIPES):
            recipe = {}
    ...
    def convert_current_model_to_coreml(self, output_names: List[str]) -> None:
        ...
        from tests.coreml import _create_coreml_model
        self.coreml_model = _create_coreml_model(
            self.model.to(torch.float32), self.torch_jit_trace_data, output_names)
    ...
    def apply_recipe_coreml(self, recipe_key: str) -> None:
        ...
        for nbits in active_nbits:
            ...
            logger.info(
                f"{precision_stats[nbits]['num_tensors']} tensors are palettized with {nbits} bits")
