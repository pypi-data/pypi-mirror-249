...
class TestAuth(unittest.TestCase):
    ...
    @patch("package.auth.decode_token")
    @patch("package.auth.get_codararc_value")
    @patch("package.auth.start_gunicorn")
    @patch("package.auth.check_for_signal_file")
    @patch("package.auth.stop_gunicorn")
    def test_login(self, mock_stop_gunicorn, mock_check_signal, mock_start_gunicorn, mock_get_token, mock_decode_token,
                   mock_webbrowser, mock_os_path_exists):
        mock_get_token.return_value = "token"
        mock_decode_token.return_value = {"email_verified": True}
        mock_os_path_exists.return_value = False
        auth.login()
        mock_webbrowser.assert_called_once()
        mock_start_gunicorn.assert_called_once()
