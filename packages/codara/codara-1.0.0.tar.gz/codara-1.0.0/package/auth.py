...
from package.server import app, start_gunicorn, stop_gunicorn, check_for_signal_file
...
def login(port=54371):
    if os.path.exists(os.path.expanduser('~/.codararc')):
        ...
        gunicorn_process = start_gunicorn('package.server:app', port)
        ...
        stop_gunicorn(gunicorn_process)
