...
global height, width
width, height = 800, 600  # Adjust the resolution as needed
resolution = width  # Remove the redundant assignment
...
with open(resource_filename('py_raytrace', 'objects.json'), 'r') as json_file:
    objects_data = json.load(json_file)

# Define multiple spheres
spheres = objects_data.get("spheres", [])
floor = objects_data.get("floor", {})
...
def trace_ray(origin, direction, depth=0):
    ...
    for sphere in spheres:
        ...
        if discriminant > 0:
            ...
            if 0 < t < closest_t:
                closest_t = t
                closest_object = {"type": "sphere", "properties": sphere}

    # Check for intersect with floor
    denom = np.dot(floor["normal"], direction)
    if abs(denom) > 1e-6:
        t = np.dot(floor["position"] - origin, floor["normal"]) / denom
        if t > 0 and (closest_object is None or t < closest_t):
            closest_t = t
            intersection_point = origin + t * direction
            normal = floor["normal"]
            closest_object = {"type": "floor", "properties": {"color": get_floor_color(intersection_point)}}

    if closest_object:
        intersection_point = origin + closest_t * direction

        if closest_object["type"] == "sphere":
            normal = (intersection_point - closest_object["properties"]["position"]) / closest_object["properties"]["radius"]
        else:
            normal = floor["normal"]
        ...
        # Convert color to NumPy array before multiplication
        final_color = np.array(closest_object["properties"]["color"]) * diffuse_intensity

        reflection_direction = direction - 2 * np.dot(direction, np.array(normal)) * np.array(normal)
...
# create a single slider for adjusting both width and height
ax_resolution = plt.axes([0.25, 0.15, 0.65, 0.03], facecolor='lightgoldenrodyellow')
slider_resolution = Slider(ax_resolution, 'Resolution', 100, 1204, valinit=width)
...
def res_render(val):
    global img, height, width, camera_direction, resolution, image
    resolution = int(val)

    # Calculate the new width and height in proportion to the resolution
    scale_factor = resolution / max(width, height)
...
def update(val):
    global img, height, width, camera_direction, resolution
    resolution = int(slider_resolution.val)

    slider_resolution.on_changed(res_render)
    camera_direction = np.array([
        np.sin(np.radians(slider_camera_yaw.val)) * np.cos(np.radians(slider_camera_pitch.val)),
        np.sin(np.radians(slider_camera_pitch.val)),
        -np.cos(np.radians(slider_camera_yaw.val)) * np.cos(np.radians(slider_camera_pitch.val))
    ])
