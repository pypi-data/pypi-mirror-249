...
def ocr_pdf_file(pdf_file_path):
    ...
    for i, image in enumerate(images):
        ...
        result_text.append(ocr_text_result)
    return "\n".join(result_text)


def img_url2img(img_url):
    filename = download_file(url)
    ocr_it(encode_image(filename))
    pass


def ocr_img_file_to_text(image_path):
...
def process_msg_text(msg_json, process_text_rule_dict, debug=False):
    ...
    if "img" not in msg_json:
        ...
        markdown_text = lyytext.batch_strip_text(markdown_text, process_text_rule_dict["strip_characters"])
