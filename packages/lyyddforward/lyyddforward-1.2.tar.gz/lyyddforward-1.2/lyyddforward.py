...
def format_mengxia(item, number, debug=False):
...
def format_remove_subtext(text, to_remove_list):
    for item in to_remove_list:
        text = text.replace(item, "")
    return text


def format_trim_characters(text, trim_words_list):
    for item in trim_words_list:
        text = text.strip(item)
    return text
