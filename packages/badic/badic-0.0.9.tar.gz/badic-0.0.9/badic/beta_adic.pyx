...
cdef class BetaAdicSet:
	...
	def is_included(self, a, m=1, verb=False):
		"""
...
#		return (log(len(S)) / (niter * abs(log(m))))
	...
	def proj(self, other=None, pl=None, pls=None, plo=None, arel=None, arel0=None, t=0, m=1, closure=False, verb=False):
		r"""
		...
		if arel is None:
			if pl is None and pls is None and plo is None:
				arel = BetaAdicSet(self.b, aa).relations_automaton(BetaAdicSet(self.b, ab),
										  t=t, m=m, arel0=arel0, ext=closure, verb=verb-1)
				if verb > 0:
					print("arel = %s" % arel)
				r = arel.proji(1)
				if verb > 1:
			...
			arel = get_beta_adic(self.b, pls).relations_automaton(get_beta_adic(self.b, plo), t=t, m=m,
									  arel0=arel0, ext=closure, verb=verb-1)
		if verb > 0:
			print("arel = %s" % arel)
		# browse the result
		A = aa.alphabet
...
#
	...
	def domain_exchange (self, pl=None, n=None, ndiam=30, interior=True, testPisot=True, arel0=None, algo_inter=1, pref=True, verb=False):
		r"""
	...
	def find_invariance (self, pl=None, proj=True, kmax=30, verb=False):
		r"""
		...
		for k in range(1,kmax+1):
			if verb > 0:
				print("k=%s..." % k)
			if pl is None:
				mp = self.proj(m=self.b**k)
				mp = mp.proj(self, m=self.b**(-k))
				if self.a.included(mp.a):
					if verb > 0:
						print("k=%s works !!" % k)
					break
			else:
				mp = self.proj(pl, pl=pl, m=self.b**k)
				if not mp.a.included(self.a):
					continue
				mp = mp.proj(self, pl=pl, m=self.b**(-k))
				if self.a.included(mp.a):
					if verb > 0:
						print("k=%s works !" % k)
					break
	...
	def first_return_map (self, de, pl, lm=None, darel=None, interior=True, algo_inter=1, n=None, verb=False):
		"""
		...
		INPUT:
			...
			- ``n`` - int (default: ``None``) - stop after n steps
		...
		if lm is None:
			...
			to_see = [([i], getDetAutomaton(self, m)) for i,m in enumerate(lm)]
		R = []
		if n is None:
			n = 0
		while len(to_see) > 0:
	...
	def substitution (self, de=None, pl=None, interior=True, algo=1, algo_inter=1, get_domains=False, n=None, proj=True, pref=True, verb=False):
		r"""
		...
		if de is None:
			de = self.domain_exchange(pl=pl, arel0=arel0) #interior=interior, arel0=arel0)
		if interior:
	...
	def relations_automaton(self, other=None, t=0, m=1, arel0=None, algo=1, ext=False, prune=True, minimize=True, verb=False):
		"""
		...
		if arel0 is None:
			for p,bo in Pe:
				if abs_val(K, p, t) > bo - .0000000001:
					break
			else:
				if verb > 0:
					print("Could be computed directly with algo 1")
			arel0 = bb.relations_automaton(algo=algo, A=A, B=B, m=m, ext=ext, keep_labels=True, prune=False)
		if verb > 0:
			print("arel0 = %s" % arel0)
		S0 = set(arel0.states)
		if verb > 1:
		...
		while len(to_see) > 0:
			...
			for j1 in range(self.a.a.na):
				...
				for j2 in range(a2.a.na):
					...
					if z not in S0:
						for p,bo in Pe:
							if abs_val(K, p, z) > bo - .0000000001:
								break
						else:
							if verb > 2:
								print("%s avoided" % z)
