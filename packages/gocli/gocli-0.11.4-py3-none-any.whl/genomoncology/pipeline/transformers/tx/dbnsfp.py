...
NAME_MAPPING = {
    ...
    "phyloP30way_mammalian__float": "phyloP470way_mammalian",
    "phyloP30way_mammalian_rankscore__float": "phyloP470way_mammalian_rankscore",  # noqa E501
    ...
    "phastCons30way_mammalian__float": "phastCons470way_mammalian",
    "phastCons30way_mammalian_rankscore__float": "phastCons470way_mammalian_rankscore",  # noqa E501
    ...
    # arup
    "BayesDel_noAF_score__mfloat": "BayesDel_noAF_score",
    "BayesDel_noAF_rankscore__mfloat": "BayesDel_noAF_rankscore",
    "BayesDel_noAF_pred__mstring": "BayesDel_noAF_pred",
    "BayesDel_addAF_pred__mstring": "BayesDel_addAF_pred",
    "BayesDel_addAF_rankscore__mfloat": "BayesDel_addAF_rankscore",
    "BayesDel_addAF_score__mfloat": "BayesDel_addAF_score",
}
...
def none_to_nan(x):  # pragma: no mccabe
    ...
    for k, v in x.items():
        if isinstance(v, list) and k.endswith("__mstring"):
            if None in v:
        ...
        if isinstance(v, list) and (
            ...
            if all(val in values_to_purge for val in x[k]):
                keys_marked_for_removal.append(k)
        if isinstance(v, list) and (k.endswith("__mint")):
...
def remove_blanks_from_bayesdel_dataset(x):
    for k, v in x.items():
        if k.startswith('BayesDel') and '' in v:
            x[k] = [val for val in v if val != '']
    return x
...
register(
    ...
    transformer=compose(
        ...
        remove_blanks_from_bayesdel_dataset,
        none_to_nan,
