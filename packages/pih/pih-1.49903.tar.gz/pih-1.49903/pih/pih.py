...
class PIH:
    ...
    class UPDATE:
        ...
        def install_module(
            ...
            result: bool | None = PIH.EXECUTOR.extract_returncode(
            ...
            if n(result):
                ...
                return str(PIH.RESULT.SSH.execute(js(command_list), host).data).find(js(("Successfully installed", module_name))) != -1
            else:
                return result
    ...
    class EXECUTOR:
            ...
            process_result: CompletedProcess = PIH.EXECUTOR.execute(
                DataTool.check_not_none(
                    ...
                    lambda: PIH.EXECUTOR.create_command_for_psexec(
                        ...
                        PIH.DATA.VARIABLE.value(LINK.ADMINISTRATOR_PASSWORD),
                        True,
                    ),
                    ping_command,
                ),
        ...
        def python_version(host: str | None = None) -> str | None:
            ...
            if PIH.OS.host_is_local(host):
                host = None
            processs_result: CompletedProcess = PIH.EXECUTOR.execute(
                if_else(
                    n(host),
                    command,
                    lambda: PIH.EXECUTOR.create_command_for_psexec(
            ...
            stderr: str = processs_result.stderr
            if ne(stderr):
                if (
                    stderr.find(js(("could not start", CONST.PYTHON.EXECUTOR_ALIAS)))
                    != -1
                ):
                    return None
            return j(str(processs_result.stdout).strip().split(" ")[1:])
        ...
        def create_command_for_executor(
            ...
            login = if_else(
                e(login),
                None,
                lambda: login if use_raw_login else j((AD.DOMAIN_NAME, login), "\\"),
            )
            return (
                [
        ...
        @staticmethod
        def extract_returncode(value: CompletedProcess, check_on_success: bool = False) -> int | bool | None:
            returncode: int = value.returncode
            return True if returncode == 0 else None if returncode in [2, 2250, 2120] else False if check_on_success else returncode
        ...
        def process_is_exists(
            ...
            password = password or PIH.DATA.VARIABLE.value(LINK.ADMINISTRATOR_PASSWORD)
            if ne(host):
                command_list += ["/s", host]
                command_list += ["/u", login]
                command_list += ["/p", password]
            output: str = (
                PIH.EXECUTOR.execute(command_list, True, True, command_as_text=False)
    ...
    class EVENT:
        ...
        def new_polibase_scanned_document_detected(
            value: PolibaseScannedDocument,
        ) -> None:
            PIH.EVENT.send(
                Events.NEW_POLIBASE_DOCUMENT_DETECTED,
                (value.pin, value.file_path, value.document_name, 0),
            )
        ...
        class ADMIN:
            ...
            def start(
                ...
                try:
                    ...
                    command.append(j((ARGUMENT_PREFIX, "isolated")))
                    command.append("0")
                    if ne(parameters):
                        if isinstance(parameters , list) or isinstance(parameters, Tuple):
                            for parameter_item in parameters:
                                command.append(parameter_item)
                        else:
                            command.append(parameters)
                    process_result: CompletedProcess = PIH.EXECUTOR.execute(
                        command, show_output
                    )
                    if n(PIH.EXECUTOR.extract_returncode(process_result, True)):
                        return False
                    return True
                except Exception as error:
    ...
    class OS:
        ...
        def make_sudo(command: str, password: str) -> str:
            return js(("echo -e", nl(password), "| sudo -S", command))
