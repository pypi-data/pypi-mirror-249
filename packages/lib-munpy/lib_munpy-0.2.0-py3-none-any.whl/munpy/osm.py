...
from src.munpy import config
from src.munpy.general import findStreets, haversine_distance, dat_to_db
...
class OSMHandler(SimpleHandler):
    ...
    def create_munich_files(self, save=False, merge=False, merge_length=10.0):
        ...
        if save:
            self.street_df.rename(columns={'street_id': '#id'}).to_csv(
                os.path.join(config.LEZ_DIR, f'{city}/domain/street.dat'),
            ...
            street_database.to_csv(os.path.join(config.LEZ_DIR, f'{city}/domain/street.csv'))
            self.intersection_df.to_csv(
                os.path.join(config.LEZ_DIR, f'{city}/domain/intersection.csv'),
    ...
    def _writeIntersections(self):
        ...
        filepath = os.path.join(config.LEZ_DIR, f'{city}/domain/intersection.dat')
...
if __name__ == '__main__':
    import json
    from src.munpy.general import dat_to_geojson

    city = 'valencia'
    city_dir = os.path.join(config.LEZ_DIR, city)
    domain_dir = os.path.join(city_dir, 'domain')
    osm_file = os.path.join(domain_dir, 'map.osm')

    handler = OSMHandler(city)
    handler.apply_file(osm_file)

    streets, intersections = handler.create_munich_files(save=True)
    # streets.to_csv('/home/ngomariz/st.csv', index=False)
    # intersections.to_csv('/home/ngomariz/inter.csv', index=False)
    domain_gjs = dat_to_geojson(streets, intersections, color='red')

    with open(os.path.join(domain_dir, 'street.geojson'), 'w') as gj:
        json.dump(domain_gjs, gj, indent=2)
