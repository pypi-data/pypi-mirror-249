...
def sync_http_route(payload, ctx, vault_id):
    ...
    try:
        ...
        print(result)
        print(result.body)
        result = normalize_one(result.body["data"])
...
def normalize_one(route):
    """
    This method is extracted from `normalize` in the VGS SDK.
    TODO: refactor and replace where it belongs.
    """
    for filter_item in route["attributes"]["entries"]:
