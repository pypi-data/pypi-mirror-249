...
def find_invalid_expressions(content: str) -> list[ExprError]:
    expressions = preprocessor.get_expressions(content)
    res: list[ExprError] = []
...
def parse_file(fname: str | Path, teaching: bool = False) -> Parsed:
    ...
    propaccess = {
        ...
        for match in re.finditer(prop_regex, comments_removed)
    }
