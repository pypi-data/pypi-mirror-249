...
class Task:
    ...
    def subscribe(self, source: Subscription) -> None:
        # The inability to subscribe to multiple streams is a platform limitation,
        # not a desirable property - when we can do this, we should allow it.
        if len(self.subscriptions) > 0:
            peer = next(iter(self.subscriptions))
            if peer.stream() != source.stream():
                raise SeaplaneError(
                    "{peer.subject} and {source.subject} can't be associated with the same task"
                    " since they're part of different streams."
                )
...
class TaskFactory:
    ...
    def __call__(
        self, instance_name: Optional[str] = None, replicas: int = 1, ack_wait_secs: int = 8 * 60
    ) -> Task:
        if instance_name is None:
            instance_name = f"{self.task_name}-default"
...
class App:
    ...
    def edge(
        self,
        a: Union[Task, TaskFactory, Bucket, Subscription],
        b: Union[Task, TaskFactory, Subject],
    ) -> None:
        """
        ...
        if isinstance(a, TaskFactory):
            a = a()

        if isinstance(b, TaskFactory):
            b = b()

        # Type tomfoolery is for ergonomics
        if not (isinstance(a, Task) or isinstance(b, Task)):
        ...
        if isinstance(a, Task) and isinstance(b, Task):
            ...
            # TODO foolery here.
            if isinstance(a.subject, _TaskSubject):
                # This is a typical, addresssable subject that we know how to match
                a = a.subject.task_subscription("*")
            else:
                raise SeaplaneError(
                    f"can't listen to task subject {a.subject.subject} automatically."
                    " To listen to this, you'll need to use an explicit Subscription"
                    " and write the Seaplane carrier filter yourself"
                )

        if isinstance(a, Task) and isinstance(b, Subject):
            a.subject = b

        elif isinstance(a, Subscription) and isinstance(b, Task):
            b.subscribe(a)
        else:
