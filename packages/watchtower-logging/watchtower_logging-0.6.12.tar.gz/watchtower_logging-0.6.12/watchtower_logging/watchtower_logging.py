    ...
    if self.isEnabledFor(lvl_num):
        ...
        try:
            ...
            if not sys.exc_info()[2] is None:
                ...
                # Extracting code context using linecache
                context_lines = linecache.getlines(last_call_stack.filename)
                start_line = max(last_call_stack.lineno - 3, 0)  # Example: 3 lines of context
                end_line = min(last_call_stack.lineno + 2, len(context_lines))  # Adjust as needed
                code_context = context_lines[start_line:end_line]
                index = last_call_stack.lineno - start_line - 1
