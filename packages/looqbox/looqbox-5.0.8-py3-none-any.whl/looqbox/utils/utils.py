...
def format_date_range(header, start_date, end_date, language):
    ...
    return header + language["var_from"] + formatted_start_date + language["var_to"] + formatted_end_date \
          + date_range_name([start_date_as_object, end_date_as_object], language.get("language_source", GlobalCalling.looq.language))
...
def title_with_date(header: str = None, date_int: list = None, language: str = GlobalCalling.looq.language):
    ...
    if _period_does_not_have_start_date(date_int):
        return header + f" {language_vars['period_end']} " + format(date_int[1], determine_format_type(date_int[1]).lower(), language_vars)
    if _period_does_not_have_end_date(date_int):
        return header + f" {language_vars['period_start']} "+ format(date_int[0], determine_format_type(date_int[0]).lower(), language_vars)
