...
from lemon_scheduler.configs.local_dev_config import config
from settings import BASE_DIR

base_model_file_template: str
with open(os.path.join(BASE_DIR, "lemon_scheduler", "lemon_runtime", "models_template.py"), "r") as template_file:
...
def generate_data_model_stub_v2():
    clients = [DesignClient(**config.lemon_scheduler.dict())]
    ...
    while True:
        ...
        if need_update:
            ...
            with open(os.path.join(BASE_DIR, "lemon_scheduler", "lemon_runtime", "models.py"), "w") as f:
...
def generate_data_model_stub():
    ...
    rag_module_client = DesignClient(**config.lemon_scheduler.dict())
    ledger_module_client = DesignClient(**config.lemon_scheduler.dict())
    while True:
