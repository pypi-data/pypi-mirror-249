...
from deepchecks_llm_client.data_types import Tag, EnvType, AnnotationType, Interaction, LogInteractionType, Step
from deepchecks_llm_client.openai_instrumentor import OpenAIInstrumentor
from deepchecks_llm_client.utils import get_timestamp, HandleExceptions, set_verbosity, HandleGeneratorExceptions
...
class DeepchecksLLMClient:
    ...
    def log_interaction(self,
        ...
        app_version = self._get_version_object(version_name=self.api._version_name)
    ...
    @HandleExceptions(logger)
    def log_batch_interactions(self, interactions: t.List[LogInteractionType]):
        app_version = self._get_version_object(version_name=self.api._version_name)
        result = self.api.log_batch(app_version_id=app_version["id"], interactions=interactions)
        return result.json()
    ...
    def data_iterator(self,
                      ...
                      end_time: datetime | int | None = None) -> t.Iterable[Interaction]:
        """
        ...
        """
        version = self._get_version_object(version_name)
        ...
        while True:
            interactions = \
                self.api.get_interactions(version["id"],
    ...
    @HandleExceptions(init_logger)
    def get_data(self, environment: EnvType | str,
                 version_name: t.Union[str, None] = None,
