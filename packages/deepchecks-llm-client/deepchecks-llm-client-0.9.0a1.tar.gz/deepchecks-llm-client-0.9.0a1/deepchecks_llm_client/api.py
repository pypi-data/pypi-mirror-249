...
from deepchecks_llm_client.data_types import Tag, EnvType, AnnotationType, Step, LogInteractionType
...
class API:
    ...
    def log_batch(self, app_version_id: int, interactions: t.List[LogInteractionType]):
        for interaction in interactions:
            interaction.raw_json_data = self._tags

        return maybe_raise(
            self.session.post(
                f"application-versions/{app_version_id}/interactions",
                json={
                    "env_type": self._env_type.value,
    ...
    def log_interaction(self,
        ...
        return maybe_raise(
            self.session.post(
                f"application-versions/{app_version_id}/interactions",
                json={"env_type": self._env_type, "interactions": [interaction]}
            ),
            expected=201
        )
    ...
    def get_interactions(self, application_version_id: int,
                         ...
                         end_time_epoch: int | None) -> t.List:
        return maybe_raise(
