...
class DataHandler(object):
    ...
    def predict_with_single_epoch(self, eeg, acc, clf, cached_feature_arr):
        ...
        cached_feature = None
        if cached_feature_arr is None or len(cached_feature_arr) % raw_feature_count != 0:
            cached_feature = features
        else:
            cached_feature = pd.DataFrame(columns=features.columns)
            cached_feature_count = (len(cached_feature_arr) // raw_feature_count)
        ...
        excluded_n3 = raw_n3[np.where(raw_score[:, 2][raw_n3] < 0.5)[0]]
