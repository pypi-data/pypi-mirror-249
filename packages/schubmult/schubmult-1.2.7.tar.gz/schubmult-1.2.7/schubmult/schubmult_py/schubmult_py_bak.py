...
def schubmult(perm_dict,v):
	...
	mn_th = [th[i] for i in range(len(th))]
	for index in range(len(th)):
		for vp in vpathdicts[index]:
			for v2,vdiff,s in vpathdicts[index][vp]:
				if th[index]-vdiff > mx_th[index]:
					mx_th[index] = th[index] - vdiff
				if th[index]-vdiff < mn_th[index]:
					mn_th[index] = th[index] - vdiff
	sum_mn_th = [sum(mn_th[i+1:]) for i in range(len(th))]
	...
	for u,val in perm_dict.items():
		...
		for index in range(len(th)):
			...
			for up in vpathsums:
				inv_up = inv(up)
				newperms = elem_sym_perms(up,min(mx_th[index],inv_mu-inv_vmu-(inv_up-inv_u)-sum_mn_th[index]),th[index])
				for up2, udiff in newperms:
					if udiff < mn_th[index]:
						continue
