...
def fakeJsoncomFOR(json_dataFOR: Dict[str, str]) -> Dict[str, str]:
    novo_json_data = {}

    for key, value in json_dataFOR.items():
        if value in tipo_dados_mapper:
            novo_json_data[key] = tipo_dados_mapper[value]()
        else:
            raise HTTPException(status_code=400, detail=f"Tipo de dado não suportado para a chave '{key}': {value}")
