...
class TestParseManifest(unittest.TestCase):
    ...
    def test_parse_manifest_with_node_match_regex(self):
        ...
        node_match_regex = 'model*'
        expected_dependencies = {
            'model.model1': ['source1', 'source2'],
            'model.model2': ['model.model1'],
            'source1': [],
            'source2': [],
        }
        nodes, dependencies = get_nodes_and_dependencies(manifest, node_match_regex=node_match_regex)
        self.assertDictEqual(dependencies, expected_dependencies)
