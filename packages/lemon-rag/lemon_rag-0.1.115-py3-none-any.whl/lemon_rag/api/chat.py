...
from lemon_rag.llm.agents.prompts.default_agent import default_rag_chat
...
from lemon_rag.protocols.chat import ChatRole, Session, KnowledgeBase, Message
from lemon_rag.utils import log
from lemon_rag.utils.file_utils import get_file_hash
from lemon_rag.utils.response_utils import response, ErrorCodes, stream
...
def send_message(request: SendMessageRequest):
    ...
    if session.assistant_role != ChatRole.assistant:
        return response(code=ErrorCodes.permission_denied)

    user_message, ai_message = data_access.create_message_and_response(
        get_user(), session, request.text, request.client_ts
    )
    default_kb = data_access.get_default_knowledge_base(get_user())
    file_ids = []
    if default_kb:
        file_ids = [link.file.id for link in default_kb.file_links]
    ref = retrieve_related_paragraphs(request.text, file_ids)
    task = partial(default_rag_chat, request.text, [], ref, user_message, ai_message)
    return stream(task)
