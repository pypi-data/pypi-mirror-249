...
def install_directly(package:str, v: str):
    wait_for_pypi_index(package, v)
    url = f"https://lemon.lemonstudio.tech:8443/{config.lemon_rag.app_uuid}test/{config.lemon_rag.tenant_uuid}/restful/v1/ai_assistant/upgrade_package"
    print(url)
    res = requests.post(
        url,
        json={"package_content": base64.b64encode(open(f"dist/lemon_rag-{v}-py3-none-any.whl", "rb").read()).decode()}
    )
    res.raise_for_status()
