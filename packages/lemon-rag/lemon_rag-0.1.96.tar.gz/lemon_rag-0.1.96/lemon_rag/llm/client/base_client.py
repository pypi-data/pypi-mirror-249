...
from lemon_rag.settings import lemon_env, LemonEnv
...
def get_embedding_function(metrics_name: str) -> Embedding_F:
    embeddings: List[Optional[OpenAIEmbeddings]] = [OpenAIEmbeddings() if lemon_env == LemonEnv.dev else None]
...
llm: List[Optional[ChatOpenAI]] = [None]


def get_llm():
    if llm[0] is None:
        openai_config: Optional[models.OPENAIConfig] = models.OPENAIConfig.get_or_none()
        llm[0] = ChatOpenAI(openai_api_base=openai_config.base_url, openai_api_key=openai_config.api_key)
    return llm[0]
...
class ChatChain(BaseModel):
    """
    一次call的版本信息，messages是浅拷贝的，可以用于重试的时候。
    """
    ...
    def run(self, **kwargs):
        if not self.functions:
            print(kwargs, self.template.format_messages(**kwargs))
            return self.llm.generate([self.template.format_messages(**kwargs)])
        chain = create_openai_fn_chain(self.functions, self.llm, self.template)
