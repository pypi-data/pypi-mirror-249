...
{# 这是一个注释，它将不会出现在渲染的输出中 #}
根据给定的数据源、元数据和问题生成一个或多个结构化查询语句（structure_queries）
或提供可能缺失的表名（possible_missing_table）。

{# 
  根据给定的数据源、元数据和问题，首先评估问题的完整性。
  如果问题不完整（例如"查工资"没有说清楚查谁的工资）,则需要更多信息（need_feedback）
  如果问题完整，直接生成一个或多个结构化查询语句（structure_queries）或提供可能缺失的表名（possible_missing_table）。
#}

请注意以下准则：
1. 仔细分析问题，确定是否缺少关键数据，若缺失，给出表名即可。
2. 根据数据源类型和元数据来判断是否需要多条查询语句，若需要，则生成多条。
3. 在生成查询语句时，请注意是否需要对标记为 is_filter 的字段进行过滤。
{# 3. 如果问题不完整，提供一个明确的反馈信息（need_feedback），说明需要补充哪些信息。 #}
...
{# 
  - 首先检查问题的完整性。
  - 如果问题完整，根据数据源和问题需求，生成 structure_queries 或指出 possible_missing_table。
  - 如果问题不完整，给出具体的反馈信息（need_feedback），指明需要补充的信息。
#}
