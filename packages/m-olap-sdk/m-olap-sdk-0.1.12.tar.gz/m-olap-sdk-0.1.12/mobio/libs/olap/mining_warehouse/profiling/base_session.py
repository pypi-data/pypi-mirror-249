...
class RoutingSession(Session):
    ...
    def get_bind(self, mapper=None, clause=None):
        ...
        if self._flushing or isinstance(clause, (Update, Delete)):
            ...
            for key in list(self.dict_shards):
                value = self.dict_shards.get(key)
                if value and value.get(Engines.ROLE) == EngineRole.FOLLOWER and (
                    value.get(Engines.ALIVE) is True or value.get(Engines.ALIVE) is None
                ):
...
class BaseSession:
    def __init__(self, olap_uri):
        ...
        self.SessionLocal = sessionmaker(
            ...
            dict_shards=Engines(uri=self.olap_uri).dict_shards,
        )
