...
def test_is_similar_dict_validator():
    reference_dict = {'key1': 1, 'key2': 'value2', 'key3': {'nested_key': 'nested_value'}}

    @ParameterValidator("param").is_similar_dict(reference_dict, "Value must be a similar dictionary")
    def example_function(param):
        return param

    assert example_function(param={'key1': 2, 'key2': 'value3', 'key3': {'nested_key': 'nested_value2'}})

    with pytest.raises(ParameterValidationError) as exc_info:
        example_function(param={'key1': 1, 'key2': 'value2'})
    assert "Value must be a similar dictionary" in str(exc_info.value)
