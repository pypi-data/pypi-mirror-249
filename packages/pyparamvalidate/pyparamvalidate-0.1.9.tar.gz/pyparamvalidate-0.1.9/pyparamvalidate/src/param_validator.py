...
from pyparamvalidate.utils.dict_utils import DictUtility
...
class ParameterValidator:
    ...
    def is_file(self, exception_msg=None):
        return self.add_validator(lambda value: os.path.isfile(value), exception_msg)

    def is_dir(self, exception_msg=None):
        return self.add_validator(lambda value: os.path.isdir(value), exception_msg)

    def is_similar_dict(self, dictionary, exception_msg=None, ignore_keys_whitespace=True):
        def validate(value):
            dict_util = DictUtility()
            return dict_util.is_similar_dict(value, dictionary, ignore_keys_whitespace)

        return self.add_validator(validate, exception_msg)
