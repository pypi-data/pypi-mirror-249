...
from hgraph import TS, SCALAR, TIME_SERIES_TYPE, TSD, compute_node, REMOVE_IF_EXISTS, SCALAR_1, SCALAR_2, TS_SCHEMA, \
    TSB, graph
...
def bundle_to_tsd(tsb: TSB[TS_SCHEMA]) -> TSD[str, TIME_SERIES_TYPE]:
    """
    Converts a bundled TSD into a TSD of string keys and TIME_SERIES_TYPE values.
    For this to work, it is a requirement that the bundle only contain keys with the same value type.
    """


@graph
def tsd_to_bundle(tsd: TSD[str, TIME_SERIES_TYPE], schema: TS_SCHEMA) -> TSB[TS_SCHEMA]:
    ...
