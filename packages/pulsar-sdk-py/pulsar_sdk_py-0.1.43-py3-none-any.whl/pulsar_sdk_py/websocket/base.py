...
from pulsar_sdk_py.sleep import wait_for_condition
...
class WebSocketClient:
    ...
    async def create_new_websocket_connection(self, attempt: int = 1) -> None:
        ...
        try:
            await wait_for_condition(lambda: self.websocket_conn and self.websocket_conn.state == State.OPEN, 10000)
        except asyncio.TimeoutError as e:
            await self.websocket_conn.close()
            if attempt >= 3:
                raise APIError("WebSocket connection timeout", status_code=408) from e
    ...
    async def handle_websocket_loop(self) -> None:
        try:
            ...
            await self.handle_websocket_closure(e)
        except (CancelledError, ConnectionClosedOK) as e:
            return
        except Exception as e:
            await self.handle_websocket_closure(e)

    async def handle_websocket_closure(self, e: ConnectionClosedError | Exception) -> None:
                ...
                if not future.future.done():
                    if isinstance(e, ConnectionClosedError):
                        future.set_exception(
                            APIError(f"WebSocket connection closed unexpectedly: {e.reason}", status_code=e.code)
                        )
                    else:
                        future.set_exception(
                            APIError(f"Unexpected error occurred while handling WebSocket: {e}", status_code=500)
                        )
    ...
    async def wait_for_message(self, request_id: str) -> Any:
        ...
        try:
            ...
            raise APIError("WebSocket response timeout.", status_code=408) from e
