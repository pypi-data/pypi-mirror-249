...
class OptionData:
    def __init__(self, base_response):
        ...
        data = base_response.get('expireDateList', [])
    ...
    def portfolio_gamma(self, gammas, ois):
        if gammas is not None and ois is not None:
    ...
    def portfolio_vega(self, vegas, ois):
        if vegas is not None and ois is not None:
    ...
    def portfolio_theta(self, thetas, ois):
        if thetas is not None and ois is not None:
            weighted_thetas = [theta * oi for theta, oi in zip(thetas, ois)]
    ...
    def average_implied_volatility(self):
        ...
        if ois.size == 0 or (ois < 0).any():
            print(ValueError("Open interest array is empty or contains negative values."))
        
        # Ensure ivs and ois have the same length
        if len(ivs) != len(ois):
            print(ValueError("The length of IVs and OIs must be the same."))
    ...
    def implied_volatility_skew(self, ivs, strike_prices, underlying_close):
        if not isinstance(underlying_close, (int, float)):
            print(TypeError("underlying_close must be a single numeric value."))
