...
def process_socket_request(wmdf, query):
    query = query.strip("\n").strip()
    print("get request from socket server, query=", query)
    #  解析查询条件
    try:
        result = wmdf.query(query)
...
def update_mysql_from_wmdf(df, engine=None, debug=False):
    ...
    if debug: print(max_dayint_dict)
...
def update_signal_txt(df, debug=True):
    debug = True
    if debug: print("enter update_signal_txt, input para len=", len(df))
    ...
    if debug: print("get the row with max dayint")
    ...
    if debug: print("apply code 666 to grouped_df")

    if debug: print(grouped_df, "----------------here is grouped df---------------")
    data_list = []
    ...
    # df_chonggao  =  grouped_df.apply(lambda  x:  pd.DataFrame([{'market':  lyystkcode.get_market(x['code']), 'code':  x['code'],'signal_id':  '666','text':'','number':  x['chonggao']}]), axis=1)
    # # df_chonggao.reset_index(inplace=True, drop=True)
    # print(df_chonggao,"up is grouped df")
    # if debug: print("apply df_huitoubo to grouped_df")
    # # df_huitoubo.reset_index(inplace=True, drop=True)
    # df_huitoubo = grouped_df.apply(lambda  x:  pd.DataFrame([{'market':  lyystkcode.get_market(x['code']), 'code':  x['code'],'signal_id':  '665','text':'','number':  x['chonggao']}]), axis=0)
    # print(df_huitoubo)
    ...
    # df2['text'] = ""
    if debug: print("concat df_chonggao,df_huitoubo,df_reason")
    ...
    if debug: print("contact finished. Try to filter no gbk code")
...
def get_wmdf_and_last_date(q=None, debug=False):
    if debug: print("enter datacenter")
    old_df = lyytools.get_data_from_cache_or_func("stock_wmdf.pkl", 3600 * 8, read_data_from_sql, debug=True)
    if debug: print("try to ogg in get wmdf last date")
...
if __name__ == '__main__':
    ...
    if len(stkcode_list) < 5000: print("stkcode_list<5000,check it")
    server_list = lyywmdf.perfact_new_fast_server_list(nextfuntion=instance_lyymysql.get_tdx_server_list)
    if len(server_list) < 10: print("server_list<10,check it")
