...
def depth_to_world_coordinates(midas_depth_value, transformation_matrix, intrinsic_matrix, pixel_coordinates):
    x, y = pixel_coordinates

    z = 1.0 / midas_depth_value if midas_depth_value != 0 else 0

    homogeneous_image_point = np.array([x * z, y * z, z, 1])

    point_camera = np.dot(np.linalg.inv(intrinsic_matrix), homogeneous_image_point[:3])

    homogeneous_camera_point = np.append(point_camera, 1)

    point_world_homogeneous = np.dot(transformation_matrix, homogeneous_camera_point)
