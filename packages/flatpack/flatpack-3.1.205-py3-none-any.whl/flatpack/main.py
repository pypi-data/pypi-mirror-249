...
def create_point_cloud_image(depth_map):
    try:
        ...
        return point_cloud_image, pcd_points
    except Exception as e:
        print(f"Error in create_point_cloud_image: {e}")
        return None


def map_point_cloud_to_image(image_np, pcd_points, depth_map):
    gray_image = cv2.cvtColor(image_np, cv2.COLOR_BGR2GRAY)
    gray_image_3ch = cv2.cvtColor(gray_image, cv2.COLOR_GRAY2BGR)

    for point in pcd_points:
        x, y, z = point
        if 0 <= x < gray_image_3ch.shape[1] and 0 <= y < gray_image_3ch.shape[0]:
            depth_readable = z

            text = f"({int(x)}, {int(y)}, {depth_readable:.2f})"

            font = cv2.FONT_HERSHEY_SIMPLEX

            cv2.circle(gray_image_3ch, (int(x), int(y)), radius=2, color=(255, 255, 255), thickness=-1)
            cv2.putText(gray_image_3ch, text, (int(x) + 5, int(y)), font, 0.3, (255, 255, 255), 1, cv2.LINE_AA)
...
async def process_point_cloud(file: UploadFile = File(...)):
    try:
        contents = await file.read()
        original_image_np = open_and_convert_image(contents)

        if len(original_image_np.shape) == 3 and original_image_np.shape[2] == 3:
            depth_map = cv2.cvtColor(original_image_np, cv2.COLOR_BGR2GRAY)
        else:
            depth_map = original_image_np

        _, pcd_points = create_point_cloud_image(depth_map)

        final_image = map_point_cloud_to_image(original_image_np, pcd_points)

        resized_img = cv2.resize(final_image, (256, 256))
        jpeg_quality = 50
