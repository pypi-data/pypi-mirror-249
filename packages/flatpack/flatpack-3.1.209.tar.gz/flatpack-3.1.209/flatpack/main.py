...
async def process_depth_map(file: UploadFile = File(...), pose_data: str = Form(...),
    ...
    try:
        ...
        if pose_data:
            ...
            depth_map_colored = fpk_process_depth_map_np(image_np, model_type, colormap)

            annotated_image = depth_map_colored.copy()
            ...
            for detection in detection_result.detections:
                ...
                label = f"{detection.categories[0].category_name} ({world_coordinates})"
                cv2.putText(annotated_image, label, (int(bbox.origin_x), int(bbox.origin_y) - 10),
