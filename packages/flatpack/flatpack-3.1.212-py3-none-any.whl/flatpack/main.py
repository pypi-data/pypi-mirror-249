    ...
    try:
        ...
        if pose_data:
            ...
            for detection in detection_result.detections:
                ...
                x, y, z = world_coordinates.ravel()

                label_x = f"x: {x:.2f}"
                label_y = f"y: {y:.2f}"
                label_z = f"z: {z:.2f}"

                cv2.putText(annotated_image, label_x, (int(bbox.origin_x), int(bbox.origin_y) - 30),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
                cv2.putText(annotated_image, label_y, (int(bbox.origin_x), int(bbox.origin_y) - 15),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
                cv2.putText(annotated_image, label_z, (int(bbox.origin_x), int(bbox.origin_y)),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
