...
async def process_depth_map(file: UploadFile = File(...), pose_data: str = Form(...),
    ...
    try:
        ...
        depth_map = fpk_process_depth_map_np(image_np, model_type, colormap)
        ...
        if pose_data:
            ...
            for detection in detection_result.detections:
                ...
                label1 = f"{detection.categories[0].category_name}"
                label2 = f"({world_coordinates})"
                ...
                cv2.putText(annotated_image, label1,
                            (int(bbox.origin_x), int(bbox.origin_y) - text_size1[1] - text_size2[1] - 10),
                            font, font_scale, text_color, font_thickness, line_type)

                cv2.putText(annotated_image, label2, (int(bbox.origin_x), int(bbox.origin_y) - text_size2[1] - 10),
                            font, font_scale, text_color, font_thickness, line_type)

            resized_img = cv2.resize(annotated_image, (256, 256))
            jpeg_quality = 50
            ...
            return JSONResponse(status_code=400, content={"message": "Pose data is required for processing"})
    except Exception as e:
