...
def map_point_cloud_to_image(image_np, pcd_points, depth_map):
    color_map = cv2.applyColorMap(cv2.convertScaleAbs(depth_map, alpha=0.03), cv2.COLORMAP_JET)

    for point in pcd_points:
        x, y, z = point
        if 0 <= x < image_np.shape[1] and 0 <= y < image_np.shape[0]:
            color = color_map[y, x]
            cv2.circle(image_np, (int(x), int(y)), radius=2, color=color, thickness=-1)
...
async def process_point_cloud(file: UploadFile = File(...)):
    try:
        ...
        final_image = map_point_cloud_to_image(original_image_np, pcd_points, depth_map)
