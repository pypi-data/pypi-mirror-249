...
async def process_depth_map(file: UploadFile = File(...), pose_data: str = Form(...),
    ...
    try:
        ...
        if pose_data:
            ...
            for detection in detection_result.detections:
                ...
                object_details.append({
                    ...
                    "coordinates": {
                        "x": round(world_coordinates[0], 2),
                        "y": round(world_coordinates[1], 2),
                        "z": round(world_coordinates[2], 2)
                    }
                })

        resized_img = cv2.resize(depth_map, (256, 256))
        jpeg_quality = 50
        ...
        response_content = {
            "image": StreamingResponse(io.BytesIO(encoded_img.tobytes()), media_type="image/jpeg"),
