...
class ClamsPack:
    ...
    def html_export_packages(clams_packs: list[ClamsPack], path_name: str, html_exporter: HTMLDocExport) -> list:
        ...
        out_index = ClamsPack.html_export_index(clams_packs, "docs", html_exporter)
        ...
        for i in range(len(clams_packs)):
            ...
            html_exporter.prev_module = None if i == 0 else clams_packs[i - 1].name
            html_exporter.next_module = None if i + 1 == len(clams_packs) else clams_packs[i + 1].name
            out_html = clams_pack.html_export_clams("docs", html_exporter)
            out.extend(out_html)
