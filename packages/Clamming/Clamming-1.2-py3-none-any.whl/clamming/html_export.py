...
class HTMLDocExport:
    ...
    HTML_HEAD = \
        ...
        <head>
            ...
            <title>{TITLE}</title>
            <link rel="logo icon" href="../etc/{FAVICON}" />
            <link rel="stylesheet" href="../etc/{THEME}.css" type="text/css" />
    ...
    def get_header(self) -> str:
        ...
        if len(self.__icon) > 0:
            h.append('        <p><img class="small-logo" src="../etc/{:s}" '
                     'alt="Software logo"/></p>'.format(self.__icon))
        h.append("    </header>")
        return "\n".join(h)
    ...
    def get_nav(self) -> str:
        ...
        if self.__icon != HTMLDocExport.DEFAULT_ICON:
            nav.append("    <img class=\"small-logo center\" src=\"../etc/{:s}\" alt=\"\"/>".format(self.__icon))
        nav.append("    <hr>")
