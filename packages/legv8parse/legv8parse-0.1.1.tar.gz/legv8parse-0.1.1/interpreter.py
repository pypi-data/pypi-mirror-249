...
def parse():
    ...
    if len(branchlocs) ==0:
        branches[""]=branchline
    ir = dict()
    pc = (branches[branchlocs[0]], 0)
    for i in range(len(branchlocs)):
        try:
            branches[branchline[branchlocs[i]].replace(":", "")] = branchline[branchlocs[i]+2:branchlocs[i+1]]
        except IndexError:
            branches[branchline[branchlocs[i]].replace(":", "")] = branchline[branchlocs[i]+2:]
    if len(branchlocs)>0:
        branches[branchline[branchlocs[-1]].replace(":", "")] = branchline[branchlocs[-1]+2:]
...
if __name__ == "__main__":
    print(interpretOne(parse()[''][0], registers, flags, pc))