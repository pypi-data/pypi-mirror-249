...
use nom::{
    ...
    character::complete::{char, digit1, one_of, alphanumeric1, alpha1},
    branch::alt,
...
pub fn branch_name(input: &str)-> IResult<&str, String>{
    let (input,alp1) = alpha1(input)?;
    let (input,alpn1) = alphanumeric1(input)?;
    Ok((input, (alp1.to_string()+alpn1)))
  }
