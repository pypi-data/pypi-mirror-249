...
class BinaryWrapper:
    ...
    def unpack(self, payload:bytes) -> bytes:
        i = WRAPPER_HEAD_LENGTH
        j = i + len(VERSION_BYTES)
        k = j + len(self.payload_wrapper.agent_id.encode())
        # logger.debug(f"i: {i}, j: {j}, k: {k}")
        # logger.debug(f"version: {payload[i: j]}")
        # logger.debug(f"sender: {payload[j: k]}")
        payload_json = {
            "version":  payload[i: j].decode(),
            "sender":   payload[j: k].decode(),
            "receiver": payload[j: k].decode(),
            "content":  payload[k:]
        }
    ...
    def wrap_for_response(self, payload:bytes, managed_request_payload:bytes) -> bytes:
        ...
        head_items = [WRAPPER_RESPONSE_HEAD_BYTES, 
                      VERSION_BYTES, 
                      receiver_agent_id.encode('utf-8')]
        head_bytes = b''.join(item for item in head_items)
