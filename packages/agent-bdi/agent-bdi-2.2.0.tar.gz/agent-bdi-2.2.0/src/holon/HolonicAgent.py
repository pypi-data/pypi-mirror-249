...
# =====================
    ...
    def _on_response(self, topic, managed_payload):
    ...
    def _on_request(self, topic, managed_payload):
        response_topic, response_payload = self.on_request(topic, managed_payload["content"])
        logger.debug(f"response_topic: {response_topic}, response_payload: {len(response_payload)}")
        if response_topic:
...
# ==================================
    ...
    def _on_message(self, topic:str, payload):
        logger.debug(f"payload: {len(payload)}")
        if self._payload_wrapper.is_request(payload):
            managed_payload = self._payload_wrapper.unpack(payload)
            logger.debug(f"is_request, managed_payload: {len(managed_payload['content'])}")
            self._on_request(topic, managed_payload)
        elif self._payload_wrapper.is_response(payload):
            managed_payload = self._payload_wrapper.unpack(payload)
            logger.debug(f"is_response, managed_payload: {len(managed_payload['content'])}")
            self._on_response(topic, managed_payload)
        else:
            logger.debug(f"is not managed payload")
