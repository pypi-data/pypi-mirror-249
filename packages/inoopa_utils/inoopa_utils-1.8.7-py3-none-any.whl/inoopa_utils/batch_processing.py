...
def parralelize_company_processing_and_saving(
    ...
    This queue will be consumed in a separate thread to avoid blocking the main thread (processing)
    ...
    :param processing_function: function to use to process the companies. Any function used here should these params: 
        ...
        - `dead_letter_queue_name` as a parameter. This is the name of the queue to push the companies in case of error
                ...
                with Session() as session:
                    with ThreadPoolExecutor() as executor:
                        processed_companies = list(executor.map(
                            partial(processing_function, dead_letter_queue_name=dead_letter_queue_name, session=session), 
                            processing_function_params,
                        ))
            else:
                with ThreadPoolExecutor() as executor:
                    processed_companies = list(executor.map(
                        partial(processing_function, dead_letter_queue_name=dead_letter_queue_name), 
                        processing_function_params,
                    ))
