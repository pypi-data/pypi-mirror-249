...
def _fn_crawl(args: Args) -> None:
    def traverse_jobs(jobs: Sequence[Json], path: Path = Path()) -> Iterator[Job]:
        """Recursively traverse a raw Jenkins job tree and yield validated Job instances"""
        for job_path, raw_job in ((path / job["name"], job) for job in jobs):
            if (class_name := raw_job["_class"].split(".")[-1]) == "Folder":
                print(f"Folder({raw_job['name']}, jobs: {len(cast(list[Any], raw_job['jobs']))})")
                yield from traverse_jobs(raw_job["jobs"], job_path)
            elif class_name == "WorkflowJob":
                yield Job.model_validate(jenkins.get_job_info(job_path.as_posix()))
            elif class_name in {"FreeStyleProject"}:
                pass
            else:
                raise Fatal(f"Don't know class type {class_name}")

    with jenkins_client(**extract_credentials(args.credentials), timeout=args.timeout) as jenkins:
        for job in traverse_jobs(jenkins.get_jobs()):
            job.expand(jenkins, max_build_infos=None)
            print(job)
            for build_nr, build in job.build_infos.items():
