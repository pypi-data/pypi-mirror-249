...
def eviction(request, id):
    eviction = get_object_or_404(Eviction, id=id)
    if not (eviction.active or request.user.has_perm("evictionsplit:management")):
    ...
    applicants = []
    if eviction.has_applicants() and request.user.has_perm("evictionsplit:management"):
...
def stop_eviction(request, eviction_id):
    ...
    return redirect("evictionsplit:management")
