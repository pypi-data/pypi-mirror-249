...
def get_dashboard_nodes(xmlfiles: list[tuple[str, str]]) -> list[tuple[str, str]]:
...
def get_dashboard_nodes_all(app: "App", file_view: "FileView") -> list[tuple[str, str]]:
    """Helper function to return SXML dashboard root nodes."""
    findings = []
    try:
...
def find_xml_nodes_usages(xml_files: list[tuple[str, str]], nodes: list[xml_node]) -> list[tuple[bs4.Tag, str]]:
...
def find_xml_nodes_usages_all(app: "App", file_view: "FileView", nodes: list[xml_node]) -> list[tuple[str, str]]:
    """Helper function to find xml node usage."""
    #  Outputs not_applicable if no xml files found
    findings = []
    for directory, filename, ext in file_view.iterate_files(basedir="ui/views"):
