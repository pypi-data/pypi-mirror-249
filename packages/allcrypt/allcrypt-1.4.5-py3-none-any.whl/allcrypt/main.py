...
def main():
    ...
    def perform_file_operation(mode, input_path, output_path, compress, shred):
        try:
            ...
            fernet = Fernet(key)
            # setup for gzip or not
            # setup for shredding or not

            # block to block copy function     
            ...
            if mode == "Encrypt":
                ...
                with open(output_path, 'wb') as output_file, open(input_path, 'rb') as input_file:
                    chunk_iterator = iter(lambda: input_file.read(chunk_size), b'')
                    if compress:
                        with gzip.open(output_file, 'wb') as compressed_file:
                            for chunk in chunk_iterator:
                                compressed_file.write(fernet.encrypt(chunk))
                            print('Compressed')
                    else:
                        
                        for chunk in chunk_iterator:
                            output_file.write(fernet.encrypt(chunk))
                if shred:
                ...
                with open(input_path, 'rb') as input_file, open(output_path, 'wb') as output_file:
                        chunk_iterator = iter(lambda: input_file.read(chunk_size), b'')
                        for chunk in chunk_iterator:
                            output_file.write(fernet.decrypt(chunk))
