...
class readabform:
    def __init__(self, text: int | Decimal | str) -> None:
        self.text = text
        self.abbrs = [
            "",
            "K",
    ...
    def form(self) -> int | float | str:
        if isinstance(self.text, (int, float)):
            test_num = self.text
            try:
                self.text = Decimal("{:.3g}".format(self.text))
            ...
            try:
                numeric_part = Decimal(self.text[:-1])
...
class form:
    def __new__(cls, text: int | float | str) -> readabform:
