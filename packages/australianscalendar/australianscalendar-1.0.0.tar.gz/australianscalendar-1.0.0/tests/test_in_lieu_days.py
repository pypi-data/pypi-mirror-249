...
class InLieuTests(unittest.TestCase):
    def test_in_lieu_should_be_holiday(self):
        self.assertTrue(all(australians_calendar.is_holiday(_) for _ in australians_calendar.in_lieu_days))

    def test_in_lieu_should_equal_workdays_amount(self):
        min_year, max_year = min(australians_calendar.in_lieu_days.keys()).year, max(australians_calendar.holidays.keys()).year
        for year in range(min_year, max_year + 1):
            in_lieu_days_amount = len(
                list(filter(lambda date: date.year == year, australians_calendar.in_lieu_days.keys()))
            )
            workdays_amount = len(list(filter(lambda date: date.year == year, australians_calendar.workdays.keys())))
            if year == 2006:
    ...
    def test_in_lieu_should_be_weekday(self):
        for in_lieu_day in australians_calendar.in_lieu_days.keys():
