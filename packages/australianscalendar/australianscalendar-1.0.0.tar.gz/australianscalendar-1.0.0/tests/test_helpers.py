...
class HelperTests(unittest.TestCase):
    def test_get_dates(self):
        ...
        for start, end, duration in cases:
            ...
            dates = australians_calendar.get_dates(start, end)
    ...
    def test_get_workdays_holidays(self):
        ...
        for start, end, include_weekends, exclude_weekends, workdays, workdays_exclude_weekends in cases:
            start, end = datetime.date(*start), datetime.date(*end)
            self.assertEqual(include_weekends, len(australians_calendar.get_holidays(start, end)))
            self.assertEqual(exclude_weekends, len(australians_calendar.get_holidays(start, end, include_weekends=False)))
            self.assertEqual(workdays, len(australians_calendar.get_workdays(start, end)))
            self.assertEqual(
                workdays_exclude_weekends, len(australians_calendar.get_workdays(start, end, include_weekends=False))
            )
    ...
    def test_find_workday(self):
        ...
        cases = [[australians_calendar.find_workday(i, date) for i in range(-7, 7)] for date in dates]
