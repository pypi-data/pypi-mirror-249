...
class MultiProcess:
    ...
    def run_task_func(self, function, task_data):
        Interface.init_mongodb(self.config.get('MONGODB_CONFIG'))
        Interface.init_rabbitmq(self.config.get('RABBITMQ_CONFIG'))
        ...
        Interface.delete_run_task(service_name=self.work_name, service_ip=self.work_ip, task_id=task_data['task_id'])

    def mq_callback(self, ch, method, properties, body):
        ch.basic_ack(delivery_tag=method.delivery_tag)
        task_data = json.loads(body.decode())
        if 'task_id' in task_data:
            _server = Interface.get_service_by_name_and_ip(service_name=self.work_name, service_ip=self.work_ip)
            _max_work = _server.get('max_work')
            _run_work = len(_server.get('run_work'))

            if _run_work < _max_work:
                _stops = Interface.get_stop_tasks_by_task_id(task_data['task_id'])
                if len(_stops) < 1:
                    Interface.update_run_task(
...
if __name__ == '__main__':
    ...
    Interface.init_config(config_obj)
